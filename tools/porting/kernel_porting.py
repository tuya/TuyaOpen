...
DEFAULT_RETURN = {
    "OPERATE_RET": "OPRT_NOT_SUPPORTED", "VOID*": "NULL", "VOID": "",
    "VOID_T*": "NULL", "VOID_T": "", "UINT64_T": "0", "INT64_T": "0",
    "UINT32_T": "0", "INT32_T": "0", "INT_T": "0", "UINT_T": "0",
    "UINT16_T": "0", "INT16_T": "0", "UINT8_T": "0", "INT8_T": "0",
    "CHAR_T": "0", "SCHAR_T": "0"
}
...
class porting(object):
    def __init__(self, path, target, template=None) -> None:
        ...
        if (os.path.exists(self.target_tuyaos_kernel_config_path)):
            ...
            print(
                "     platform kernel config is ",
                self.target_tuyaos_kernel_config_path)
    ...
    def __load_files(self):
        ...
        for root, _, files in os.walk(self.target_tuyaos_adapter_path + "include/"):
            ...
            for f in files:
                nfile = porting_new_file.parse_new_file(
                    os.path.join(root, f)).load_file()
                ofile = porting_old_file.parse_old_file(
                    self.target_tuyaos_adapter_path + "src/" + re.sub("\\.h", ".c", f)).load_file()
                if ofile:
    ...
    def __gen_scripts(self):
        ...
        if not os.path.exists(self.target_path + "/platform_config.cmake"):
            ...
            shutil.copy(self.porting_template_path + "platform_config.cmake",
                        self.target_path + "/platform_config.cmake")

        # toolchain_file.cmake
        if not os.path.exists(self.target_path + "toolchain_file.cmake"):
            print("    cp toolchain_file.cmake template")
            shutil.copy(self.porting_template_path + "toolchain_file.cmake",
                        self.target_path + "/toolchain_file.cmake")

        # kconfig
        if not os.path.exists(self.target_path + "Kconfig"):
            print("    cp Kconfig template")
            shutil.copy(self.porting_template_path + "Kconfig",
                        self.target_path + "/Kconfig")
        ...
        if not os.path.exists(self.target_path + "build_example.sh"):
            ...
            shutil.copy(self.porting_template_path + "build_example.sh",
                        self.target_path + "/build_example.sh")

        if self.abilitys['OPERATING_SYSTEM'] == '100':
            print("    cp makefile")
            shutil.copy(self.porting_template_linux_path +
                        "Makefile", self.target_path + "/Makefile")
    ...
    def __gen_adapter_default(self):
        ...
        # include directory, can be empty, kernel header files can be put in
        # during component management in the development environment
        if not os.path.exists(self.target_tuyaos_adapter_path + "include"):
            print("    create include dir")
            os.makedirs(self.target_tuyaos_adapter_path +
    ...
    def __gen_adapter(self):
        ...
        for d in self.dirs:
            ...
            for f in d['files']:
                ...
                # security-related adapter files can be not generated to avoid
                # compilation issues.
                ...
                if (f['isnew']):
                    ...
                    if self.template_path:
                        if os.path.exists(self.template_path + f['name']):
                            print(
                                "        new file:", f['name'] + ", use template")
                            shutil.copy(
                                self.template_path + f['name'], self.target_tuyaos_adapter_path + "/src/")
                            continue

                    # If there is no template, regenerate
                    print("        new file:", f['name'])
                else:
                    # Update old header files (may have new interfaces), to be
                    # opened after the kernel is officially built
                    print("        merge file: ", f['name'])

                fd = open(self.target_tuyaos_adapter_path +
                          "/src/" + f['name'], 'w+')
                # C file header
