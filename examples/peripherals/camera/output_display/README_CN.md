...
```c
// 单次交换操作
Cycles per swap: ~2-4 CPU cycles (位运算)

// 320×240图像
Pixel count: 76,800
Total cycles: 76,800 × 3 = 230,400 cycles
Time @ 240MHz: ~0.96ms
```
...
```c
// 方法1: DMA硬件交换 (如果支持)
dma_config.byte_swap = true;

// 方法2: 使用SIMD指令 (ARM NEON)
// 一次处理多个像素

// 方法3: 在DMA2D转换时直接输出正确字节序
dma2d_config.output_byte_order = DMA2D_BIG_ENDIAN;
```
...
| LCD控制器 | 接口类型 | 需要字节交换 | 说明 |
|-----------|---------|-------------|------|
| ST7789 | SPI | 是 | 期望大端序 |
| ILI9341 | SPI | 是 | 期望大端序 |
| ST7735 | SPI | 是 | 期望大端序 |
| ILI9488 | SPI/并口 | 视配置而定 | 可配置 |
| RGB接口LCD | RGB | 否 | 直接像素流 |
| MIPI DSI | MIPI | 否 | 协议层处理 |
...
### 5.3 刷新时序
```
相机帧就绪 → 数据转换 → 旋转处理 → 刷新显示 → 切换缓冲 → 等待下一帧
    |          ~1-5ms      ~1-2ms      ~5-20ms       |
    └──────────────── 单帧处理周期 ─────────────────┘
```
