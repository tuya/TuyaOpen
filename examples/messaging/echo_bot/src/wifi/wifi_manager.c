...
{
    uint32_t delay = ECHO_WIFI_RETRY_BASE_MS;
    while (retry_count > 0 && delay < ECHO_WIFI_RETRY_MAX_MS) {
        if (delay > (ECHO_WIFI_RETRY_MAX_MS / 2)) {
...
static void schedule_wifi_retry(const char *reason)
{
    ...
    if (s_retry_count >= ECHO_WIFI_MAX_RETRY) {
        s_retry_pending = false;
        ECHO_LOGW(TAG, "wifi retry exhausted (%u), reason=%s", (unsigned)ECHO_WIFI_MAX_RETRY,
    ...
    ECHO_LOGW(TAG, "schedule wifi retry %u/%u in %u ms, reason=%s", (unsigned)(s_retry_count + 1),
              (unsigned)ECHO_WIFI_MAX_RETRY, (unsigned)delay_ms, reason ? reason : "unknown");
}
...
static void wifi_event_callback(WF_EVENT_E event, void *arg)
{
    ...
    case WFE_CONNECTED:
        ...
        reset_retry_state();
        update_ip_from_wifi();
        ECHO_LOGI(TAG, "wifi connected, ip=%s", s_ip_str);
        break;

    case WFE_CONNECT_FAILED:
        reset_link_state();
        ECHO_LOGW(TAG, "wifi connect failed");
        schedule_wifi_retry("connect_failed");
        break;

    case WFE_DISCONNECTED:
        reset_link_state();
        ECHO_LOGW(TAG, "wifi disconnected");
...
OPERATE_RET wifi_manager_init(void)
{
    ...
    if (rt != OPRT_OK) {
        ECHO_LOGW(TAG, "tal_wifi_init failed: %d", rt);
        return rt;
    }
...
OPERATE_RET wifi_manager_start(void)
{
    ...
    (void)echo_kv_get_string(ECHO_NVS_WIFI, ECHO_NVS_KEY_SSID, ssid, sizeof(ssid));
    (void)echo_kv_get_string(ECHO_NVS_WIFI, ECHO_NVS_KEY_PASS, pass, sizeof(pass));

    if (ssid[0] == '\0' && ECHO_SECRET_WIFI_SSID[0] != '\0') {
        snprintf(ssid, sizeof(ssid), "%s", ECHO_SECRET_WIFI_SSID);
        snprintf(pass, sizeof(pass), "%s", ECHO_SECRET_WIFI_PASS);
    }
    ...
    if (rt != OPRT_OK) {
        ECHO_LOGW(TAG, "tal_wifi_set_work_mode station failed: %d", rt);
        return rt;
    }

    ECHO_LOGI(TAG, "connecting wifi ssid=%s", s_target_ssid);
    rt = tal_wifi_station_connect((int8_t *)ssid, (int8_t *)pass);
    if (rt != OPRT_OK) {
        ECHO_LOGW(TAG, "tal_wifi_station_connect failed: %d", rt);
...
OPERATE_RET wifi_manager_wait_connected(uint32_t timeout_ms)
{
    ...
    while (1) {
        ...
        if (s_retry_pending && now >= s_next_retry_ms) {
            ...
            if (s_retry_count < ECHO_WIFI_MAX_RETRY) {
                s_retry_count++;
                ECHO_LOGI(TAG, "retrying wifi connect attempt %u/%u ssid=%s", (unsigned)s_retry_count,
                          (unsigned)ECHO_WIFI_MAX_RETRY, s_target_ssid);
                OPERATE_RET rt = tal_wifi_station_connect((int8_t *)s_target_ssid, (int8_t *)s_target_pass);
                if (rt != OPRT_OK) {
                    ECHO_LOGW(TAG, "retry connect call failed: %d", rt);
...
OPERATE_RET wifi_manager_set_credentials(const char *ssid, const char *password)
{
    ...
    OPERATE_RET rt = echo_kv_set_string(ECHO_NVS_WIFI, ECHO_NVS_KEY_SSID, ssid);
    if (rt != OPRT_OK) {
        return rt;
    }

    rt = echo_kv_set_string(ECHO_NVS_WIFI, ECHO_NVS_KEY_PASS, password);
    if (rt != OPRT_OK) {
        return rt;
    }
...
OPERATE_RET wifi_manager_scan_and_print(void)
{
    OPERATE_RET rt = wifi_manager_init();
    if (rt != OPRT_OK) {
        ECHO_LOGW(TAG, "wifi scan init failed: %d", rt);
        return rt;
    }
    ...
    if (rt != OPRT_OK) {
        ECHO_LOGW(TAG, "wifi scan get mode failed: %d", rt);
        return rt;
    }
    if (mode != WWM_STATION && mode != WWM_STATIONAP) {
        rt = tal_wifi_set_work_mode(WWM_STATION);
        if (rt != OPRT_OK) {
            ECHO_LOGW(TAG, "wifi scan set station mode failed: %d", rt);
            return rt;
        }
        tal_system_sleep(200);
    }
    ...
    if (rt != OPRT_OK) {
        ECHO_LOGW(TAG, "wifi scan failed: %d", rt);
        return rt;
    }

    if (ap_num == 0 || !ap_list) {
        ECHO_LOGW(TAG, "wifi scan found 0 ap(s)");
        return OPRT_NOT_FOUND;
    }

    ECHO_LOGI(TAG, "wifi scan found %u ap(s)", (unsigned)ap_num);
    ...
    for (uint32_t i = 0; i < ap_num; i++) {
        ...
        ECHO_LOGI(TAG, "ap[%u] ssid=%s ch=%u rssi=%d sec=%u bssid=%s", (unsigned)i, ssid[0] ? ssid : "<hidden>",
...
OPERATE_RET wifi_manager_init(void)
{
    ECHO_LOGW(TAG, "wifi disabled (ENABLE_WIFI!=1)");
...
OPERATE_RET wifi_manager_set_credentials(const char *ssid, const char *password)
{
    ...
    return echo_kv_set_string(ECHO_NVS_WIFI, ECHO_NVS_KEY_PASS, password);
}

OPERATE_RET wifi_manager_scan_and_print(void)
{
    ECHO_LOGW(TAG, "wifi scan disabled (ENABLE_WIFI!=1)");
    return OPRT_NOT_SUPPORTED;
}
