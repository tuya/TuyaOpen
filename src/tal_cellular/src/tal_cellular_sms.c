...
{
    if(tkl_cellular_sms_desc_get()->send == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    return tkl_cellular_sms_desc_get()->send(simId, smsMsg);
}
...
{
    if(tkl_cellular_sms_desc_get()->recv_cb_register == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    return tkl_cellular_sms_desc_get()->recv_cb_register(callback);
}
...
{
    if(tkl_cellular_sms_desc_get()->sms_mute == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    return tkl_cellular_sms_desc_get()->sms_mute(mute);
}
...
{
    if(tkl_cellular_sms_desc_get()->convert_str == NULL) {
        return NULL;
    }
    return tkl_cellular_sms_desc_get()->convert_str(from, from_size, from_chset, to_chset, to_size);
}
...
{
    if(tkl_cellular_sms_desc_get()->set_charactor == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    return tkl_cellular_sms_desc_get()->set_charactor(chset);
}
