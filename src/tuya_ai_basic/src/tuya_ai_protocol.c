...
static void __ai_atop_cfg_free(void)
{
    ...
    if (ai_basic_proto->config.username) {
        tal_free(ai_basic_proto->config.username);
    }
    if (ai_basic_proto->config.credential) {
        tal_free(ai_basic_proto->config.credential);
    }
    if (ai_basic_proto->config.client_id) {
        tal_free(ai_basic_proto->config.client_id);
    }
    if (ai_basic_proto->config.derived_algorithm) {
        tal_free(ai_basic_proto->config.derived_algorithm);
    }
    if (ai_basic_proto->config.derived_iv) {
        tal_free(ai_basic_proto->config.derived_iv);
    }
    if (ai_basic_proto->config.hosts) {
        for (idx = 0; idx < ai_basic_proto->config.host_num; idx++) {
            tal_free(ai_basic_proto->config.hosts[idx]);
        }
        tal_free(ai_basic_proto->config.hosts);
    }
...
static void __ai_basic_proto_deinit(void)
{
    if (ai_basic_proto) {
        ...
        if (ai_basic_proto->connection_id) {
            tal_free(ai_basic_proto->connection_id);
            ai_basic_proto->connection_id = NULL;
        }
        tal_free(ai_basic_proto);
        ai_basic_proto = NULL;
    }
    return;
}
...
static OPERATE_RET __ai_basic_proto_init(void)
{
    ...
    ai_basic_proto = tal_malloc(sizeof(AI_BASIC_PROTO_T));
...
OPERATE_RET tuya_ai_basic_atop_req(void)
{
    ...
    ai_basic_proto->config.hosts = tal_malloc(ai_basic_proto->config.host_num * sizeof(char *));
                                     ...
                                     AI_FRAG_FLAG frag, uint32_t origin_len)
{
    ...
    char *buf = tal_malloc(packet_len);
...
static OPERATE_RET __ai_packet_write(AI_SEND_PACKET_T *info, AI_FRAG_FLAG frag, uint32_t origin_len)
{
    ...
    char *send_pkt_buf = tal_malloc(uncrypt_len);
...
static void tuya_ai_free_attribute(AI_ATTRIBUTE_T *attr)
{
    ...
    case ATTR_PT_BYTES:
        if (attr->value.bytes) {
            tal_free(attr->value.bytes);
        }
        break;
    case ATTR_PT_STR:
        if (attr->value.str) {
            tal_free(attr->value.str);
        }
        break;
    default:
        break;
...
{
    AI_ATTRIBUTE_T *attr = (AI_ATTRIBUTE_T *)tal_malloc(sizeof(AI_ATTRIBUTE_T));
    if (!attr) {
    ...
    case ATTR_PT_U64:
        ...
        break;
    case ATTR_PT_BYTES:
        attr->value.bytes = tal_malloc(len);
...
OPERATE_RET tuya_ai_basic_pkt_read(char **out, uint32_t *out_len)
{
    ...
    decrypt_buf = tal_malloc(packet_len + head_len + AI_ADD_PKT_LEN);
    ...
    if (current_frag_flag == AI_PACKET_FRAG_START) {
        ...
        ai_basic_proto->frag_mng.data = tal_malloc(frag_total_len);
...
EXIT:
    ...
    if (ai_basic_proto->frag_mng.data) {
        tal_free(ai_basic_proto->frag_mng.data);
    }
    memset(&ai_basic_proto->frag_mng, 0, sizeof(AI_BASIC_FRAG_MNG_T));
    return recv_len;
}
 ...
 * @return OPERATE_RET OPRT_OK on success, error code otherwise
 *
 * @note Caller must free the output attribute array using tal_free()
 */
OPERATE_RET tuya_parse_user_attrs(char *in, uint32_t attr_len, AI_ATTRIBUTE_T **attr_out, uint32_t *attr_num)
{
    ...
    AI_ATTRIBUTE_T *attr = tal_malloc(num * sizeof(AI_ATTRIBUTE_T));
    if (!attr) {
        PR_ERR("malloc attr failed");
        return OPRT_MALLOC_FAILED;
    }
...
OPERATE_RET __ai_parse_auth_resp(char *de_buf, uint32_t attr_len)
{
    ...
    for (idx = 0; idx < attr_num; idx++) {
        if (attr[idx].type == AI_ATTR_CONNECT_STATUS_CODE) {
            ...
            ai_basic_proto->connection_id = tal_malloc(attr[idx].length + 1);
 ...
 *          - Strings
 *
 * @warning Caller must free the output buffer using tal_free()
 */
OPERATE_RET tuya_pack_user_attrs(AI_ATTRIBUTE_T *attr, uint32_t attr_num, uint8_t **out, uint32_t *out_len)
{
    ...
    char *usr_buf = tal_malloc(attr_len);
