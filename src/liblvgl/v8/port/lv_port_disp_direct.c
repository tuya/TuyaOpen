 ...
 * @brief Direct (zero-copy) flush for LVGL v8: render into the driver's vram fbs
 *
 * Compiled only when ENABLE_LVGL_DIRECT_FLUSH == 1. The two fbs come from
 * tdl_disp_get_vram(); LVGL renders into them with direct_mode + full_refresh
 * and the flush only commits the scanned buffer - the tdd waits for the scan
 * switch (anti-tearing) and no copy happens anywhere.
 */
...
OPERATE_RET lv_port_disp_set_buffers(LV_DISP_NODE_T *node)
{
     ...
     * full_refresh keeps px_map at the buffer base and one flush per frame. */
    lv_disp_draw_buf_init(&node->draw_buf_dsc,
                          node->direct_fb[0]->frame,
                          node->direct_fb[1]->frame,
                          node->dev_info.width * node->dev_info.height);

    node->lv_disp_drv.direct_mode = 1;
    node->lv_disp_drv.full_refresh = 1;
                           ...
                           const lv_area_t *area, uint8_t *color_ptr)
{
    ...
    if (lv_disp_flush_is_last(disp_drv)) {
        /* px_map is the buffer base with full_refresh. The first LVGL frame may
