...
static OPERATE_RET __alsa_setup_capture(TDD_AUDIO_ALSA_HANDLE_T *hdl)
{
    ...
    err = snd_pcm_open(&hdl->capture_handle, hdl->cfg.capture_device, SND_PCM_STREAM_CAPTURE, 0);
    if (err < 0) {
        PR_ERR("Cannot open capture device %s: %s", hdl->cfg.capture_device, snd_strerror(err));
...
static OPERATE_RET __alsa_setup_playback(TDD_AUDIO_ALSA_HANDLE_T *hdl)
{
    ...
    err = snd_pcm_open(&hdl->playback_handle, hdl->cfg.playback_device, SND_PCM_STREAM_PLAYBACK, 0);
    if (err < 0) {
        PR_ERR("Cannot open playback device %s: %s", hdl->cfg.playback_device, snd_strerror(err));
        return OPRT_COM_ERROR;
    }
