...
#define SYMBOL_SIC_SWITCH_EN                                                                                           \
    0x10 /* When set to logic 1, the SVDD switch is enabled and the SVDD output delivers power to secure IC and        \
            internal pads (SIGIN, SIGOUT and P34). */
 ...
 */
typedef enum {
    NORMAL,    // In that case, there is no power saved but the PN53x reacts as fast as possible on the host controller
               // interface.
 ...
 */
typedef enum { PSM_NORMAL = 0x01, PSM_VIRTUAL_CARD = 0x02, PSM_WIRED_CARD = 0x03, PSM_DUAL_CARD = 0x04 } pn532_sam_mode;
...
struct pn53x_data {
    ...
    /** Register cache for REG_CIU_BIT_FRAMING, SYMBOL_TX_LAST_BITS: The last TX bits setting, we need to reset this if
...
int pn53x_decode_target_data(const uint8_t *pbtRawData, size_t szRawData, pn53x_type chip_type, nfc_modulation_type nmt,
...
int pn53x_target_send_bits(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTxBits,
...
int pn53x_InJumpForDEP(struct nfc_device *pnd, const nfc_dep_mode ndm, const nfc_baud_rate nbr,
                       const uint8_t *pbtPassiveInitiatorData, const uint8_t *pbtNFCID3i, const uint8_t *pbtGB,
                       const size_t szGB, nfc_target *pnt, const int timeout);
int pn53x_TgInitAsTarget(struct nfc_device *pnd, pn53x_target_mode ptm, const uint8_t *pbtMifareParams,
                         ...
                         const uint8_t *pbtGB, const size_t szGB, uint8_t *pbtRx, const size_t szRxLen,
...
int pn53x_get_supported_modulation(nfc_device *pnd, const nfc_mode mode,
