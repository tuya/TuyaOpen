...
// Platform compatibility layer (must be after unistd.h)
...
static size_t pn532_uart_scan(const nfc_context *context, nfc_connstring connstrings[], const size_t connstrings_len)
{
    ...
    while ((acPort = acPorts[iDevice++])) {
        ...
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG,
                "Trying to find PN532 device on serial port: %s at %d baud.", acPort, PN532_UART_DEFAULT_SPEED);
        ...
        if ((sp != INVALID_SERIAL_PORT) && (sp != CLAIMED_SERIAL_PORT)) {
            ...
            snprintf(connstring, sizeof(nfc_connstring), "%s:%s:%" PRIu32, PN532_UART_DRIVER_NAME, acPort,
            ...
            // SAMConfiguration command if needed to wakeup the chip and pn53x_SAMConfiguration check if the chip is a
            // PN532
...
static nfc_device *pn532_uart_open(const nfc_context *context, const nfc_connstring connstring)
{
    ...
    if (connstring_decode_level == 3) {
        ...
        if (sscanf(speed_s, "%10" PRIu32, &ndd.speed) != 1) {
            ...
            PR_ERR("[pn532_uart_open] Invalid speed string");
            free(ndd.port);
            free(speed_s);
            return NULL;
        }
    ...
    if (connstring_decode_level < 2) {
        PR_ERR("[pn532_uart_open] Failed to decode connstring (level < 2)");
        return NULL;
    }
    if (connstring_decode_level < 3) {
    ...
    if (sp == CLAIMED_SERIAL_PORT) {
        PR_ERR("[pn532_uart_open] Serial port already claimed: %s", ndd.port);
    ...
    if (!pnd) {
        perror("malloc");
        PR_ERR("[pn532_uart_open] Failed to allocate nfc_device");
        free(ndd.port);
        uart_close(sp);
        return NULL;
    }
    ...
    if (!pnd->driver_data) {
        perror("malloc");
        PR_ERR("[pn532_uart_open] Failed to allocate driver_data");
        uart_close(sp);
        nfc_device_free(pnd);
        return NULL;
    }
...
static int pn532_uart_send(nfc_device *pnd, const uint8_t *pbtData, const size_t szData, int timeout)
{
    ...
    case LOWVBAT: {
        ...
        // According to PN532 application note, C106 appendix: to go out Low Vbat mode and enter in normal mode we need
        // to send a SAMConfiguration command
...
static int pn532_uart_receive(nfc_device *pnd, uint8_t *pbtData, const size_t szDataLen, int timeout)
{
    ...
    if (len > szDataLen) {
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
                "Unable to receive data: buffer too small. (szDataLen: %" PRIuPTR ", len: %" PRIuPTR ")", szDataLen,
                len);
        pnd->last_error = NFC_EIO;
        goto error;
    }
