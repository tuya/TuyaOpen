...
// Tuya platform logging
#include "tal_log.h"
#include "tkl_system.h" // For tkl_system_sleep
...
const char *nfc_property_name[] = {"NP_TIMEOUT_COMMAND",       "NP_TIMEOUT_ATR",
                                   "NP_TIMEOUT_COM",           "NP_HANDLE_CRC",
                                   "NP_HANDLE_PARITY",         "NP_ACTIVATE_FIELD",
                                   "NP_ACTIVATE_CRYPTO1",      "NP_INFINITE_SELECT",
                                   "NP_ACCEPT_INVALID_FRAMES", "NP_ACCEPT_MULTIPLE_FRAMES",
                                   "NP_AUTO_ISO14443_4",       "NP_EASY_FRAMING",
...
#endif /* DRIVER_PN53X_USB_ENABLED */
#if defined(DRIVER_PCSC_ENABLED)
    PR_DEBUG("[nfc_drivers_init] + pcsc");
    nfc_register_driver(&pcsc_driver);
#endif /* DRIVER_ACR122_PCSC_ENABLED */
#if defined(DRIVER_ACR122_PCSC_ENABLED)
    PR_DEBUG("[nfc_drivers_init] + acr122_pcsc");
    nfc_register_driver(&acr122_pcsc_driver);
#endif /* DRIVER_ACR122_PCSC_ENABLED */
#if defined(DRIVER_ACR122_USB_ENABLED)
    PR_DEBUG("[nfc_drivers_init] + acr122_usb");
    nfc_register_driver(&acr122_usb_driver);
#endif /* DRIVER_ACR122_USB_ENABLED */
#if defined(DRIVER_ACR122S_ENABLED)
    PR_DEBUG("[nfc_drivers_init] + acr122s");
...
#endif /* DRIVER_PN532_UART_ENABLED */
#if defined(DRIVER_PN532_SPI_ENABLED)
    PR_DEBUG("[nfc_drivers_init] + pn532_spi");
    nfc_register_driver(&pn532_spi_driver);
#endif /* DRIVER_PN532_SPI_ENABLED */
#if defined(DRIVER_PN532_I2C_ENABLED)
    PR_DEBUG("[nfc_drivers_init] + pn532_i2c");
    nfc_register_driver(&pn532_i2c_driver);
#endif /* DRIVER_PN532_I2C_ENABLED */
#if defined(DRIVER_ARYGON_ENABLED)
    PR_DEBUG("[nfc_drivers_init] + arygon");
 ...
 *
 * If \e connstring is set, this function will try to claim the right device using information provided by \e
 * connstring.
 *
 ...
 *
 * @note Depending on the desired operation mode, the device needs to be configured by using nfc_initiator_init() or
 * nfc_target_init(), optionally followed by manual tuning of the parameters if the default parameters are not suiting
 * your goals.
 */
nfc_device *nfc_open(nfc_context *context, const nfc_connstring connstring)
{
    ...
    nfc_connstring ncs;
    if (connstring == NULL) {
        PR_DEBUG("[nfc_open] No connstring, listing devices...");
        if (!nfc_list_devices(context, &ncs, 1)) {
            PR_ERR("[nfc_open] No devices found");
            return NULL;
        }
    } else {
    ...
    if (!pndl) {
        PR_ERR("[nfc_open] ERROR: nfc_drivers is NULL! No drivers registered!");
        return NULL;
    }
    ...
    while (pndl) {
        ...
        if (0 != strncmp(ndr->name, ncs, strlen(ndr->name))) {
            PR_DEBUG("[nfc_open] Driver name doesn't match connstring");
            // Check if connstring driver is usb -> accept any driver *_usb
            if ((0 != strncmp("usb", ncs, strlen("usb"))) ||
                0 != strncmp("_usb", ndr->name + (strlen(ndr->name) - 4), 4)) {
                PR_DEBUG("[nfc_open] Not a USB match either, skipping");
                pndl = pndl->next;
                continue;
            }
        }
        ...
        if (pnd == NULL) {
            if (0 == strncmp("usb", ncs, strlen("usb"))) {
                ...
                PR_WARN("[nfc_open] USB driver failed, trying next");
                pndl = pndl->next;
                continue;
            }
            PR_ERR("[nfc_open] Driver->open() FAILED for \"%s\"", ncs);
        ...
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "\"%s\" (%s) has been claimed.", pnd->name,
                pnd->connstring);
        return pnd;
    }
...
size_t nfc_list_devices(nfc_context *context, nfc_connstring connstrings[], const size_t connstrings_len)
{
    ...
    if (context->allow_autoscan) {
        ...
        while (pndl) {
            const struct nfc_driver *ndr = pndl->driver;
            if ((ndr->scan_type == NOT_INTRUSIVE) ||
                ((context->allow_intrusive_scan) && (ndr->scan_type == INTRUSIVE))) {
                size_t _device_found =
                    ndr->scan(context, connstrings + (device_found), connstrings_len - (device_found));
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%ld device(s) found using %s driver",
...
{
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "set_property_int %s %s", nfc_property_name[property],
            value ? "True" : "False");
...
{
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "set_property_bool %s %s", nfc_property_name[property],
            bEnable ? "True" : "False");
 ...
 * - for an ISO/IEC 14443 type B modulation, pbbInitData contains Application Family Identifier (AFI) (see ISO/IEC
 14443-3) and optionally a second byte = 0x01 if you want to use probabilistic approach instead of timeslot approach;
 ...
 */
int nfc_initiator_select_passive_target(nfc_device *pnd, const nfc_modulation nm, const uint8_t *pbtInitData,
 ...
 * should be supplied.
 */
int nfc_initiator_list_passive_targets(nfc_device *pnd, const nfc_modulation nm, nfc_target ant[],
                                       const size_t szTargets)
{
    ...
    while (nfc_initiator_select_passive_target(pnd, nm, pbtInitData, szInitDataLen, &nt) > 0) {
        ...
        // ISO/IEC 14443 B' cards are polled at 100% probability so it's not possible to detect correctly two cards at
        // the same time
        if ((nm.nmt == NMT_FELICA) || (nm.nmt == NMT_JEWEL) || (nm.nmt == NMT_BARCODE) || (nm.nmt == NMT_ISO14443BI) ||
            (nm.nmt == NMT_ISO14443B2SR) || (nm.nmt == NMT_ISO14443B2CT)) {
            break;
        }
    }
    if (bInfiniteSelect) {
 ...
 */
int nfc_initiator_poll_target(nfc_device *pnd, const nfc_modulation *pnmModulations, const size_t szModulations,
 ...
 * @param pndiInitiator pointer \a nfc_dep_info struct that contains \e NFCID3 and \e General \e Bytes to set to the
 * initiator device (optionnal, can be \e NULL)
 ...
 */
int nfc_initiator_select_dep_target(nfc_device *pnd, const nfc_dep_mode ndm, const nfc_baud_rate nbr,
 ...
 * @note \a nfc_dep_info will be returned when the target was acquired successfully.
 */
int nfc_initiator_poll_dep_target(struct nfc_device *pnd, const nfc_dep_mode ndm, const nfc_baud_rate nbr,
                                  const nfc_dep_info *pndiInitiator, nfc_target *pnt, const int timeout)
{
 ...
 *
 * If \a NP_EASY_FRAMING option is disabled the frames will sent and received in raw mode: \e PN53x will not handle
 * input neither output data.
 *
 ...
 *
 * Tests show that on average this way of communicating is much faster than using the regular driver/middle-ware (often
 * supplied by manufacturers).
 *
 * @warning The configuration option \a NP_HANDLE_PARITY must be set to \c true (the default value).
 *
 * @note When used with MIFARE Classic, NFC_EMFCAUTHFAIL error is returned if authentication command failed. You need to
 * re-select the tag to operate with.
 *
 * If timeout equals to 0, the function blocks indefinitely (until an error is raised or function is completed)
 * If timeout equals to -1, the default timeout will be used
 */
 ...
 */
int nfc_initiator_transceive_bits(nfc_device *pnd, const uint8_t *pbtTx, const size_t szTxBits, const uint8_t *pbtTxPar,
                                  uint8_t *pbtRx, const size_t szRx, uint8_t *pbtRxPar)
{
    (void)szRx;
 ...
 * @warning The configuration option \a NP_HANDLE_PARITY must be set to \c true (the default value).
 */
int nfc_initiator_transceive_bytes_timed(nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx,
 ...
 * @warning The configuration option \a NP_HANDLE_PARITY must be set to \c true (the default value).
 */
int nfc_initiator_transceive_bits_timed(nfc_device *pnd, const uint8_t *pbtTx, const size_t szTxBits,
                                        const uint8_t *pbtTxPar, uint8_t *pbtRx, const size_t szRx, uint8_t *pbtRxPar,
                                        uint32_t *cycles)
{
    (void)szRx;
 ...
 * In target mode, the emulation is stoped (no target available from external initiator) and the device is set to low
 * power mode (if avaible).
 */
 ...
 * @param pnd \a nfc_device struct pointer that represent currently used device
 *
 * Some commands (ie. nfc_target_init()) are blocking functions and will return only in particular conditions (ie.
 * external initiator request). This function attempt to abort the current running command.
 *
 ...
 * @param timeout in milliseconds
 *
 * This function retrieves bytes frames (e.g. ADPU) sent by the \e initiator to the NFC device (configured as \e
 * target).
 *
 * If timeout equals to 0, the function blocks indefinitely (until an error is raised or function is completed)
 * If timeout equals to -1, the default timeout will be used
 ...
 *
 */
int nfc_device_get_supported_modulation(nfc_device *pnd, const nfc_mode mode,
 ...
 *
 */
int nfc_device_get_supported_baud_rate(nfc_device *pnd, const nfc_modulation_type nmt,
 ...
 *
 */
int nfc_device_get_supported_baud_rate_target_mode(nfc_device *pnd, const nfc_modulation_type nmt,
                                                   const nfc_baud_rate **const supported_br)
{
 ...
 * @return Upon successful return, this function returns the number of characters printed (excluding the null byte used
 * to end output to strings), otherwise returns libnfc's error code (negative value)
 * @param pnd \a nfc_device struct pointer that represent currently used device
