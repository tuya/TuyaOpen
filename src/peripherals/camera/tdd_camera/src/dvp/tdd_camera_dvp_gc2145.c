...
OPERATE_RET tdl_camera_dvp_gc2145_register(char *name, DVP_GC2145_CFG_T *cfg)
{
    ...
    memcpy(&sg_dvp_sensor.pwr, &cfg->pwr, sizeof(TUYA_CAMERA_IO_CTRL_T));
    memcpy(&sg_dvp_sensor.rst, &cfg->rst, sizeof(TUYA_CAMERA_IO_CTRL_T));
    memcpy(&sg_dvp_sensor.i2c, &cfg->i2c, sizeof(DVP_I2C_CFG_T));
