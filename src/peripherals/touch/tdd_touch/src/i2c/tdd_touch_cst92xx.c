...
************************macro define************************
***********************************************************/
#define TP_RESET_PIN TUYA_GPIO_NUM_43
...
#define CST92XX_MAX_TOUCH_POINTS (1)
...
static void __cst92xx_reset(void)
{
    tkl_gpio_write(TP_RESET_PIN, TUYA_GPIO_LEVEL_LOW);
    tal_system_sleep(5);
    tkl_gpio_write(TP_RESET_PIN, TUYA_GPIO_LEVEL_HIGH);
    tal_system_sleep(50);
}
...
static OPERATE_RET __cst92xx_read_config(TUYA_I2C_NUM_E port)
{
    ...
    TUYA_CALL_ERR_RETURN(tdd_touch_i2c_port_write(port, CST92XX_ADDR, REG_CST92XX_CMD_MODE, 2, cmd_mode, 2));
    tal_system_sleep(10);

    memset(read_buf, 0, sizeof(read_buf));
    TUYA_CALL_ERR_RETURN(tdd_touch_i2c_port_read(port, CST92XX_ADDR, REG_CST92XX_CHECKCODE, 2, read_buf, 4));
    PR_INFO("Checkcode: 0x%02X%02X%02X%02X", read_buf[0], read_buf[1], read_buf[2], read_buf[3]);

    memset(read_buf, 0, sizeof(read_buf));
    TUYA_CALL_ERR_RETURN(tdd_touch_i2c_port_read(port, CST92XX_ADDR, REG_CST92XX_RESOLUTION, 2, read_buf, 4));
    uint16_t res_x = (read_buf[1] << 8) | read_buf[0];
    uint16_t res_y = (read_buf[3] << 8) | read_buf[2];
    PR_INFO("Resolution: %d x %d", res_x, res_y);

    memset(read_buf, 0, sizeof(read_buf));
    TUYA_CALL_ERR_RETURN(tdd_touch_i2c_port_read(port, CST92XX_ADDR, REG_CST92XX_PROJECT_ID, 2, read_buf, 4));
...
static OPERATE_RET __tdd_i2c_cst92xx_open(TDD_TOUCH_DEV_HANDLE_T device)
{
    OPERATE_RET rt = OPRT_OK;
    ...
    TUYA_CALL_ERR_LOG(tkl_gpio_init(TP_RESET_PIN, &out_pin_cfg));
    __cst92xx_reset();
...
static OPERATE_RET __tdd_i2c_cst92xx_read(TDD_TOUCH_DEV_HANDLE_T device, uint8_t max_num, TDL_TOUCH_POS_T *point,
                                          uint8_t *point_num)
{
    OPERATE_RET rt = OPRT_OK;
    TDD_TOUCH_INFO_T *info = (TDD_TOUCH_INFO_T *)device;
    ...
    TUYA_CALL_ERR_RETURN(
        tdd_touch_i2c_port_read(info->i2c_cfg.port, CST92XX_ADDR, REG_CST92XX_DATA, 2, data, sizeof(data)));
    ...
    points = (points > CST92XX_MAX_TOUCH_POINTS) ? CST92XX_MAX_TOUCH_POINTS : points;
    if (points > max_num) {
        points = max_num;
    }
...
static OPERATE_RET __tdd_i2c_cst92xx_close(TDD_TOUCH_DEV_HANDLE_T device)
{
    OPERATE_RET rt = OPRT_OK;
