/**
 * @file tdd_touch_cst9217.c
 * @brief CST9217 series capacitive touch controller driver implementation
 *
 * This file implements the TDD (Tuya Device Driver) layer for the CST9217 series.
...
***********************function define**********************
***********************************************************/
static OPERATE_RET __tdd_i2c_cst9217_open(TDD_TOUCH_DEV_HANDLE_T device)
{
    ...
    //     tdd_touch_i2c_port_read(info->i2c_cfg.port, CST9217_ADDR, REG_CHIP_ID, 1, &chip_id, sizeof(chip_id)));
    // PR_DEBUG("Touch Chip id: 0x%08x\r\n", chip_id);

    // tmp = 0x01;
    // tdd_touch_i2c_port_write(info->i2c_cfg.port, CST9217_ADDR, REG_DIS_AUTOSLEEP, 1, &tmp, 1);

    // tmp = IRQ_EN_MOTION;
    // tdd_touch_i2c_port_write(info->i2c_cfg.port, CST9217_ADDR, REG_IRQ_CTL, 1, &tmp, 1);

    return OPRT_OK;
}

static OPERATE_RET __tdd_i2c_cst9217_read(TDD_TOUCH_DEV_HANDLE_T device, uint8_t max_num, TDL_TOUCH_POS_T *point,
                                          uint8_t *point_num)
{
    OPERATE_RET rt = OPRT_OK;
    (void)rt;
    ...
    *point_num = 0;
    ...
    // TUYA_CALL_ERR_RETURN(tdd_touch_i2c_port_read(info->i2c_cfg.port, CST9217_ADDR, REG_STATUS, 1, buf, sizeof(buf)));
    // /* get point number */
...
static OPERATE_RET __tdd_i2c_cst9217_close(TDD_TOUCH_DEV_HANDLE_T device)
{
    OPERATE_RET rt = OPRT_OK;
    TDD_TOUCH_INFO_T *info = (TDD_TOUCH_INFO_T *)device;
...
OPERATE_RET tdd_touch_i2c_cst9217_register(char *name, TDD_TOUCH_I2C_CFG_T *cfg)
{
    ...
    infs.open = __tdd_i2c_cst9217_open;
    infs.read = __tdd_i2c_cst9217_read;
    infs.close = __tdd_i2c_cst9217_close;
