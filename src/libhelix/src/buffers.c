...
static void *(*helix_malloc)(size_t size) = tkl_system_psram_malloc;
static void (*helix_free)(void *buff) = tkl_system_psram_free;
#else 
static void *(*helix_malloc)(size_t size) = tal_malloc;
static void (*helix_free)(void *buff) = tal_free;
#endif

void MP3SetBufferFunc(void *(*malloc)(size_t size), void (*free)(void *buff))
{
	helix_malloc = malloc;
	helix_free = free;
}
