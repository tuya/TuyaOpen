...
typedef struct {
    ...
    uint8_t *p_video_buf; ///< video data buf
    uint32_t buf_len;     ///< video buf len
...
typedef struct {
    ...
    uint8_t *p_audio_buf;               ///< audio data buffer
    uint32_t buf_len;                   ///< audio buf len
...
typedef struct {
    ...
    int max_pic_len;          ///< max picture size, in KB
    int decoder_cnt;          ///< other decoder cnt
 ...
 */
typedef struct {
    int need_memory; ///< memory resource need to alloc
 ...
 */
typedef void (*MEDIA_RECV_AUDIO_CB)(int device, int channel, const MEDIA_AUDIO_FRAME_T *media_audio_frame);
 ...
 */
typedef void (*MEDIA_RECV_VIDEO_CB)(int device, int channel, const MEDIA_VIDEO_FRAME_T *media_video_frame);
 ...
 */
typedef void (*MEDIA_RECV_FILE_CB)(int device, int channel, const MEDIA_FILE_DATA_T *media_file_data);
 ...
 */
typedef void (*MEDIA_GET_SNAPSHOT_CB)(int device, int channel, char *pic_buf, int *pic_len);

/** @brief callback when media info change
 * @param[in]  info new media info
 */
typedef void (*MEDIA_INFO_CB)(int device, int channel, const DEVICE_MEDIA_INFO_T info);
typedef void (*MEDIA_CLOSE_CB)(int device, int channel);
...
typedef struct {
    ...
    int available_media_memory;           ///< max memory size of P2P/webrtc(MB). 0 means default, 5MB
...
typedef struct {
    ...
    uint32_t seq_no;     ///< sequence number, start from 1, keep same within one frame
    uint8_t frag_status; ///< fragment flag:0-no fragment，1-first fragment，2-middle fragment，3-last fragment
    uint8_t frag_no;     ///< fragment number, start from 0
} FRAME_FRAGMENT_T;
...
typedef struct {
    ...
    (int device, int channel, IPC_STREAM_E stream, STREAM_SOURCE_OPEN_TYPE_E open_type);
     ...
     * - Others  fail
     */
    OPERATE_RET (*seek_frame)(MEDIA_STREAM_HANDLE handle, uint32_t frame_num, BOOL_T check_overlap);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_snapshot_get(int device, int channel, char **buf, int *buf_len);

/** @brief delete a snapshot
 * @param[in] buf the image buffer returned by snapshot_get
 */
void tuya_ipc_media_adapter_snapshot_delete(char *buf);
 ...
 */
int tuya_ipc_media_adapter_get_max_pic_len(int device, int channel);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_get_max_frame(int device, int channel, IPC_STREAM_E stream);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_get_media_info(int device, int channel, DEVICE_MEDIA_INFO_T *media_info);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_set_media_info(int device, int channel, DEVICE_MEDIA_INFO_T media_info);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_attach_media_info(int device, int channel, MEDIA_INFO_CB media_cb,
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_detach_media_info(int device, int channel, MEDIA_INFO_CB media_cb,
                                                     MEDIA_CLOSE_CB media_close_cb);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_speak_start(int device, int channel, MEDIA_USER_HANDLE user_handle);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_speak_stop(int device, int channel, MEDIA_USER_HANDLE user_handle);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_display_start(int device, int channel, MEDIA_USER_HANDLE user_handle);
 ...
 * - Others  fail
 */
OPERATE_RET tuya_ipc_media_adapter_display_stop(int device, int channel, MEDIA_USER_HANDLE user_handle);
 ...
 */
void tuya_ipc_media_adapter_audio_output(int device, int channel, MEDIA_AUDIO_FRAME_T *audio,
 ...
 * @param[in] user_handle handle returned by alloc_resource
 */
void tuya_ipc_media_adapter_video_output(int device, int channel, MEDIA_VIDEO_FRAME_T *video,
                                         MEDIA_USER_HANDLE user_handle);
 ...
 * @param[in] user_handle handle returned by alloc_resource
 */
void tuya_ipc_media_adapter_file_output(int device, int channel, MEDIA_FILE_DATA_T *file,
                                        MEDIA_USER_HANDLE user_handle);
 ...
 */
BOOL_T tuya_ipc_media_adapter_output_device_is_busy(int device, int channel);
