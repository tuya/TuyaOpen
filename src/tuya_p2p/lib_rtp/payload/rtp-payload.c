...
static int s_max_packet_size = /*1434*/ 1114; // from VLC
    ...
    if ((payload >= RTP_PAYLOAD_DYNAMIC || RTP_PAYLOAD_TUYA_H265 == payload) && encoding) {
        ...
        } else if (0 == strcasecmp(encoding, "MP1S")) // MPEG-1 Systems Streams video (RFC 2250)
        {
            ...
            codec->decoder = rtp_ts_decode();
                   ...
                   || 0 == strcasecmp(encoding, "G7221") || 0 == strcasecmp(encoding, "PCMU") ||
                   0 == strcasecmp(encoding, "PCMA") ||
                   0 == strcasecmp(encoding, "PCM")) // RFC5577 RTP Payload Format for ITU-T Recommendation G.722.1
        {
