 ...
 * pj_ioqueue_set_lock()
 */
PJ_DEF(pj_status_t) pj_ioqueue_set_lock(pj_ioqueue_t *ioqueue, pj_lock_t *lock, pj_bool_t auto_delete)
{
...
static pj_status_t ioqueue_init_key(pj_pool_t *pool, pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *key, pj_sock_t sock,
                                    pj_grp_lock_t *grp_lock, void *user_data, const pj_ioqueue_callback *cb)
{
    pj_status_t rc;
    int optlen;
    ...
    rc = pj_sock_getsockopt(sock, pj_SOL_SOCKET(), pj_SO_TYPE(), &key->fd_type, &optlen);
    if (rc != PJ_SUCCESS)
 ...
 */
PJ_DEF(pj_status_t) pj_ioqueue_set_user_data(pj_ioqueue_key_t *key, void *user_data, void **old_data)
{
 ...
 * framework.
 */
pj_bool_t ioqueue_dispatch_write_event(pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *h)
{
    pj_status_t rc;
    ...
    if (h->connecting) {
         ...
         */
        {
            ...
            int gs_rc = pj_sock_getsockopt(h->fd, SOL_SOCKET, SO_ERROR, &value, &vallen);
            if (gs_rc != 0) {
                /* Argh!! What to do now???
        ...
        if (key_has_pending_write(h)) {
            ...
            if (write_op->op == PJ_IOQUEUE_OP_SEND) {
                send_rc = pj_sock_send(h->fd, write_op->buf + write_op->written, &sent, write_op->flags);
                ...
                int retry = 2;
                while (--retry >= 0) {
                    send_rc = pj_sock_sendto(h->fd, write_op->buf + write_op->written, &sent, write_op->flags,
                                             &write_op->rmt_addr, write_op->rmt_addrlen);
#if defined(PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT) && PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT != 0
                    /* Special treatment for dead UDP sockets here, see ticket #1107 */
                    if (send_rc == PJ_STATUS_FROM_OS(EPIPE) && !IS_CLOSING(h) && h->fd_type == pj_SOCK_DGRAM()) {
                        PJ_PERROR(4, (THIS_FILE, send_rc, "Send error for socket %d, retrying", h->fd));
            ...
            if (send_rc != PJ_SUCCESS || write_op->written == (pj_ssize_t)write_op->size ||
...
pj_bool_t ioqueue_dispatch_read_event(pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *h)
{
    pj_status_t rc;
    ...
    if (!pj_list_empty(&h->accept_list)) {
        ...
        if (read_op->op == PJ_IOQUEUE_OP_RECV_FROM) {
            ...
            rc = pj_sock_recvfrom(h->fd, read_op->buf, &bytes_read, read_op->flags, read_op->rmt_addr,
                                  read_op->rmt_addrlen);
...
pj_bool_t ioqueue_dispatch_exception_event(pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *h)
{
    pj_bool_t has_lock;
    pj_status_t rc;
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_recv(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, void *buffer, pj_ssize_t *length, unsigned flags)
{
    struct read_operation *read_op;
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_recvfrom(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, void *buffer, pj_ssize_t *length,
                    unsigned flags, pj_sockaddr_t *addr, int *addrlen)
{
    struct read_operation *read_op;
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_send(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, const void *data, pj_ssize_t *length,
                unsigned flags)
{
    struct write_operation *write_op;
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_sendto(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, const void *data, pj_ssize_t *length,
                  pj_uint32_t flags, const pj_sockaddr_t *addr, int addrlen)
{
    struct write_operation *write_op;
    unsigned retry;
...
#if defined(PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT) && PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT != 0
                /* Special treatment for dead UDP sockets here, see ticket #1107 */
                if (status == PJ_STATUS_FROM_OS(EPIPE) && !IS_CLOSING(key) && key->fd_type == pj_SOCK_DGRAM()) {
                    if (!restart_retry) {
                        PJ_PERROR(4, (THIS_FILE, status, "Send error for socket %d, retrying", key->fd));
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_accept(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, pj_sock_t *new_sock, pj_sockaddr_t *local,
 ...
 */
PJ_DEF(pj_status_t) pj_ioqueue_connect(pj_ioqueue_key_t *key, const pj_sockaddr_t *addr, int addrlen)
{
    pj_status_t status;
    ...
    if (status == PJ_SUCCESS) {
        ...
        if (status == PJ_STATUS_FROM_OS(PJ_BLOCKING_CONNECT_ERROR_VAL)) {
            ...
            ioqueue_add_to_set2(key->ioqueue, key, WRITEABLE_EVENT | EXCEPTION_EVENT);
            pj_ioqueue_unlock_key(key);
            return PJ_EPENDING;
        } else {
            /* Error! */
 ...
 */
PJ_DEF(pj_bool_t) pj_ioqueue_is_pending(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_post_completion(pj_ioqueue_key_t *key, pj_ioqueue_op_key_t *op_key, pj_ssize_t bytes_status)
{
    struct generic_operation *op_rec;
    ...
    while (op_rec != (void *)&key->accept_list) {
        if (op_rec == (void *)op_key) {
            ...
            if (key->cb.on_accept_complete) {
                (*key->cb.on_accept_complete)(key, op_key, PJ_INVALID_SOCKET, (pj_status_t)bytes_status);
            }
            return PJ_SUCCESS;
        }
        op_rec = op_rec->next;
    ...
    if (key->connecting) {
        key->connecting = 0;
        ioqueue_remove_from_set2(key->ioqueue, key, WRITEABLE_EVENT | EXCEPTION_EVENT);
    }
...
PJ_DEF(pj_status_t) pj_ioqueue_clear_key(pj_ioqueue_key_t *key)
{
    ...
    ioqueue_remove_from_set2(key->ioqueue, key, READABLE_EVENT | WRITEABLE_EVENT | EXCEPTION_EVENT);
...
PJ_DEF(pj_status_t) pj_ioqueue_set_default_concurrency(pj_ioqueue_t *ioqueue, pj_bool_t allow)
{
...
PJ_DEF(pj_status_t) pj_ioqueue_set_concurrency(pj_ioqueue_key_t *key, pj_bool_t allow)
{
    PJ_ASSERT_RETURN(key, PJ_EINVAL);
