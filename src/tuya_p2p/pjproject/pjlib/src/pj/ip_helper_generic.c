...
#elif defined(PJ_HAS_IFADDRS_H) && PJ_HAS_IFADDRS_H != 0 && defined(PJ_HAS_NET_IF_H) && PJ_HAS_NET_IF_H != 0
...
static pj_status_t if_enum_by_af(int af, unsigned *p_cnt, pj_sockaddr ifs[])
{
    ...
    TRACE_((THIS_FILE, "Starting interface enum with getifaddrs() for af=%d", af));
    ...
    *p_cnt = 0;
    for (; it != NULL && *p_cnt < max; it = it->ifa_next) {
        ...
        if (ad->sa_family != af) {
            TRACE_((THIS_FILE, "  address %s ignored (af=%d)", get_addr(ad), ad->sa_family));
            continue; /* Skip when interface is down */
        }
         ...
         */
        if (af == pj_AF_INET() &&
            (pj_ntohl(((pj_sockaddr_in *)ad)->sin_addr.s_addr) >> 4) == 201326592) /* 0b1100000000000000000000000000
                                                                                      which is 192.0.0.0 >> 4 */
        {
            TRACE_((THIS_FILE, "  address %s ignored (192.0.0.0/29 class)", get_addr(ad), ad->sa_family));
            continue;
        }
         ...
         */
        if (af == pj_AF_INET() && (pj_ntohl(((pj_sockaddr_in *)ad)->sin_addr.s_addr) >> 24) == 0) {
            TRACE_((THIS_FILE, "  address %s ignored (0.0.0.0/8 class)", get_addr(ad), ad->sa_family));
            continue;
        }

        TRACE_((THIS_FILE, "  address %s (af=%d) added at index %d", get_addr(ad), ad->sa_family, *p_cnt));
...
static pj_status_t if_enum_by_af(int af, unsigned *p_cnt, pj_sockaddr ifs[])
{
    ...
    TRACE_((THIS_FILE, "Starting interface enum with SIOCGIFCONF for af=%d", af));
    ...
    *p_cnt = 0;
    for (i = 0; i < count; ++i) {
        ...
        if (ad->sa_family != af) {
            TRACE_((THIS_FILE, "  address %s (af=%d) ignored", get_addr(ad), (int)ad->sa_family));
            continue;
        }

        if (ioctl(sock, SIOCGIFFLAGS, &iff) != 0) {
            TRACE_((THIS_FILE, "  ioctl(SIOCGIFFLAGS) failed: %s", get_os_errmsg()));
    ...
    *p_cnt = 0;
    for (i = 0; if_list[i].if_index && *p_cnt < max_count; ++i) {
        ...
        if ((rc = ioctl(sock, SIOCGIFADDR, &ifreq)) != 0) {
            TRACE_((THIS_FILE, "  ioctl(SIOCGIFADDR) failed: %s", get_os_errmsg()));
        ...
        if (ad->sa_family != af) {
            TRACE_((THIS_FILE, "  address %s family %d ignored", get_addr(&ifreq.ifr_addr), ifreq.ifr_addr.sa_family));
 ...
 */
PJ_DEF(pj_status_t) pj_enum_ip_interface(int af, unsigned *p_cnt, pj_sockaddr ifs[])
{
    unsigned start;
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t) pj_enum_ip_route(unsigned *p_cnt, pj_ip_route_entry routes[])
{
    pj_sockaddr itf;
    pj_status_t status;
            ...
            if (ifaddrmsg_ptr->ifa_flags & IFA_F_DEPRECATED || ifaddrmsg_ptr->ifa_flags & IFA_F_TENTATIVE) {
                ...
                for (; RTA_OK(rtattr_ptr, ifaddrmsg_len); rtattr_ptr = RTA_NEXT(rtattr_ptr, ifaddrmsg_len)) {
                    ...
                    case IFA_ADDRESS:
                        ...
                        inet_ntop(ifaddrmsg_ptr->ifa_family, RTA_DATA(rtattr_ptr), deprecatedAddr,
                                  sizeof(deprecatedAddr));
                        pj_str_t pj_addr_str;
                        pj_cstr(&pj_addr_str, deprecatedAddr);
                        pj_sockaddr_init(pj_AF_INET6(), &addr[idx], &pj_addr_str, 0);
                        ++idx;
                    default:
                        break;
                    }
 ...
 * Enumerate the local IP interface currently active in the host.
 */
PJ_DEF(pj_status_t) pj_enum_ip_interface2(const pj_enum_ip_option *opt, unsigned *p_cnt, pj_sockaddr ifs[])
{
