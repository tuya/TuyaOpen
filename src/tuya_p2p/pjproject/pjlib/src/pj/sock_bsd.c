...
#if (defined(PJ_WIN32) && PJ_WIN32) || (defined(PJ_WIN64) && PJ_WIN64) || (defined(IPPROTO_IP))
...
PJ_DEF(pj_status_t) pj_inet_pton(int af, const pj_str_t *src, void *dst)
{
     ...
     */
    {
        ...
        rc = WSAStringToAddress(PJ_STRING_TO_NATIVE(tempaddr, wtempaddr, sizeof(wtempaddr)), af, NULL,
...

{
    ...
    *dst = '\0';
     ...
     */
    {
        ...
        rc = WSAAddressToString((LPSOCKADDR)&sock_addr, addr_len, NULL, wtempaddr, &addr_str_len);
        if (rc == 0) {
            pj_unicode_to_ansi(wtempaddr, wcslen(wtempaddr), dst, size);
        }
#else
        rc = WSAAddressToString((LPSOCKADDR)&sock_addr, addr_len, NULL, dst, &addr_str_len);
#endif
...
PJ_DEF(pj_status_t) pj_sock_socket(int af, int type, int proto, pj_sock_t *sock)
{
    ...
    PJ_ASSERT_RETURN((SOCKET)PJ_INVALID_SOCKET == INVALID_SOCKET, (*sock = PJ_INVALID_SOCKET, PJ_EINVAL));
...
#if PJ_SOCK_DISABLE_WSAECONNRESET && (!defined(PJ_WIN32_WINCE) || PJ_WIN32_WINCE == 0)
...
#define SIO_UDP_CONNRESET _WSAIOW(IOC_VENDOR, 12)
    ...
    if (type == PJ_SOCK_DGRAM) {
        ...
        rc = WSAIoctl(*sock, SIO_UDP_CONNRESET, &bNewBehavior, sizeof(bNewBehavior), NULL, 0, &dwBytesReturned, NULL,
                      NULL);
...
PJ_DEF(pj_status_t) pj_sock_socket(int af, int type, int proto, pj_sock_t *sock)
{
    ...
    /* Sanity checks. */
    PJ_ASSERT_RETURN(sock != NULL, PJ_EINVAL);
    PJ_ASSERT_RETURN(PJ_INVALID_SOCKET == -1, (*sock = PJ_INVALID_SOCKET, PJ_EINVAL));
    ...
    *sock = socket(af, type, proto);
    if (*sock == PJ_INVALID_SOCKET)
        ...
        if (type == pj_SOCK_STREAM()) {
            pj_sock_setsockopt(*sock, pj_SOL_SOCKET(), pj_SO_NOSIGPIPE(), &val, sizeof(val));
        }
#if defined(PJ_SOCK_HAS_IPV6_V6ONLY) && PJ_SOCK_HAS_IPV6_V6ONLY != 0
        if (af == PJ_AF_INET6) {
            pj_sock_setsockopt(*sock, PJ_SOL_IPV6, IPV6_V6ONLY, &val, sizeof(val));
        }
#endif
#if defined(PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT) && PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT != 0
        if (type == pj_SOCK_DGRAM()) {
            pj_sock_setsockopt(*sock, pj_SOL_SOCKET(), SO_NOSIGPIPE, &val, sizeof(val));
        }
#endif
        return PJ_SUCCESS;
    }
...
pj_sock_setsockopt(pj_sock_t sock, pj_uint16_t level, pj_uint16_t optname, const void *optval, int optlen)
{
    ...
    status = setsockopt(sock, level, ((optname & 0xff00) == 0xff00) ? (int)optname | 0xffff0000 : optname,
...
PJ_DEF(pj_status_t) pj_sock_setsockopt_params(pj_sock_t sockfd, const pj_sockopt_params *params)
{
    ...
    for (; i < params->cnt && i < PJ_MAX_SOCKOPT_PARAMS; ++i) {
        pj_status_t status =
            pj_sock_setsockopt(sockfd, (pj_uint16_t)params->options[i].level, (pj_uint16_t)params->options[i].optname,
                               params->options[i].optval, params->options[i].optlen);
        if (status != PJ_SUCCESS) {
            retval = status;
            PJ_PERROR(4, (THIS_FILE, status, "Warning: error applying sock opt %d", params->options[i].optname));
        }
    }
