 ...
 */
PJ_DEF(char *) pj_sockaddr_print(const pj_sockaddr_t *addr, char *buf, int size, unsigned flags)
{
    enum { WITH_PORT = 1, WITH_BRACKETS = 2 };
...
PJ_DEF(pj_status_t) pj_sockaddr_in_set_str_addr(pj_sockaddr_in *addr, const pj_str_t *str_addr)
{
    ...
    if (str_addr && str_addr->slen) {
        ...
        if (addr->sin_addr.s_addr == PJ_INADDR_NONE) {
            ...
            if (status == PJ_SUCCESS) {
                pj_memcpy(&addr->sin_addr, &ai.ai_addr.ipv4.sin_addr, sizeof(addr->sin_addr));
            } else {
                return status;
            }
        }
...
PJ_DEF(pj_status_t) pj_sockaddr_set_str_addr(int af, pj_sockaddr *addr, const pj_str_t *str_addr)
{
    pj_status_t status;
    ...
    if (str_addr && str_addr->slen) {
        ...
        if (status != PJ_SUCCESS) {
            ...
            if (status == PJ_SUCCESS) {
                pj_memcpy(&addr->ipv6.sin6_addr, &ai.ai_addr.ipv6.sin6_addr, sizeof(addr->ipv6.sin6_addr));
 ...
 */
PJ_DEF(pj_status_t) pj_sockaddr_in_init(pj_sockaddr_in *addr, const pj_str_t *str_addr, pj_uint16_t port)
{
 ...
 */
PJ_DEF(pj_status_t) pj_sockaddr_init(int af, pj_sockaddr *addr, const pj_str_t *cp, pj_uint16_t port)
{
    pj_status_t status;
 ...
 */
PJ_DEF(int) pj_sockaddr_cmp(const pj_sockaddr_t *addr1, const pj_sockaddr_t *addr2)
{
    ...
    /* Compare addresses */
    result = pj_memcmp(pj_sockaddr_get_addr(a1), pj_sockaddr_get_addr(a2), pj_sockaddr_get_addr_len(a1));
    if (result != 0)
        return result;
...
PJ_DEF(void *) pj_sockaddr_get_addr(const pj_sockaddr_t *addr)
{
    ...
    PJ_ASSERT_RETURN(a->addr.sa_family == PJ_AF_INET || a->addr.sa_family == PJ_AF_INET6, NULL);
...
PJ_DEF(pj_bool_t) pj_sockaddr_has_addr(const pj_sockaddr_t *addr)
{
    ...
    if (a->addr.sa_family != PJ_AF_INET && a->addr.sa_family != PJ_AF_INET6) {
        ...
        return pj_memcmp(a->ipv6.sin6_addr.s6_addr, zero, sizeof(pj_in6_addr)) != 0;
    } else
...
PJ_DEF(pj_uint16_t) pj_sockaddr_get_port(const pj_sockaddr_t *addr)
{
    ...
    PJ_ASSERT_RETURN(a->addr.sa_family == PJ_AF_INET || a->addr.sa_family == PJ_AF_INET6, (pj_uint16_t)0xFFFF);
...
{
    const pj_sockaddr *a = (const pj_sockaddr *)addr;
    PJ_ASSERT_RETURN(a->addr.sa_family == PJ_AF_INET || a->addr.sa_family == PJ_AF_INET6, 0);
 ...
 * Synthesize address.
 */
PJ_DEF(pj_status_t) pj_sockaddr_synthesize(int dst_af, pj_sockaddr_t *dst, const pj_sockaddr_t *src)
{
 ...
 */
PJ_DEF(pj_status_t) pj_sockaddr_set_port(pj_sockaddr *addr, pj_uint16_t hostport)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_sockaddr_parse2(int af, unsigned options, const pj_str_t *str, pj_str_t *p_hostpart, pj_uint16_t *p_port, int *raf)
{
    ...
    PJ_ASSERT_RETURN((af == PJ_AF_INET || af == PJ_AF_INET6 || af == PJ_AF_UNSPEC) && options == 0 && str != NULL,
                     PJ_EINVAL);
 ...
 * Parse address
 */
PJ_DEF(pj_status_t) pj_sockaddr_parse(int af, unsigned options, const pj_str_t *str, pj_sockaddr *addr)
{
    pj_str_t hostpart;
    pj_uint16_t port;
    pj_status_t status;
    ...
    PJ_ASSERT_RETURN(af == PJ_AF_UNSPEC || af == PJ_AF_INET || af == PJ_AF_INET6, PJ_EINVAL);
...
PJ_DEF(pj_status_t) pj_gethostip(int af, pj_sockaddr *addr)
{
    ...
    struct spec_ipv4_t {
        ...
        pj_uint32_t mask;
        int weight;
    } spec_ipv4[] = {/* 127.0.0.0/8, loopback addr will be used if there is no other
                      * addresses.
                      */
    ...
    struct spec_ipv6_t {
        ...
        pj_uint8_t mask[16];
        int weight;
    } spec_ipv6[] = {/* Loopback address, ::1/128 */
...
#if !defined(PJ_GETHOSTIP_DISABLE_LOCAL_RESOLUTION) || PJ_GETHOSTIP_DISABLE_LOCAL_RESOLUTION == 0
    /* Get hostname's IP address */
    {
        ...
        if (status == PJ_SUCCESS) {
            ...
            TRACE_((THIS_FILE, "hostname IP is %s", pj_sockaddr_print(&ai.ai_addr, strip, sizeof(strip), 3)));
        }
    }
#else
    PJ_UNUSED_ARG(ai);
    ...
    if (cand_cnt < PJ_ARRAY_SIZE(cand_addr)) {
        ...
        if (status == PJ_SUCCESS) {
            TRACE_((THIS_FILE, "default IP is %s", pj_sockaddr_print(addr, strip, sizeof(strip), 3)));
        ...
        if (status == PJ_SUCCESS && count) {
            ...
            for (i = 0; i < count; ++i) {
                ...
                for (j = 0; j < cand_cnt; ++j) {
                    if (pj_sockaddr_cmp(&cand_addr[start_if + i], &cand_addr[j]) == 0)
                        break;
                }
    ...
    for (i = 0; i < cand_cnt; ++i) {
        TRACE_((THIS_FILE, "Checking candidate IP %s, weight=%d",
                pj_sockaddr_print(&cand_addr[i], strip, sizeof(strip), 3), cand_weight[i]));
    ...
    if (selected_cand == -1) {
        ...
        TRACE_((THIS_FILE, "Loopback IP %s returned", pj_sockaddr_print(addr, strip, sizeof(strip), 3)));
    } else {
        pj_sockaddr_copy_addr(addr, &cand_addr[selected_cand]);
        TRACE_((THIS_FILE, "Candidate %s selected", pj_sockaddr_print(addr, strip, sizeof(strip), 3)));
    }
...
PJ_DEF(pj_status_t)
pj_getipinterface(int af, const pj_str_t *dst, pj_sockaddr *itf_addr, pj_bool_t allow_resolve, pj_sockaddr *p_dst_addr)
{
    ...
    pj_bzero(zero, sizeof(zero));
    if (pj_memcmp(pj_sockaddr_get_addr(itf_addr), zero, pj_sockaddr_get_addr_len(itf_addr)) == 0) {
        return PJ_ENOTFOUND;
    }
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_bind_random(pj_sock_t sockfd, const pj_sockaddr_t *addr, pj_uint16_t port_range, pj_uint16_t max_try)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_setsockopt_sobuf(pj_sock_t sockfd, pj_uint16_t optname, pj_bool_t auto_retry, unsigned *buf_size)
{
    pj_status_t status;
    ...
    status = pj_sock_getsockopt(sockfd, pj_SOL_SOCKET(), optname, &cur_size, &size_len);
    if (status != PJ_SUCCESS)
        return status;
    ...
    for (i = 0; i < (MAX_TRY - 1); ++i) {
        ...
        status = pj_sock_setsockopt(sockfd, pj_SOL_SOCKET(), optname, &try_size, sizeof(try_size));
        if (status == PJ_SUCCESS) {
            status = pj_sock_getsockopt(sockfd, pj_SOL_SOCKET(), optname, &cur_size, &size_len);
            if (status != PJ_SUCCESS) {
...
PJ_DEF(char *) pj_addr_str_print(const pj_str_t *host_str, int port, char *buf, int size, unsigned flag)
{
    enum { WITH_PORT = 1 };
    char *bquote, *equote;
    int af = pj_AF_UNSPEC();
    ...
    if (flag & WITH_PORT) {
        pj_ansi_snprintf(buf, size, "%s%.*s%s:%d", bquote, (int)host_str->slen, host_str->ptr, equote, port);
    } else {
        pj_ansi_snprintf(buf, size, "%s%.*s%s", bquote, (int)host_str->slen, host_str->ptr, equote);
    }
    return buf;
}
 ...
 */
static pj_status_t socketpair_imp(int family, int type, int protocol, pj_sock_t sv[2])
{
    pj_status_t status;
...
#if PJ_HAS_TCP
    PJ_ASSERT_RETURN(type == pj_SOCK_DGRAM() || type == pj_SOCK_STREAM(), PJ_EINVAL);
#else
    PJ_ASSERT_RETURN(type == pj_SOCK_DGRAM(), PJ_EINVAL);
#endif

    PJ_ASSERT_RETURN(family == pj_AF_INET() || family == pj_AF_INET6(), PJ_EINVAL);
...
PJ_DEF(pj_status_t) pj_sock_socketpair(int family, int type, int protocol, pj_sock_t sv[2])
{
    int status;
    int tmp_sv[2];
