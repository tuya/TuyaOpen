...
#if defined(PJ_HAS_SSL_SOCK) && PJ_HAS_SSL_SOCK != 0 && (PJ_SSL_SOCK_IMP == PJ_SSL_SOCK_IMP_APPLE)
 ...
 */
PJ_DEF(pj_status_t) pj_ioqueue_create(pj_pool_t *pool, pj_size_t max_fd, pj_ioqueue_t **p_ioqueue)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_create2(pj_pool_t *pool, pj_size_t max_fd, const pj_ioqueue_cfg *cfg, pj_ioqueue_t **p_ioqueue)
{
    ...
    PJ_ASSERT_RETURN(pool != NULL && p_ioqueue != NULL && max_fd > 0 && max_fd <= PJ_IOQUEUE_MAX_HANDLES, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_register_sock2(pj_pool_t *pool, pj_ioqueue_t *ioqueue, pj_sock_t sock, pj_grp_lock_t *grp_lock,
                          void *user_data, const pj_ioqueue_callback *cb, pj_ioqueue_key_t **p_key)
{
...
PJ_DEF(pj_status_t)
pj_ioqueue_register_sock(pj_pool_t *pool, pj_ioqueue_t *ioqueue, pj_sock_t sock, void *user_data,
                         const pj_ioqueue_callback *cb, pj_ioqueue_key_t **p_key)
{
 ...
 */
#if VALIDATE_FD_SET
static void validate_sets(const pj_ioqueue_t *ioqueue, const pj_fd_set_t *rfdset, const pj_fd_set_t *wfdset,
 ...
 */
static void ioqueue_remove_from_set(pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *key, enum ioqueue_event_type event_type)
{
    ioqueue_remove_from_set2(ioqueue, key, event_type);
}

static void ioqueue_remove_from_set2(pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *key, unsigned event_types)
{
    pj_lock_acquire(ioqueue->lock);
 ...
 * set for the specified event.
 */
static void ioqueue_add_to_set(pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *key, enum ioqueue_event_type event_type)
{
    ioqueue_add_to_set2(ioqueue, key, event_type);
}

static void ioqueue_add_to_set2(pj_ioqueue_t *ioqueue, pj_ioqueue_key_t *key, unsigned event_types)
{
    pj_lock_acquire(ioqueue->lock);
...
static pj_status_t replace_udp_sock(pj_ioqueue_key_t *h)
{
    enum flags { HAS_PEER_ADDR = 1, HAS_QOS = 2 };
    ...
    for (msec = 20; (msec < 1000 && status != PJ_SUCCESS); msec < 1000 ? msec = msec * 2 : 1000) {
        if (msec > 20) {
        ...
        status = pj_sock_socket(local_addr.addr.sa_family, PJ_SOCK_DGRAM, 0, &new_sock);
        if (status != PJ_SUCCESS) {
...
PJ_DEF(int) pj_ioqueue_poll(pj_ioqueue_t *ioqueue, const pj_time_val *timeout)
{
    ...
    } __except (GetExceptionCode() == STATUS_INVALID_HANDLE ? EXCEPTION_CONTINUE_EXECUTION
...
#if PJ_HAS_TCP
        if (key_has_pending_connect(h) && PJ_FD_ISSET(h->fd, &xfdset) && !IS_CLOSING(h) && event_cnt < MAX_EVENTS) {
#if PJ_IOQUEUE_HAS_SAFE_UNREG
            increment_counter(h);
#endif
            event[event_cnt].key = h;
    ...
    TRACE__((THIS_FILE, "     poll: count=%d events=%d processed=%d", count, event_cnt, processed_cnt));
