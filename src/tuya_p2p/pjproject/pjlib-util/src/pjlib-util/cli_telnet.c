 ...
 */
typedef enum out_parse_state { OP_NORMAL, OP_TYPE, OP_SHORTCUT, OP_CHOICE } out_parse_state;
...
/* Forward Declaration */
static pj_status_t telnet_sess_send2(cli_telnet_sess *sess, const unsigned char *str, int len);
    ...
    if (recv_buf->len + 1 >= PJ_CLI_MAX_CMDBUF) {
        ...
        if (*data == '\t' || *data == '?' || *data == '\r') {
            ...
            if (rlen > 0) {
                ...
                pj_memmove(&recv_buf->rbuf[cur_pos + 1], &recv_buf->rbuf[cur_pos], rlen + 1);
            }
...
static pj_bool_t recv_buf_backspace(telnet_recv_buf *recv_buf)
{
    if ((recv_buf->cur_pos == 0) || (recv_buf->len == 0)) {
        ...
        if (rlen) {
            ...
            pj_memmove(&recv_buf->rbuf[cur_pos - 1], &recv_buf->rbuf[cur_pos], rlen);
        }
        --recv_buf->cur_pos;
        --recv_buf->len;
        recv_buf->rbuf[recv_buf->len] = 0;
...
static pj_status_t insert_history(cli_telnet_sess *sess, char *cmd_val)
{
    ...
    if (!in_history) {
        if (pj_list_size(sess->history) < PJ_CLI_MAX_CMD_HISTORY) {
            ...
            data_history = (char *)pj_pool_calloc(sess->pool, sizeof(char), PJ_CLI_MAX_CMDBUF);
 ...
 * referenced - (RFC-854).
 */
static pj_bool_t send_telnet_cmd(cli_telnet_sess *sess, cli_telnet_command cmd, unsigned char option)
{
    unsigned char buf[3];
 ...
 * negotiation process.
 */
static pj_bool_t send_enable_option(cli_telnet_sess *sess, pj_bool_t is_local, unsigned char option)
{
 ...
 * negotiation process.
 */
static pj_bool_t receive_enable_option(cli_telnet_sess *sess, pj_bool_t is_local, unsigned char option)
{
 ...
 * negotiation process.
 */
static pj_bool_t receive_disable_option(cli_telnet_sess *sess, pj_bool_t is_local, unsigned char option)
{
    cli_telnet_sess_option *sess_opt;
    enum cli_telnet_option_states *state;
...
static void set_local_option(cli_telnet_sess *sess, unsigned char option, pj_bool_t enable)
{
    sess->telnet_option[option].local_is_enable = enable;
}

static void set_peer_option(cli_telnet_sess *sess, unsigned char option, pj_bool_t enable)
{
    sess->telnet_option[option].peer_is_enable = enable;
}

static pj_bool_t is_local_option_state_ena(cli_telnet_sess *sess, unsigned char option)
{
 ...
 */
static void send_err_arg(cli_telnet_sess *sess, const pj_cli_exec_info *info, const pj_str_t *msg,
...
static void send_inv_arg(cli_telnet_sess *sess, const pj_cli_exec_info *info, pj_bool_t with_return,
...
static void send_too_many_arg(cli_telnet_sess *sess, const pj_cli_exec_info *info, pj_bool_t with_return,
                              pj_bool_t with_last_cmd)
{
...
static void send_hint_arg(cli_telnet_sess *sess, pj_str_t *send_data, const pj_str_t *desc, pj_ssize_t cmd_len,
                          pj_ssize_t max_len)
{
    if ((desc) && (desc->slen > 0)) {
        int j;
 ...
 */
static void send_ambi_arg(cli_telnet_sess *sess, const pj_cli_exec_info *info, pj_bool_t with_return,
                          pj_bool_t with_last_cmd)
{
    unsigned i;
    pj_size_t len;
    ...
    for (i = 0; i < info->hint_cnt; ++i) {
        ...
        if ((parse_state == OP_TYPE) || (parse_state == OP_CHOICE) || ((i + 1) >= info->hint_cnt) ||
            (pj_strncmp(&hint[i].desc, &hint[i + 1].desc, hint[i].desc.slen))) {
            /* Add description info */
            send_hint_arg(sess, &send_data, &hint[i].desc, cmd_length, max_length);
...
static pj_status_t get_last_token(pj_str_t *cmd, pj_str_t *str)
{
    pj_scanner scanner;
    PJ_USE_EXCEPTION;
    pj_scan_init(&scanner, cmd->ptr, cmd->slen, PJ_SCAN_AUTOSKIP_WS, &on_syntax_error);
    PJ_TRY
    {
...
static pj_bool_t handle_tab(cli_telnet_sess *sess)
{
    ...
    pool = pj_pool_create(sess->pool->factory, "handle_tab", PJ_CLI_TELNET_POOL_SIZE, PJ_CLI_TELNET_POOL_INC, NULL);

    cmd_val = PJ_POOL_ZALLOC_T(pool, pj_cli_cmd_val);

    status = pj_cli_sess_parse(&sess->base, (char *)&sess->rcmd->rbuf, cmd_val, pool, &info);
        ...
        if (info.hint_cnt > 0) {
            ...
            if (get_last_token(&cmd, &last_token) == PJ_SUCCESS) {
                ...
                if (hint_info->slen >= last_token.slen) {
                    ...
                    pj_memmove(hint_info->ptr, &hint_info->ptr[last_token.slen], hint_info->slen);
                }
                send_comp_arg(sess, &info);

                pj_memcpy(&sess->rcmd->rbuf[len], info.hint[0].name.ptr, info.hint[0].name.slen);
...
static pj_bool_t handle_return(cli_telnet_sess *sess)
{
    ...
    pool = pj_pool_create(sess->pool->factory, "handle_return", PJ_CLI_TELNET_POOL_SIZE, PJ_CLI_TELNET_POOL_INC, NULL);

    status = pj_cli_sess_exec(&sess->base, (char *)&sess->rcmd->rbuf, pool, &info);
...
static pj_bool_t handle_up_down(cli_telnet_sess *sess, pj_bool_t is_up)
{
    ...
    if (history) {
        ...
        enum { MOVE_CURSOR_LEFT = 0x08, CLEAR_CHAR = 0x20 };
        send_data.ptr = str;
        send_data.slen = 0;
        ...
        if (sess->rcmd->len > (unsigned)history->slen) {
            ...
            /* Move cursor position to the beginning of line */
            pj_memset(&send_data.ptr[send_data.slen], MOVE_CURSOR_LEFT, buf_len);
            send_data.slen += buf_len;
        }
        /* Send data */
...
static pj_status_t telnet_sess_send(cli_telnet_sess *sess, const pj_str_t *str)
{
    ...
    if (sess->buf_len == 0)
        status = pj_activesock_send(sess->asock, &sess->op_key, str->ptr, &sz, 0);
    ...
    if (sess->buf_len > 0 || (status != PJ_SUCCESS && status != PJ_EPENDING)) {
        ...
        if (clen < sz) {
            pj_ansi_snprintf((char *)sess->buf + CLI_TELNET_BUF_SIZE, MAX_CUT_MSG_LEN, CUT_MSG);
            sess->buf_len = (unsigned)(CLI_TELNET_BUF_SIZE + pj_ansi_strlen((char *)sess->buf + CLI_TELNET_BUF_SIZE));
        } else
            sess->buf_len += clen;
 ...
 */
static pj_status_t telnet_sess_send_with_format(cli_telnet_sess *sess, const pj_str_t *str)
{
    pj_scanner scanner;
    pj_str_t out_str;
    ...
    pj_scan_init(&scanner, str->ptr, str->slen, PJ_SCAN_AUTOSKIP_WS, &on_syntax_error);
...
static void telnet_fe_write_log(pj_cli_front_end *fe, int level, const char *data, pj_size_t len)
{
...
static pj_bool_t telnet_sess_on_data_sent(pj_activesock_t *asock, pj_ioqueue_op_key_t *op_key, pj_ssize_t sent)
{
    cli_telnet_sess *sess = (cli_telnet_sess *)pj_activesock_get_user_data(asock);
...
static pj_bool_t telnet_sess_on_data_read(pj_activesock_t *asock, void *data, pj_size_t size, pj_status_t status,
                                          pj_size_t *remainder)
{
    cli_telnet_sess *sess = (cli_telnet_sess *)pj_activesock_get_user_data(asock);
...
static pj_bool_t telnet_fe_on_accept(pj_activesock_t *asock, pj_sock_t newsock, const pj_sockaddr_t *src_addr,
                                     int src_addr_len, pj_status_t status)
{
    ...
    if (status != PJ_SUCCESS && status != PJ_EPENDING) {
        ...
        if (status == PJ_ESOCKETSTOP) {
            ...
            if (sstatus != PJ_SUCCESS) {
                ...
                TRACE_((THIS_FILE, "Error restarting telnet (status=%d)", status));
            }
        }
    ...
    pool = pj_pool_create(fe->pool->factory, "telnet_sess", PJ_CLI_TELNET_POOL_SIZE, PJ_CLI_TELNET_POOL_INC, NULL);
    if (!pool) {
    ...
    sstatus = pj_mutex_create_recursive(pool, "mutex_telnet_sess", &sess->smutex);
    if (sstatus != PJ_SUCCESS)
        goto on_exit;

    sstatus =
        pj_activesock_create(pool, newsock, pj_SOCK_STREAM(), NULL, fe->cfg.ioqueue, &asock_cb, sess, &sess->asock);
    if (sstatus != PJ_SUCCESS) {
...
PJ_DEF(pj_status_t) pj_cli_telnet_create(pj_cli_t *cli, pj_cli_telnet_cfg *param, pj_cli_front_end **p_fe)
{
    cli_telnet_fe *fe;
    pj_pool_t *pool;
    pj_status_t status;
    ...
    pool =
        pj_pool_create(pj_cli_get_param(cli)->pf, "telnet_fe", PJ_CLI_TELNET_POOL_SIZE, PJ_CLI_TELNET_POOL_INC, NULL);
...
static pj_status_t telnet_start(cli_telnet_fe *fe)
{
    ...
    for (msec = MIN_WAIT_ON_TELNET_RESTART, restart_retry = 0; restart_retry < MAX_RETRY_ON_TELNET_RESTART;
         ++restart_retry, msec = (msec < MAX_WAIT_ON_TELNET_RESTART ? msec * 2 : MAX_WAIT_ON_TELNET_RESTART)) {
    ...
    if (status == PJ_SUCCESS) {
        ...
        if (fe->cfg.prompt_str.slen == 0) {
            ...
            char *prompt_data = pj_pool_alloc(fe->pool, pj_gethostname()->slen + 2);
    ...
    status = pj_activesock_create(fe->pool, sock, pj_SOCK_STREAM(), NULL, fe->cfg.ioqueue, &asock_cb, fe, &fe->asock);
    if (status != PJ_SUCCESS)
        goto on_exit;
    ...
    if (fe->own_ioqueue) {
        ...
        status = pj_thread_create(fe->pool, "worker_telnet_fe", &poll_worker_thread, fe, 0, 0, &fe->worker_thread);
        if (status != PJ_SUCCESS)
            goto on_exit;
    }
...
PJ_DEF(pj_status_t) pj_cli_telnet_get_info(pj_cli_front_end *fe, pj_cli_telnet_info *info)
{
    pj_sockaddr hostip;
    pj_status_t status;
    cli_telnet_fe *tfe = (cli_telnet_fe *)fe;

    PJ_ASSERT_RETURN(fe && (fe->type == PJ_CLI_TELNET_FRONT_END) && info, PJ_EINVAL);
