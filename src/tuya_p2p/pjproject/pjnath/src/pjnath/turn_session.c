...
static const char *state_names[] = {"Null",  "Resolving",    "Resolved",    "Allocating",
                                    "Ready", "Deallocating", "Deallocated", "Destroying"};

enum timer_id_t { TIMER_NONE, TIMER_KEEP_ALIVE, TIMER_DESTROY };
...
static pj_status_t stun_on_send_msg(pj_stun_session *sess, void *token, const void *pkt, pj_size_t pkt_size,
                                    const pj_sockaddr_t *dst_addr, unsigned addr_len);
static void stun_on_request_complete(pj_stun_session *sess, pj_status_t status, void *token, pj_stun_tx_data *tdata,
                                     const pj_stun_msg *response, const pj_sockaddr_t *src_addr, unsigned src_addr_len);
static pj_status_t stun_on_rx_indication(pj_stun_session *sess, const pj_uint8_t *pkt, unsigned pkt_len,
                                         const pj_stun_msg *msg, void *token, const pj_sockaddr_t *src_addr,
                                         unsigned src_addr_len);
static void dns_srv_resolver_cb(void *user_data, pj_status_t status, const pj_dns_srv_record *rec);
static struct ch_t *lookup_ch_by_addr(pj_turn_session *sess, const pj_sockaddr_t *addr, unsigned addr_len,
                                      pj_bool_t update, pj_bool_t bind_channel);
static struct ch_t *lookup_ch_by_chnum(pj_turn_session *sess, pj_uint16_t chnum);
static struct perm_t *lookup_perm(pj_turn_session *sess, const pj_sockaddr_t *addr, unsigned addr_len,
                                  pj_bool_t update);
 ...
 */
PJ_DEF(void) pj_turn_alloc_param_copy(pj_pool_t *pool, pj_turn_alloc_param *dst, const pj_turn_alloc_param *src)
{
    PJ_UNUSED_ARG(pool);
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_create(const pj_stun_config *cfg, const char *name, int af, pj_turn_tp_type conn_type,
                       pj_grp_lock_t *grp_lock, const pj_turn_session_cb *cb, unsigned options, void *user_data,
                       pj_turn_session **p_sess)
{
    ...
    pool = pj_pool_create(cfg->pf, name, PJNATH_POOL_LEN_TURN_SESS, PJNATH_POOL_INC_TURN_SESS, NULL);
    ...
    status = pj_stun_session_create(&sess->stun_cfg, sess->obj_name, &stun_cb, PJ_FALSE, sess->grp_lock, &sess->stun);
    if (status != PJ_SUCCESS) {
        do_destroy(sess);
        return status;
    }
...
static void do_destroy(pj_turn_session *sess)
{
    PJ_LOG(4, (sess->obj_name, "TURN session destroy request, ref_cnt=%d", pj_grp_lock_get_ref(sess->grp_lock)));
...
static void set_state(pj_turn_session *sess, enum pj_turn_state_t state)
{
    ...
    PJ_LOG(4, (sess->obj_name, "State changed %s --> %s", state_names[old_state], state_names[state]));
    sess->state = state;
...
static void sess_shutdown(pj_turn_session *sess, pj_status_t status)
{
    ...
    PJ_LOG(4, (sess->obj_name, "Request to shutdown in state %s, cause:%d", state_names[sess->state], status));
    ...
    if (can_destroy) {
        ...
        pj_timer_heap_cancel_if_active(sess->timer_heap, &sess->timer, TIMER_NONE);
        pj_timer_heap_schedule_w_grp_lock(sess->timer_heap, &sess->timer, &delay, TIMER_DESTROY, sess->grp_lock);
    }
}
 ...
 */
PJ_DEF(pj_status_t) pj_turn_session_destroy(pj_turn_session *sess, pj_status_t last_err)
{
    PJ_ASSERT_RETURN(sess, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t) pj_turn_session_get_info(pj_turn_session *sess, pj_turn_session_info *info)
{
    pj_time_val now;
    ...
    pj_memcpy(&info->mapped_addr, &sess->mapped_addr, sizeof(sess->mapped_addr));
 ...
 * Re-assign user data.
 */
PJ_DEF(pj_status_t) pj_turn_session_set_user_data(pj_turn_session *sess, void *user_data)
{
    sess->user_data = user_data;
    return PJ_SUCCESS;
}
 ...
 * Set software name
 */
PJ_DEF(pj_status_t) pj_turn_session_set_software_name(pj_turn_session *sess, const pj_str_t *sw)
{
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_set_server(pj_turn_session *sess, const pj_str_t *domain, int default_port, pj_dns_resolver *resolver)
{
    ...
    if (!is_ip_addr && resolver) {
        ...
        PJ_LOG(5, (sess->obj_name, "Resolving %.*s%.*s with DNS SRV", (int)res_name.slen, res_name.ptr,
        ...
        status = pj_dns_srv_resolve(domain, &res_name, default_port, sess->pool, resolver, opt, sess,
        ...
        PJ_LOG(5, (sess->obj_name, "Resolving %.*s with DNS A", (int)domain->slen, domain->ptr));
        set_state(sess, PJ_TURN_STATE_RESOLVING);
        ...
        sess->srv_addr_list = (pj_sockaddr *)pj_pool_calloc(sess->pool, cnt, sizeof(pj_sockaddr));
        for (i = 0; i < cnt; ++i) {
 ...
 */
PJ_DEF(pj_status_t) pj_turn_session_set_credential(pj_turn_session *sess, const pj_stun_auth_cred *cred)
{
 ...
 */
PJ_DEF(pj_status_t) pj_turn_session_alloc(pj_turn_session *sess, const pj_turn_alloc_param *param)
{
    ...
    PJ_ASSERT_RETURN(sess, PJ_EINVAL);
    PJ_ASSERT_RETURN(sess->state > PJ_TURN_STATE_NULL && sess->state <= PJ_TURN_STATE_RESOLVED, PJ_EINVALIDOP);
    PJ_ASSERT_RETURN(param->peer_conn_type == PJ_TURN_TP_UDP || param->peer_conn_type == PJ_TURN_TP_TCP, PJ_EINVAL);

    /* Verify address family in allocation param */
    if (param && param->af) {
        PJ_ASSERT_RETURN(param->af == pj_AF_INET() || param->af == pj_AF_INET6(), PJ_EINVAL);
    }
    ...
    if (sess->state < PJ_TURN_STATE_RESOLVED) {
        ...
        PJ_LOG(4, (sess->obj_name, "Pending ALLOCATE in state %s", state_names[sess->state]));
    ...
    status = pj_stun_session_create_req(sess->stun, PJ_STUN_ALLOCATE_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS) {
        pj_grp_lock_release(sess->grp_lock);
        return status;
    }
    ...
    pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_REQ_TRANSPORT,
                              PJ_STUN_SET_RT_PROTO(param->peer_conn_type));

    /* Include BANDWIDTH if requested */
    if (sess->alloc_param.bandwidth > 0) {
        pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_BANDWIDTH, sess->alloc_param.bandwidth);
    }

    /* Include LIFETIME if requested */
    if (sess->alloc_param.lifetime > 0) {
        pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_LIFETIME, sess->alloc_param.lifetime);
    }

    /* Include ADDRESS-FAMILY if requested */
    if (sess->alloc_param.af || sess->af == pj_AF_INET6()) {
        enum { IPV4_AF_TYPE = 0x01 << 24, IPV6_AF_TYPE = 0x02 << 24 };

        if (sess->alloc_param.af == pj_AF_INET6() || (sess->alloc_param.af == 0 && sess->af == pj_AF_INET6())) {
            pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_REQ_ADDR_TYPE, IPV6_AF_TYPE);
        } else if (sess->alloc_param.af == pj_AF_INET()) {
            /* For IPv4, only add the attribute when explicitly requested */
            pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_REQ_ADDR_TYPE, IPV4_AF_TYPE);
        }
    }
    ...
    status = pj_stun_session_send_msg(sess->stun, NULL, PJ_FALSE, retransmit, sess->srv_addr,
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_set_perm(pj_turn_session *sess, unsigned addr_cnt, const pj_sockaddr addr[], unsigned options)
{
    pj_stun_tx_data *tdata;
    ...
    status = pj_stun_session_create_req(sess->stun, PJ_STUN_CREATE_PERM_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS) {
        pj_grp_lock_release(sess->grp_lock);
        return status;
    }
    ...
    for (i = 0; i < addr_cnt; ++i) {
        ...
        perm = lookup_perm(sess, &addr[i], pj_sockaddr_get_len(&addr[i]), PJ_TRUE);
        ...
        if (perm->req_token != req_token) {
            ...
            status = pj_stun_msg_add_sockaddr_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_XOR_PEER_ADDR, PJ_TRUE,
    ...
    /* Send the request */
    status = pj_stun_session_send_msg(sess->stun, req_token, PJ_FALSE, (sess->conn_type == PJ_TURN_TP_UDP),
...
on_error:
    ...
    while (it) {
        struct perm_t *perm = (struct perm_t *)pj_hash_this(sess->perm_table, it);
...
static void send_refresh(pj_turn_session *sess, int lifetime)
{
    ...
    status = pj_stun_session_create_req(sess->stun, PJ_STUN_REFRESH_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS)
        goto on_error;

    /* Add LIFETIME */
    if (lifetime >= 0) {
        pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_LIFETIME, lifetime);
    }
    ...
    status = pj_stun_session_send_msg(sess->stun, NULL, PJ_FALSE, (sess->conn_type == PJ_TURN_TP_UDP), sess->srv_addr,
                                      pj_sockaddr_get_len(sess->srv_addr), tdata);
    if (status != PJ_SUCCESS)
        goto on_error;
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_sendto(pj_turn_session *sess, const pj_uint8_t *pkt, unsigned pkt_len, const pj_sockaddr_t *addr,
                       unsigned addr_len)
{
    struct ch_t *ch;
    struct perm_t *perm;
    ...
    if (perm == NULL) {
        ...
        PJ_LOG(4, (sess->obj_name, "sendto(): IP %s has no permission, requesting it first..",
    ...
    ch = lookup_ch_by_addr(sess, addr, pj_sockaddr_get_len(addr), PJ_FALSE, PJ_FALSE);
    if (ch && ch->num != PJ_TURN_INVALID_CHANNEL && ch->bound) {
        ...
        status =
            sess->cb.on_send_pkt(sess, sess->tx_pkt, total_len, sess->srv_addr, pj_sockaddr_get_len(sess->srv_addr));
        ...
        status = pj_stun_msg_init(&send_ind, PJ_STUN_SEND_INDICATION, PJ_STUN_MAGIC,
                                  (const pj_uint8_t *)sess->send_ind_tsx_id);
        if (status != PJ_SUCCESS)
            goto on_return;

        /* Add XOR-PEER-ADDRESS */
        pj_stun_sockaddr_attr_init(&peer_attr, PJ_STUN_ATTR_XOR_PEER_ADDR, PJ_TRUE, addr, addr_len);
        ...
        /* Encode the message */
        status = pj_stun_msg_encode(&send_ind, sess->tx_pkt, sizeof(sess->tx_pkt), 0, NULL, &send_ind_len);
        if (status != PJ_SUCCESS)
            goto on_return;

        /* Send the Send Indication */
        status = sess->cb.on_send_pkt(sess, sess->tx_pkt, (unsigned)send_ind_len, sess->srv_addr,
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_bind_channel(pj_turn_session *sess, const pj_sockaddr_t *peer_adr, unsigned addr_len)
{
    struct ch_t *ch;
    pj_stun_tx_data *tdata;
    pj_uint16_t ch_num;
    ...
    status = pj_stun_session_create_req(sess->stun, PJ_STUN_CHANNEL_BIND_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;

    /* Lookup if this peer has already been assigned a number */
    ch = lookup_ch_by_addr(sess, peer_adr, pj_sockaddr_get_len(peer_adr), PJ_TRUE, PJ_FALSE);
    pj_assert(ch);
    ...
    pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_CHANNEL_NUMBER, PJ_STUN_SET_CH_NB(ch_num));

    /* Add XOR-PEER-ADDRESS attribute */
    pj_stun_msg_add_sockaddr_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_XOR_PEER_ADDR, PJ_TRUE, peer_adr, addr_len);

    /* Send the request, associate peer data structure with tdata
     * for future reference when we receive the ChannelBind response.
     */
    status = pj_stun_session_send_msg(sess->stun, ch, PJ_FALSE, (sess->conn_type == PJ_TURN_TP_UDP), sess->srv_addr,
                                      pj_sockaddr_get_len(sess->srv_addr), tdata);
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_session_connection_bind(pj_turn_session *sess, pj_pool_t *pool, pj_uint32_t conn_id,
                                const pj_sockaddr_t *peer_addr, unsigned addr_len)
{
    ...
    PJ_ASSERT_RETURN(sess && pool && conn_id && peer_addr && addr_len, PJ_EINVAL);
    PJ_ASSERT_RETURN(sess->state == PJ_TURN_STATE_READY, PJ_EINVALIDOP);

    pj_grp_lock_acquire(sess->grp_lock);

    /* Create blank ConnectionBind request */
    status = pj_stun_session_create_req(sess->stun, PJ_STUN_CONNECTION_BIND_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;

    /* Add CONNECTION_ID attribute */
    pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_CONNECTION_ID, conn_id);
     ...
     */
    status = pj_stun_session_send_msg(sess->stun, conn_bind, PJ_FALSE, PJ_FALSE, peer_addr, addr_len, tdata);
 ...
 */
PJ_DEF(pj_status_t) pj_turn_session_connect(pj_turn_session *sess, const pj_sockaddr_t *peer_addr, unsigned addr_len)
{
    pj_stun_tx_data *tdata;
    pj_status_t status;
    ...
    status = pj_stun_session_create_req(sess->stun, PJ_STUN_CONNECT_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS)
        goto on_return;

    status = pj_stun_msg_add_sockaddr_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_XOR_PEER_ADDR, PJ_TRUE, peer_addr,
                                           addr_len);
    if (status != PJ_SUCCESS)
        goto on_return;
    status = pj_stun_session_send_msg(sess->stun, (void *)peer_addr, PJ_FALSE, PJ_FALSE, sess->srv_addr,
                                      pj_sockaddr_get_len(sess->srv_addr), tdata);
...
PJ_DEF(pj_status_t)
pj_turn_session_on_rx_pkt(pj_turn_session *sess, void *pkt, pj_size_t pkt_len, pj_size_t *parsed_len)
{
 ...
 */
PJ_DEF(pj_status_t) pj_turn_session_on_rx_pkt2(pj_turn_session *sess, pj_turn_session_on_rx_pkt_param *prm)
{
    pj_bool_t is_stun;
    pj_status_t status;
    ...
    if (is_stun) {
        ...
        unsigned options;
        const pj_sockaddr_t *src_addr = prm->src_addr ? prm->src_addr : sess->srv_addr;
        unsigned src_addr_len = prm->src_addr_len ? prm->src_addr_len : pj_sockaddr_get_len(sess->srv_addr);

        options = PJ_STUN_CHECK_PACKET | PJ_STUN_NO_FINGERPRINT_CHECK;
        if (is_datagram)
            options |= PJ_STUN_IS_DATAGRAM;
        status = pj_stun_session_on_rx_pkt(sess->stun, prm->pkt, prm->pkt_len, options, NULL, &prm->parsed_len,
        ...
        if (sess->cb.on_rx_data) {
            (*sess->cb.on_rx_data)(sess, ((pj_uint8_t *)prm->pkt) + sizeof(cd), cd.length, &ch->addr,
 ...
 */
static pj_status_t stun_on_send_msg(pj_stun_session *stun, void *token, const void *pkt, pj_size_t pkt_size,
                                    const pj_sockaddr_t *dst_addr, unsigned addr_len)
{
    pj_turn_session *sess;
    ...
    if (sess->cb.on_stun_send_pkt) {
        return (*sess->cb.on_stun_send_pkt)(sess, (const pj_uint8_t *)pkt, (unsigned)pkt_size, dst_addr, addr_len);
    } else {
        return (*sess->cb.on_send_pkt)(sess, (const pj_uint8_t *)pkt, (unsigned)pkt_size, dst_addr, addr_len);
    }
}
 ...
 */
static void on_session_fail(pj_turn_session *sess, enum pj_stun_method_e method, pj_status_t status,
                            const pj_str_t *reason)
{
    ...
    do {
         ...
         * session.
         */
        if (method == PJ_STUN_ALLOCATE_METHOD && sess->srv_addr == &sess->srv_addr_list[sess->srv_addr_cnt - 1]) {
 ...
 */
static void on_allocate_success(pj_turn_session *sess, enum pj_stun_method_e method, const pj_stun_msg *msg)
{
    ...
    lf_attr = (const pj_stun_lifetime_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_LIFETIME, 0);
    if (lf_attr == NULL) {
        on_session_fail(sess, method, PJNATH_EINSTUNMSG, pj_cstr(&s, "Error: Missing LIFETIME attribute"));
        return;
    }
    ...
    if (sess->lifetime < PJ_TURN_KEEP_ALIVE_SEC) {
        if (sess->lifetime <= 2) {
            on_session_fail(sess, method, PJ_ETOOSMALL, pj_cstr(&s, "Error: LIFETIME too small"));
            return;
        }
     ...
     * address family.
     */
    raddr_attr = (const pj_stun_xor_relayed_addr_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_XOR_RELAYED_ADDR, 0);
    ...
    if (raddr_attr && ((sess->alloc_param.af != 0 && raddr_attr->sockaddr.addr.sa_family != sess->alloc_param.af) ||
                       (sess->alloc_param.af == 0 && raddr_attr->sockaddr.addr.sa_family != sess->af))) {
        on_session_fail(sess, method, PJNATH_EINSTUNMSG,
    ...
    /* Get mapped address */
    mapped_attr = (const pj_stun_sockaddr_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_XOR_MAPPED_ADDR, 0);
    if (mapped_attr) {
        pj_memcpy(&sess->mapped_addr, &mapped_attr->sockaddr, sizeof(mapped_attr->sockaddr));
    }
    ...
    if (sess->state < PJ_TURN_STATE_DEALLOCATING) {
        ...
        pj_timer_heap_schedule_w_grp_lock(sess->timer_heap, &sess->timer, &timeout, TIMER_KEEP_ALIVE, sess->grp_lock);
 ...
 */
static void stun_on_request_complete(pj_stun_session *stun, pj_status_t status, void *token, pj_stun_tx_data *tdata,
                                     const pj_stun_msg *response, const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    pj_turn_session *sess;
    enum pj_stun_method_e method = (enum pj_stun_method_e)PJ_STUN_GET_METHOD(tdata->msg->hdr.type);
    ...
    if (method == PJ_STUN_ALLOCATE_METHOD) {
        ...
        if (status == PJ_SUCCESS && PJ_STUN_IS_SUCCESS_RESPONSE(response->hdr.type)) {
            ...
            if (status == PJ_SUCCESS) {
                ...
                err_attr = (const pj_stun_errcode_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_ERROR_CODE, 0);
                if (err_attr) {
        ...
        if (status == PJ_SUCCESS && PJ_STUN_IS_SUCCESS_RESPONSE(response->hdr.type)) {
            ...
            /* Update hash table */
            lookup_ch_by_addr(sess, &ch->addr, pj_sockaddr_get_len(&ch->addr), PJ_TRUE, PJ_TRUE);
            ...
            PJ_LOG(1, (sess->obj_name, "ChannelBind failed: %d/%.*s", err_code, (int)reason.slen, reason.ptr));

            if (err_code == PJ_STUN_SC_ALLOCATION_MISMATCH) {
                /* Allocation mismatch means allocation no longer exists */
                on_session_fail(sess, PJ_STUN_CHANNEL_BIND_METHOD, status, &reason);
                return;
            }
        }
        ...
        if (status == PJ_SUCCESS && PJ_STUN_IS_SUCCESS_RESPONSE(response->hdr.type)) {
            ...
            if (response) {
                ...
                eattr = (const pj_stun_errcode_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_ERROR_CODE, 0);
                if (eattr) {
            ...
            while (it) {
                ...
                if (perm->req_token == token) {
                    PJ_LOG(1, (sess->obj_name, "CreatePermission failed for IP %s: %d/%.*s",
                               pj_sockaddr_print(&perm->addr, ipstr, sizeof(ipstr), 2), err_code, (int)reason.slen,
                               reason.ptr));
            ...
            if (err_code == PJ_STUN_SC_ALLOCATION_MISMATCH) {
                /* Allocation mismatch means allocation no longer exists */
                on_session_fail(sess, PJ_STUN_CREATE_PERM_METHOD, status, &reason);
                return;
            }
        }
        ...
        if (status != PJ_SUCCESS || !PJ_STUN_IS_SUCCESS_RESPONSE(response->hdr.type)) {
            ...
            pj_perror(1, sess->obj_name, status, "ConnectionBind failed: %.*s", (int)reason.slen, reason.ptr);
        }

        /* Notify app */
        if (sess->cb.on_connection_bind_status) {
            (*sess->cb.on_connection_bind_status)(sess, status, conn_bind->id, &conn_bind->peer_addr,
        ...
        if (status != PJ_SUCCESS || !PJ_STUN_IS_SUCCESS_RESPONSE(response->hdr.type)) {
            ...
            pj_perror(1, sess->obj_name, status, "Connect failed: %.*s", (int)reason.slen, reason.ptr);
        } else {
            const pj_stun_uint_attr *conn_id_attr;
            conn_id_attr = (const pj_stun_uint_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_CONNECTION_ID, 0);
            if (conn_id_attr == NULL) {
                status = PJNATH_EINSTUNMSG;
                pj_perror(1, sess->obj_name, status, "Error: Missing CONNECTION-ID attribute");
            } else {
        ...
        if (sess->cb.on_connect_complete) {
            (*sess->cb.on_connect_complete)(sess, status, conn_id, peer_addr, pj_sockaddr_get_len(peer_addr));
        }
    } else {
        PJ_LOG(4, (sess->obj_name, "Unexpected STUN %s response", pj_stun_get_method_name(response->hdr.type)));
    }
}
 ...
 * message.
 */
static pj_status_t stun_on_rx_indication(pj_stun_session *stun, const pj_uint8_t *pkt, unsigned pkt_len,
                                         const pj_stun_msg *msg, void *token, const pj_sockaddr_t *src_addr,
                                         unsigned src_addr_len)
{
    pj_turn_session *sess;
    ...
    if (msg->hdr.type == PJ_STUN_CONNECTION_ATTEMPT_INDICATION) {
        ...
        connection_id_attr = (pj_stun_uint_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_CONNECTION_ID, 0);

        /* Get XOR-PEER-ADDRESS attribute */
        peer_attr = (pj_stun_xor_peer_addr_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_XOR_PEER_ADDR, 0);

        /* Must have both XOR-PEER-ADDRESS and CONNECTION-ID attributes */
        if (!peer_attr || !connection_id_attr) {
            PJ_LOG(4, (sess->obj_name, "Received ConnectionAttempt indication with missing "
                                       "attributes"));
        ...
        if (sess->cb.on_connection_attempt) {
            (*sess->cb.on_connection_attempt)(sess, connection_id_attr->value, &peer_attr->sockaddr,
    ...
    if (msg->hdr.type != PJ_STUN_DATA_INDICATION) {
        PJ_LOG(4, (sess->obj_name, "Unexpected STUN %s indication", pj_stun_get_method_name(msg->hdr.type)));
        return PJ_EINVALIDOP;
    }
    ...
    if (!peer_attr || !data_attr) {
        PJ_LOG(4, (sess->obj_name, "Received Data indication with missing attributes"));
        return PJ_EINVALIDOP;
    }

    /* Notify application */
    if (sess->cb.on_rx_data) {
        (*sess->cb.on_rx_data)(sess, data_attr->data, data_attr->length, &peer_attr->sockaddr,
                               pj_sockaddr_get_len(&peer_attr->sockaddr));
    }
...
static void dns_srv_resolver_cb(void *user_data, pj_status_t status, const pj_dns_srv_record *rec)
{
    ...
    sess->srv_addr_list = (pj_sockaddr *)pj_pool_calloc(sess->pool, tot_cnt, sizeof(pj_sockaddr));

    /* Copy results to server entries */
    for (i = 0, cnt = 0; i < rec->count && cnt < PJ_TURN_MAX_DNS_SRV_CNT; ++i) {
        unsigned j;

        for (j = 0; j < rec->entry[i].server.addr_count && cnt < PJ_TURN_MAX_DNS_SRV_CNT; ++j) {
                                      ...
                                      pj_bool_t update, pj_bool_t bind_channel)
{
    ...
    if (ch && update) {
        ...
        if (bind_channel) {
            ...
            if (pj_hash_get(sess->ch_table, &ch->num, sizeof(ch->num), &hval2) == 0) {
                pj_hash_set(sess->pool, sess->ch_table, &ch->num, sizeof(ch->num), hval2, ch);
            }
        }
    }
...
static struct ch_t *lookup_ch_by_chnum(pj_turn_session *sess, pj_uint16_t chnum)
{
    return (struct ch_t *)pj_hash_get(sess->ch_table, &chnum, sizeof(chnum), NULL);
}

/*
 * Lookup permission and optionally create if it doesn't exist.
 */
static struct perm_t *lookup_perm(pj_turn_session *sess, const pj_sockaddr_t *addr, unsigned addr_len, pj_bool_t update)
{
    pj_uint32_t hval = 0;
    ...
    if (perm == NULL && update) {
        ...
        pj_hash_set(sess->pool, sess->perm_table, &perm->addr, addr_len, perm->hval, perm);
    }
...
static void invalidate_perm(pj_turn_session *sess, struct perm_t *perm)
{
    pj_hash_set(NULL, sess->perm_table, &perm->addr, pj_sockaddr_get_len(&perm->addr), perm->hval, NULL);
}

/*
 * Scan permission's hash table to refresh the permission.
 */
static unsigned refresh_permissions(pj_turn_session *sess, const pj_time_val *now)
{
    ...
    while (it) {
        ...
        if (perm->expiry.sec - 1 <= now->sec) {
            if (perm->renew) {
                ...
                if (tdata == NULL) {
                    /* Create a bare CreatePermission request */
                    status = pj_stun_session_create_req(sess->stun, PJ_STUN_CREATE_PERM_REQUEST, PJ_STUN_MAGIC, NULL,
                                                        &tdata);
                    if (status != PJ_SUCCESS) {
                        PJ_PERROR(1, (sess->obj_name, status, "Error creating CreatePermission request"));
                        return 0;
                    }
    ...
    if (tdata) {
        status = pj_stun_session_send_msg(sess->stun, req_token, PJ_FALSE, (sess->conn_type == PJ_TURN_TP_UDP),
                                          sess->srv_addr, pj_sockaddr_get_len(sess->srv_addr), tdata);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(1, (sess->obj_name, status, "Error sending CreatePermission request"));
            count = 0;
        }
    }
    ...
    if (eid == TIMER_KEEP_ALIVE) {
        ...
        while (it) {
            ...
            if (ch->bound && PJ_TIME_VAL_LTE(ch->expiry, now)) {
                 ...
                 * permission.
                 */
                pj_turn_session_bind_channel(sess, &ch->addr, pj_sockaddr_get_len(&ch->addr));
                pkt_sent = PJ_TRUE;
            }
        ...
        if (!pkt_sent && sess->alloc_param.ka_interval > 0) {
            ...
            /* Create blank SEND-INDICATION */
            rc = pj_stun_session_create_ind(sess->stun, PJ_STUN_SEND_INDICATION, &tdata);
            if (rc == PJ_SUCCESS) {
                /* Add DATA attribute with zero length */
                pj_stun_msg_add_binary_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_DATA, NULL, 0);

                /* Send the indication */
                pj_stun_session_send_msg(sess->stun, NULL, PJ_FALSE, PJ_FALSE, sess->srv_addr,
                                         pj_sockaddr_get_len(sess->srv_addr), tdata);
            }
        }
        ...
        if (resched) {
            ...
            pj_timer_heap_schedule_w_grp_lock(sess->timer_heap, &sess->timer, &delay, TIMER_KEEP_ALIVE, sess->grp_lock);
        }
