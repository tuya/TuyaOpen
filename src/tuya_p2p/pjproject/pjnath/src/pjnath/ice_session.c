...
static const char *check_state_name[] = {"Frozen", "Waiting", "In Progress", "Succeeded", "Failed"};
...
static void handle_incoming_check(pj_ice_sess *ice, const pj_ice_rx_check *rcheck);
...
static pj_status_t on_stun_send_msg(pj_stun_session *sess, void *token, const void *pkt, pj_size_t pkt_size,
                                    const pj_sockaddr_t *dst_addr, unsigned addr_len);
static pj_status_t on_stun_rx_request(pj_stun_session *sess, const pj_uint8_t *pkt, unsigned pkt_len,
                                      const pj_stun_rx_data *rdata, void *token, const pj_sockaddr_t *src_addr,
                                      unsigned src_addr_len);
static void on_stun_request_complete(pj_stun_session *stun_sess, pj_status_t status, void *token,
                                     pj_stun_tx_data *tdata, const pj_stun_msg *response, const pj_sockaddr_t *src_addr,
                                     unsigned src_addr_len);
static pj_status_t on_stun_rx_indication(pj_stun_session *sess, const pj_uint8_t *pkt, unsigned pkt_len,
                                         const pj_stun_msg *msg, void *token, const pj_sockaddr_t *src_addr,
                                         unsigned src_addr_len);

/* These are the callbacks for performing STUN authentication */
static pj_status_t stun_auth_get_auth(void *user_data, pj_pool_t *pool, pj_str_t *realm, pj_str_t *nonce);
static pj_status_t stun_auth_get_cred(const pj_stun_msg *msg, void *user_data, pj_pool_t *pool, pj_str_t *realm,
                                      pj_str_t *username, pj_str_t *nonce, pj_stun_passwd_type *data_type,
                                      pj_str_t *data);
static pj_status_t stun_auth_get_password(const pj_stun_msg *msg, void *user_data, const pj_str_t *realm,
 ...
 */
PJ_DEF(void)
pj_ice_calc_foundation(pj_pool_t *pool, pj_str_t *foundation, pj_ice_cand_type type, const pj_sockaddr *base_addr)
{
    ...
    if (base_addr->addr.sa_family == pj_AF_INET()) {
        ...
        val = pj_hash_calc(0, pj_sockaddr_get_addr(base_addr), pj_sockaddr_get_addr_len(base_addr));
    }
...
/* Init component */
static pj_status_t init_comp(pj_ice_sess *ice, unsigned comp_id, pj_ice_sess_comp *comp)
{
    ...
    status = pj_stun_session_create(&ice->stun_cfg, NULL, &sess_cb, PJ_TRUE, ice->grp_lock, &comp->stun_sess);
    ...
    pj_stun_session_set_credential(comp->stun_sess, PJ_STUN_AUTH_SHORT_TERM, &auth_cred);
...
PJ_DEF(void) pj_ice_sess_options_default(pj_ice_sess_options *opt)
{
    ...
    opt->controlled_agent_want_nom_timeout = ICE_CONTROLLED_AGENT_WAIT_NOMINATION_TIMEOUT;
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_sess_create(pj_stun_config *stun_cfg, const char *name, pj_ice_sess_role role, unsigned comp_cnt,
                   const pj_ice_sess_cb *cb, const pj_str_t *local_ufrag, const pj_str_t *local_passwd,
                   pj_grp_lock_t *grp_lock, pj_ice_sess **p_ice)
{
    pj_pool_t *pool;
    pj_ice_sess *ice;
    unsigned i;
    ...
    pool = pj_pool_create(stun_cfg->pf, name, PJNATH_POOL_LEN_ICE_SESS, PJNATH_POOL_INC_ICE_SESS, NULL);
    ...
    *p_ice = ice;
    ...
    LOG4((ice->obj_name, "ICE session created, comp_cnt=%d, role is %s agent", comp_cnt, role_names[ice->role]));
 ...
 */
PJ_DEF(pj_status_t) pj_ice_sess_get_options(pj_ice_sess *ice, pj_ice_sess_options *opt)
{
 ...
 */
PJ_DEF(pj_status_t) pj_ice_sess_set_options(pj_ice_sess *ice, const pj_ice_sess_options *opt)
{
    ...
    LOG5((ice->obj_name, "ICE nomination type set to %s", (ice->opt.aggressive ? "aggressive" : "regular")));
    return PJ_SUCCESS;
}
...
static void destroy_ice(pj_ice_sess *ice, pj_status_t reason)
{
    ...
    pj_timer_heap_cancel_if_active(ice->stun_cfg.timer_heap, &ice->timer, PJ_FALSE);
    ...
    pj_timer_heap_cancel_if_active(ice->stun_cfg.timer_heap, &ice->clist.timer, PJ_FALSE);
 ...
 */
PJ_DEF(pj_status_t) pj_ice_sess_detach_grp_lock(pj_ice_sess *ice, pj_grp_lock_handler *handler)
{
 ...
 * Change session role.
 */
PJ_DEF(pj_status_t) pj_ice_sess_change_role(pj_ice_sess *ice, pj_ice_sess_role new_role)
{
    PJ_ASSERT_RETURN(ice, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t) pj_ice_sess_set_prefs(pj_ice_sess *ice, const pj_uint8_t prefs[4])
{
    unsigned i;
    PJ_ASSERT_RETURN(ice && prefs, PJ_EINVAL);
    ice->prefs = (pj_uint8_t *)pj_pool_calloc(ice->pool, PJ_ICE_CAND_TYPE_MAX, sizeof(pj_uint8_t));
                                          ...
                                          pj_str_t *data)
{
    ...
    if (PJ_STUN_IS_SUCCESS_RESPONSE(msg->hdr.type) || PJ_STUN_IS_ERROR_RESPONSE(msg->hdr.type)) {
...
static pj_uint32_t CALC_CAND_PRIO(pj_ice_sess *ice, pj_ice_cand_type type, pj_uint32_t local_pref, pj_uint32_t comp_id)
{
#if PJNATH_ICE_PRIO_STD
    return ((ice->prefs[type] & 0xFF) << 24) + ((local_pref & 0xFFFF) << 8) + (((256 - comp_id) & 0xFF) << 0);
#else
    enum {
    ...
    return ((ice->prefs[type] & type_mask) << type_shift) + ((local_pref & local_mask) << local_shift) +
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_sess_add_cand(pj_ice_sess *ice, unsigned comp_id, unsigned transport_id, pj_ice_cand_type type,
                     pj_uint16_t local_pref, const pj_str_t *foundation, const pj_sockaddr_t *addr,
                     const pj_sockaddr_t *base_addr, const pj_sockaddr_t *rel_addr, int addr_len, unsigned *p_cand_id)
{
    ...
    PJ_ASSERT_RETURN(ice && comp_id && foundation && addr && base_addr && addr_len, PJ_EINVAL);
    ...
    if (ice->opt.trickle != PJ_ICE_SESS_TRICKLE_DISABLED) {
        ...
        for (i = 0; i < ice->lcand_cnt; ++i) {
            ...
            if (c->comp_id == comp_id && c->type == type && pj_sockaddr_cmp(&c->addr, addr) == 0 &&
    ...
    pj_assert(i < PJ_ARRAY_SIZE(ice->tp_data) && ice->tp_data[i].transport_id == transport_id);

    pj_ansi_strcpy(ice->tmp.txt, pj_sockaddr_print(&lcand->addr, address, sizeof(address), 2));
    LOG4((ice->obj_name,
          "Candidate %d added: comp_id=%d, type=%s, foundation=%.*s, "
          "addr=%s:%d, base=%s:%d, prio=0x%x (%u)",
          lcand->id, lcand->comp_id, cand_type_names[lcand->type], (int)lcand->foundation.slen, lcand->foundation.ptr,
          ice->tmp.txt, pj_sockaddr_get_port(&lcand->addr),
          pj_sockaddr_print(&lcand->base_addr, address, sizeof(address), 2), pj_sockaddr_get_port(&lcand->base_addr),
...
PJ_DEF(pj_status_t) pj_ice_sess_find_default_cand(pj_ice_sess *ice, unsigned comp_id, int *cand_id)
{
    unsigned i;
    ...
    *cand_id = -1;
    ...
    for (i = 0; i < ice->lcand_cnt; ++i) {
        ...
        if (lcand->comp_id == comp_id && lcand->type == PJ_ICE_CAND_TYPE_RELAYED) {
    ...
    for (i = 0; i < ice->lcand_cnt; ++i) {
        ...
        if (lcand->comp_id == comp_id &&
            (lcand->type == PJ_ICE_CAND_TYPE_SRFLX || lcand->type == PJ_ICE_CAND_TYPE_PRFLX)) {
            *cand_id = GET_LCAND_ID(lcand);
            pj_grp_lock_release(ice->grp_lock);
            return PJ_SUCCESS;
        }
...
static pj_timestamp CALC_CHECK_PRIO(const pj_ice_sess *ice, const pj_ice_sess_cand *lcand,
...
PJ_INLINE(int) CMP_CHECK_STATE(const pj_ice_sess_check *c1, const pj_ice_sess_check *c2)
{
    /* SUCCEEDED has higher state than FAILED */
    if (c1->state == PJ_ICE_SESS_CHECK_STATE_SUCCEEDED && c2->state == PJ_ICE_SESS_CHECK_STATE_FAILED) {
        return 1;
    }
    if (c2->state == PJ_ICE_SESS_CHECK_STATE_SUCCEEDED && c1->state == PJ_ICE_SESS_CHECK_STATE_FAILED) {
        return -1;
    }
...
PJ_INLINE(int) CMP_CHECK_PRIO(const pj_ice_sess_check *c1, const pj_ice_sess_check *c2)
{
    return pj_cmp_timestamp(&c1->prio, &c2->prio);
}

#if PJ_LOG_MAX_LEVEL >= 4
static const char *dump_check(char *buffer, unsigned bufsize, const pj_ice_sess_checklist *clist,
                              const pj_ice_sess_check *check)
{
    ...
    len = pj_ansi_snprintf(buffer, bufsize, "%d: [%d] %s:%d-->%s:%d", (int)GET_CHECK_ID(clist, check),
                           check->lcand->comp_id, pj_sockaddr_print(&lcand->addr, laddr, sizeof(laddr), 2),
                           pj_sockaddr_get_port(&lcand->addr), pj_sockaddr_print(&rcand->addr, raddr, sizeof(raddr), 2),
...
static void dump_checklist(const char *title, pj_ice_sess *ice, const pj_ice_sess_checklist *clist)
{
    unsigned i;
    ...
    for (i = 0; i < clist->count; ++i) {
        ...
        LOG4((ice->obj_name, " %s (%s, state=%s)", dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), clist, c),
              (c->nominated ? "nominated" : "not nominated"), check_state_name[c->state]));
    }
}
...
static void check_set_state(pj_ice_sess *ice, pj_ice_sess_check *check, pj_ice_sess_check_state st,
                            pj_status_t err_code)
{
    LOG5((ice->obj_name, "Check %s: state changed from %s to %s",
          dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), &ice->clist, check), check_state_name[check->state],
          check_state_name[st]));
...
static void clist_set_state(pj_ice_sess *ice, pj_ice_sess_checklist *clist, pj_ice_sess_checklist_state st)
{
    if (clist->state != st) {
        LOG5((ice->obj_name, "Checklist: state changed from %s to %s", clist_state_name[clist->state],
              clist_state_name[st]));
        clist->state = st;
    }
}
...
static void sort_checklist(pj_ice_sess *ice, pj_ice_sess_checklist *clist)
{
    ...
    for (i = 0; i < clist->count - 1; ++i) {
        ...
        for (j = i + 1; j < clist->count; ++j) {
            int cmp_state = CMP_CHECK_STATE(&clist->checks[j], &clist->checks[highest]);
            if (cmp_state > 0 || (cmp_state == 0 && CMP_CHECK_PRIO(&clist->checks[j], &clist->checks[highest]) > 0)) {
                highest = j;
            }
        }
        ...
        if (highest != i) {
            ...
            pj_memcpy(&clist->checks[i], &clist->checks[highest], sizeof(pj_ice_sess_check));
...
static void remove_check(pj_ice_sess *ice, pj_ice_sess_checklist *clist, unsigned check_idx, const char *reason)
{
    LOG5((ice->obj_name, "Check %s pruned (%s)",
          dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), clist, &clist->checks[check_idx]), reason));

    pj_array_erase(clist->checks, sizeof(clist->checks[0]), clist->count, check_idx);
    --clist->count;
}
 ...
 * is sorted.
 */
static pj_status_t prune_checklist(pj_ice_sess *ice, pj_ice_sess_checklist *clist)
{
    unsigned i;
    ...
    for (i = 0; i < clist->count; ++i) {
        ...
        if (srflx->type == PJ_ICE_CAND_TYPE_SRFLX || srflx->type == PJ_ICE_CAND_TYPE_PRFLX) {
            ...
            if (j == ice->lcand_cnt) {
                ...
                LOG4((ice->obj_name, "Base candidate %s:%d not found for srflx candidate %d",
                      pj_sockaddr_print(&srflx->base_addr, baddr, sizeof(baddr), 2),
                      pj_sockaddr_get_port(&srflx->base_addr), GET_LCAND_ID(srflx)));
    ...
    for (i = 0; i < clist->count; ++i) {
        ...
        for (j = i + 1; j < clist->count;) {
            ...
            } else if ((rjcand == ricand) && (pj_sockaddr_cmp(&ljcand->base_addr, &licand->base_addr) == 0)) {
                reason = "equal base";
            }
...
static void on_timer(pj_timer_heap_t *th, pj_timer_entry *te)
{
    ...
    case TIMER_CONTROLLED_WAIT_NOM:
        LOG4((ice->obj_name, "Controlled agent timed-out in waiting for the controlling "
...
static void ice_keep_alive(pj_ice_sess *ice, pj_bool_t send_now)
{
    if (send_now) {
        ...
        status = pj_stun_session_create_ind(comp->stun_sess, PJ_STUN_BINDING_INDICATION, &tdata);
        if (status != PJ_SUCCESS)
            goto done;
    ...
    if (ice->timer.id == TIMER_NONE) {
        ...
        delay.msec =
            (PJ_ICE_SESS_KEEP_ALIVE_MIN + (pj_rand() % PJ_ICE_SESS_KEEP_ALIVE_MAX_RAND)) * 1000 / ice->comp_cnt;
...
static void on_ice_complete(pj_ice_sess *ice, pj_status_t status)
{
    if (!ice->is_complete) {
        ...
        if (ice->cb.on_ice_complete) {
            ...
            pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &ice->timer, &delay, TIMER_COMPLETION_CALLBACK,
                                              ice->grp_lock);
        }
    }
}
...
static void update_comp_check(pj_ice_sess *ice, unsigned comp_id, pj_ice_sess_check *check)
{
    pj_ice_sess_comp *comp;
    ...
    if (comp->valid_check == NULL) {
        ...
        if (!comp->nominated_check && check->nominated) {
            update = PJ_TRUE;
        } else if (CMP_CHECK_PRIO(comp->valid_check, check) < 0 && (!comp->nominated_check || check->nominated)) {
            update = PJ_TRUE;
        }
...
static pj_bool_t check_ice_complete(pj_ice_sess *ice)
{
    ...
    if (no_pending_check) {
        ...
        if (ice->role == PJ_ICE_SESS_ROLE_CONTROLLED) {
            ...
            if (i < ice->comp_cnt) {
                 ...
                 */
                if (ice->timer.id == TIMER_NONE && ice->opt.controlled_agent_want_nom_timeout >= 0) {
                    pj_time_val delay;
                    ...
                    pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &ice->timer, &delay,
             ...
             * flag set.
             */
            LOG4((ice->obj_name, "All checks have completed, starting nominated checks now"));
            start_nominated_check(ice);
            return PJ_FALSE;
        }
    }
    ...
    if (/*check->err_code == PJ_SUCCESS && */
        ...
        LOG4((ice->obj_name, "Scheduling nominated check in %d ms", ice->opt.nominated_check_delay));
        ...
        pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &ice->timer, &delay, TIMER_START_NOMINATED_CHECK,
                                          ice->grp_lock);
        return PJ_FALSE;
    }
...
static pj_bool_t on_check_complete(pj_ice_sess *ice, pj_ice_sess_check *check)
{
    ...
    if (check->err_code == PJ_SUCCESS) {
        ...
        for (i = 0; i < ice->clist.count; ++i) {
            pj_ice_sess_check *c = &ice->clist.checks[i];
            if (c->foundation_idx == check->foundation_idx && c->state == PJ_ICE_SESS_CHECK_STATE_FROZEN) {
                check_set_state(ice, c, PJ_ICE_SESS_CHECK_STATE_WAITING, 0);
            }
        }

        LOG5((ice->obj_name, "Check %d is successful%s", GET_CHECK_ID(&ice->clist, check),
        ...
        for (i = 0; i < ice->clist.count; ++i) {
            ...
            if (c->lcand->comp_id == check->lcand->comp_id) {
                ...
                if (c->state < PJ_ICE_SESS_CHECK_STATE_IN_PROGRESS) {
                    ...
                    LOG5((ice->obj_name, "Check %s to be failed because state is %s",
                          dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), &ice->clist, c), check_state_name[c->state]));
                    check_set_state(ice, c, PJ_ICE_SESS_CHECK_STATE_FAILED, PJ_ECANCELLED);
                    ...
                    if (c->tdata) {
                        LOG5((ice->obj_name, "Cancelling check %s (In Progress)",
                              dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), &ice->clist, c)));
                        pj_stun_session_cancel_req(comp->stun_sess, c->tdata, PJ_FALSE, 0);
                        c->tdata = NULL;
                        check_set_state(ice, c, PJ_ICE_SESS_CHECK_STATE_FAILED, PJ_ECANCELLED);
                    }
                }
 ...
 * to checklist.
 */
static int get_check_foundation_idx(pj_ice_sess *ice, const pj_ice_sess_cand *lcand, const pj_ice_sess_cand *rcand,
                                    pj_bool_t add_if_not_found)
{
    ...
    pj_ansi_snprintf(fnd_str, sizeof(fnd_str), "%.*s|%.*s", (int)lcand->foundation.slen, lcand->foundation.ptr,
 ...
 */
static int discard_check(pj_ice_sess *ice, pj_ice_sess_checklist *clist, const pj_timestamp *prio_lower_than)
{
    ...
    if (!prio_lower_than || pj_cmp_timestamp(&clist->checks[k - 1].prio, prio_lower_than) < 0) {
...
static pj_status_t add_rcand_and_update_checklist(pj_ice_sess *ice, unsigned rem_cand_cnt,
                                                  const pj_ice_sess_cand rem_cand[], pj_bool_t trickle_done)
{
    pj_ice_sess_checklist *clist;
    ...
    for (i = 0; i < rem_cand_cnt; ++i) {
        ...
        if (ice->opt.trickle != PJ_ICE_SESS_TRICKLE_DISABLED) {
            ...
            for (j = 0; j < ice->rcand_cnt; ++j) {
                ...
                if (c1->comp_id == c2->comp_id && c1->type == c2->type && pj_sockaddr_cmp(&c1->addr, &c2->addr) == 0) {
                    break;
                }
            }
        ...
        if (ice->rcand_cnt >= PJ_ICE_MAX_CAND) {
            ...
            PJ_PERROR(3, (ice->obj_name, PJ_ETOOMANY, "Cannot add remote candidate %s",
    ...
    for (i = 0; i < ice->lcand_cnt; ++i) {
        ...
        for (j = rstart; j < ice->rcand_cnt; ++j) {
             ...
             */
            if ((lcand->comp_id != rcand->comp_id) || (lcand->addr.addr.sa_family != rcand->addr.addr.sa_family)) {
                continue;
            }
            ...
            chk->foundation_idx = get_check_foundation_idx(ice, lcand, rcand, PJ_TRUE);
     ...
     */
    if (ice->opt.trickle != PJ_ICE_SESS_TRICKLE_DISABLED && clist->count > 0 && !ice->is_complete &&
        clist->state == PJ_ICE_SESS_CHECKLIST_ST_RUNNING) {
        if (!pj_timer_entry_running(&clist->timer)) {
            pj_time_val delay = {0, 0};
            status = pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &clist->timer, &delay, PJ_TRUE,
                                                       ice->grp_lock);
            if (status == PJ_SUCCESS) {
                LOG5((ice->obj_name, "Trickle ICE resumes periodic check because "
    ...
    if (trickle_done && pj_timer_entry_running(&ice->timer_end_of_cand)) {
        pj_timer_heap_cancel_if_active(ice->stun_cfg.timer_heap, &ice->timer_end_of_cand, 0);
    }
...
PJ_DEF(pj_status_t)
pj_ice_sess_create_check_list(pj_ice_sess *ice, const pj_str_t *rem_ufrag, const pj_str_t *rem_passwd,
                              unsigned rem_cand_cnt, const pj_ice_sess_cand rem_cand[])
{
    pj_ice_sess_checklist *clist;
    char buf[128];
    pj_str_t username;
    ...
    if (ice->lcand_cnt > 0 && rem_cand_cnt > 0) {
        status = add_rcand_and_update_checklist(ice, rem_cand_cnt, rem_cand, !ice->is_trickling);
        if (status != PJ_SUCCESS) {
            pj_grp_lock_release(ice->grp_lock);
            return status;
        }
...
PJ_DEF(pj_status_t)
pj_ice_sess_update_check_list(pj_ice_sess *ice, const pj_str_t *rem_ufrag, const pj_str_t *rem_passwd,
                              unsigned rem_cand_cnt, const pj_ice_sess_cand rem_cand[], pj_bool_t trickle_done)
{
    pj_status_t status = PJ_SUCCESS;

    PJ_ASSERT_RETURN(ice && ((rem_cand_cnt == 0) || (rem_ufrag && rem_passwd && rem_cand)), PJ_EINVAL);

    pj_grp_lock_acquire(ice->grp_lock);

    /* Ignore if remote ufrag has not known yet */
    if (ice->tx_ufrag.slen == 0) {
        LOG5((ice->obj_name, "Cannot update ICE checklist when remote ufrag is unknown"));
        pj_grp_lock_release(ice->grp_lock);
    ...
    if (!ice->is_trickling && rem_cand_cnt) {
        LOG5((ice->obj_name, "Ignored remote candidate update as ICE trickling has ended"));
        pj_grp_lock_release(ice->grp_lock);
        return PJ_SUCCESS;
    }

    /* Verify remote ufrag & passwd, if remote candidate specified */
    if (rem_cand_cnt && (pj_strcmp(&ice->tx_ufrag, rem_ufrag) || pj_strcmp(&ice->tx_pass, rem_passwd))) {
    ...
    if (status == PJ_SUCCESS) {
        status = add_rcand_and_update_checklist(ice, rem_cand_cnt, rem_cand, trickle_done);
    }
...
static pj_status_t perform_check(pj_ice_sess *ice, pj_ice_sess_checklist *clist, unsigned check_id, pj_bool_t nominate)
{
    pj_ice_sess_comp *comp;
    pj_ice_msg_data *msg_data;
    ...
    /* Create request */
    status = pj_stun_session_create_req(comp->stun_sess, PJ_STUN_BINDING_REQUEST, PJ_STUN_MAGIC, NULL, &check->tdata);
    if (status != PJ_SUCCESS) {
    ...
    /* Add PRIORITY */
#if PJNATH_ICE_PRIO_STD
    prio = CALC_CAND_PRIO(ice, PJ_ICE_CAND_TYPE_PRFLX, 65535 - lcand->id, lcand->comp_id);
#else
    prio = CALC_CAND_PRIO(ice, PJ_ICE_CAND_TYPE_PRFLX, ((1 << PJ_ICE_LOCAL_PREF_BITS) - 1) - lcand->id, lcand->comp_id);
#endif
    pj_stun_msg_add_uint_attr(check->tdata->pool, check->tdata->msg, PJ_STUN_ATTR_PRIORITY, prio);
    ...
    if (ice->role == PJ_ICE_SESS_ROLE_CONTROLLING) {
        if (nominate) {
            pj_stun_msg_add_empty_attr(check->tdata->pool, check->tdata->msg, PJ_STUN_ATTR_USE_CANDIDATE);
            check->nominated = PJ_TRUE;
        }

        pj_stun_msg_add_uint64_attr(check->tdata->pool, check->tdata->msg, PJ_STUN_ATTR_ICE_CONTROLLING,
                                    &ice->tie_breaker);

    } else {
        pj_stun_msg_add_uint64_attr(check->tdata->pool, check->tdata->msg, PJ_STUN_ATTR_ICE_CONTROLLED,
                                    &ice->tie_breaker);
    }
...
static pj_status_t start_periodic_check(pj_timer_heap_t *th, pj_timer_entry *te)
{
    ...
    if (start_count != 0) {
        ...
        pj_timer_heap_schedule_w_grp_lock(th, te, &timeout, PJ_TRUE, ice->grp_lock);
    }
...
static void start_nominated_check(pj_ice_sess *ice)
{
    ...
    for (i = 0; i < ice->comp_cnt; ++i) {
        ...
        for (j = 0; j < ice->clist.count; ++j) {
            ...
            if (c->lcand->transport_id == vc->lcand->transport_id && c->rcand == vc->rcand) {
                pj_assert(c->err_code == PJ_SUCCESS);
                c->state = PJ_ICE_SESS_CHECK_STATE_FROZEN;
                check_set_state(ice, c, PJ_ICE_SESS_CHECK_STATE_WAITING, PJ_SUCCESS);
                break;
            }
        }
    }
    ...
    status =
        pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &ice->clist.timer, &delay, PJ_TRUE, ice->grp_lock);
    if (status == PJ_SUCCESS) {
...
PJ_DEF(pj_status_t) pj_ice_sess_start_check(pj_ice_sess *ice)
{
    ...
    for (i = 0; i < clist->foundation_cnt; ++i) {
        ...
        for (k = 0; k < clist->count; ++k) {
            ...
            if (c->foundation_idx != (int)i || c->state != PJ_ICE_SESS_CHECK_STATE_FROZEN) {
                continue;
            }
            ...
            if (c->lcand->comp_id == chk->lcand->comp_id && pj_cmp_timestamp(&c->prio, &chk->prio) > 0) {
                chk = c;
                continue;
            }
        }
    ...
    while (rcheck != &ice->early_check) {
        LOG4((ice->obj_name, "Performing delayed triggerred check for component %d", rcheck->comp_id));
        pj_log_push_indent();
    ...
    if (ice->is_trickling && !pj_timer_entry_running(&ice->timer_end_of_cand)) {
        ...
        pj_timer_entry_init(&ice->timer_end_of_cand, 0, ice, &end_of_cand_ind_timer);
        status = pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &ice->timer_end_of_cand, &delay, PJ_TRUE,
                                                   ice->grp_lock);
        if (status != PJ_SUCCESS) {
            LOG4((ice->obj_name, "Failed to schedule end-of-candidate indication timer"));
        }
    }
                                    ...
                                    const pj_sockaddr_t *dst_addr, unsigned addr_len)
{
    ...
    status = (*ice->cb.on_tx_pkt)(ice, sd->comp_id, msg_data->transport_id, pkt, pkt_size, dst_addr, addr_len);
                                     ...
                                     unsigned src_addr_len)
{
    ...
    if (status != PJ_SUCCESS) {
        ...
        if (status == PJ_STATUS_FROM_STUN_CODE(PJ_STUN_SC_ROLE_CONFLICT)) {
            ...
            } else if (pj_stun_msg_find_attr(req, PJ_STUN_ATTR_ICE_CONTROLLED, 0)) {
            ...
            if (new_role != ice->role) {
                LOG4((ice->obj_name, "Changing role because of role conflict response"));
            ...
            perform_check(ice, clist, ckid, check->nominated || ice->is_nominating);
            pj_log_pop_indent();
            pj_grp_lock_release(ice->grp_lock);
            return;
        }
        ...
        pj_strerror(status, errmsg, sizeof(errmsg));
        LOG4((ice->obj_name, "Check %s%s: connectivity check FAILED: %s",
    ...
    if (pj_sockaddr_cmp(&check->rcand->addr, source_addr) != 0) {
        ...
        LOG4((ice->obj_name, "Check %s%s: connectivity check FAILED: source address mismatch",
              dump_check(ice->tmp.txt, sizeof(ice->tmp.txt), &ice->clist, check),
    ...
    xaddr = (pj_stun_xor_mapped_addr_attr *)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_XOR_MAPPED_ADDR, 0);
    if (!xaddr) {
        check_set_state(ice, check, PJ_ICE_SESS_CHECK_STATE_FAILED, PJNATH_ESTUNNOMAPPEDADDR);
        on_check_complete(ice, check);
        pj_grp_lock_release(ice->grp_lock);
        return;
    }
    ...
    if (lcand == NULL) {
        ...
        pj_ice_calc_foundation(ice->pool, &foundation, PJ_ICE_CAND_TYPE_PRFLX, &check->lcand->base_addr);
        ...
        status = pj_ice_sess_add_cand(ice, check->lcand->comp_id, msg_data->transport_id, PJ_ICE_CAND_TYPE_PRFLX,
#if PJNATH_ICE_PRIO_STD
                                      65535 - (pj_uint16_t)ice->lcand_cnt,
#else
                                      ((1 << PJ_ICE_LOCAL_PREF_BITS) - 1) - ice->lcand_cnt,
#endif
                                      &foundation, &xaddr->sockaddr, &check->lcand->base_addr, &check->lcand->base_addr,
                                      pj_sockaddr_get_len(&xaddr->sockaddr), &cand_id);
        ...
        if (status != PJ_SUCCESS || cand_id == ice->lcand_cnt) {
            if (cand_id == ice->lcand_cnt) {
                LOG4((ice->obj_name, "Cannot add any candidate, all IPv6 seems deprecated"));
            }
            check_set_state(ice, check, PJ_ICE_SESS_CHECK_STATE_FAILED, status);
            on_check_complete(ice, check);
            pj_grp_lock_release(ice->grp_lock);
     ...
     */
    for (i = 0; i < ice->valid_list.count; ++i) {
        if (ice->valid_list.checks[i].lcand == lcand && ice->valid_list.checks[i].rcand == check->rcand)
            break;
    }
                                      ...
                                      unsigned src_addr_len)
{
    ...
    if (msg->hdr.type != PJ_STUN_BINDING_REQUEST) {
        pj_stun_session_respond(sess, rdata, PJ_STUN_SC_BAD_REQUEST, NULL, token, PJ_TRUE, src_addr, src_addr_len);
        return PJ_SUCCESS;
    }
    ...
    prio_attr = (pj_stun_priority_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_PRIORITY, 0);
    if (prio_attr == NULL) {
    ...
    uc_attr = (pj_stun_use_candidate_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_USE_CANDIDATE, 0);

    /* Get ICE-CONTROLLING or ICE-CONTROLLED */
    role_attr = (pj_stun_uint64_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_ICE_CONTROLLING, 0);
    if (role_attr == NULL) {
        role_attr = (pj_stun_uint64_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_ICE_CONTROLLED, 0);
    }
    ...
    if (ice->tx_ufrag.slen == 0) {
        ...
        uname_attr = (pj_stun_string_attr *)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_USERNAME, 0);
     ...
     */
    if (ice->role == PJ_ICE_SESS_ROLE_CONTROLLING && role_attr && role_attr->hdr.type == PJ_STUN_ATTR_ICE_CONTROLLING) {
        if (pj_cmp_timestamp(&ice->tie_breaker, &role_attr->value) < 0) {
            /* Switch role to controlled */
            LOG4((ice->obj_name, "Changing role because of ICE-CONTROLLING attribute"));
            pj_ice_sess_change_role(ice, PJ_ICE_SESS_ROLE_CONTROLLED);
        } else {
            /* Generate 487 response */
            pj_stun_session_respond(sess, rdata, PJ_STUN_SC_ROLE_CONFLICT, NULL, token, PJ_TRUE, src_addr,
                                    src_addr_len);
            pj_grp_lock_release(ice->grp_lock);
            return PJ_SUCCESS;
        }
        ...
        if (pj_cmp_timestamp(&ice->tie_breaker, &role_attr->value) < 0) {
            ...
            /* Switch role to controlled */
            LOG4((ice->obj_name, "Changing role because of ICE-CONTROLLED attribute"));
    ...
    if (((pj_sockaddr *)src_addr)->addr.sa_family == pj_AF_INET6()) {
        ...
        for (i = 0; i < ice->clist.count; ++i) {
            pj_ice_sess_check *c = &ice->clist.checks[i];
            if (c->lcand->comp_id == sd->comp_id && c->lcand->transport_id == transport_id) {
                lcand = c->lcand;
                break;
            }
        }
        ...
        if (lcand != NULL && lcand->addr.addr.sa_family == pj_AF_INET()) {
            ...
            for (i = 0; i < ice->rcand_cnt; ++i) {
                ...
                status = pj_sockaddr_synthesize(pj_AF_INET6(), &synth_addr, &ice->rcand[i].addr);
                if (status == PJ_SUCCESS && pj_sockaddr_cmp(src_addr, &synth_addr) == 0) {
    ...
    status = pj_stun_msg_add_sockaddr_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_XOR_MAPPED_ADDR, PJ_TRUE, source_addr,
                                           source_addr_len);
    ...
    /* Send the response */
    status = pj_stun_session_send_msg(sess, msg_data, PJ_TRUE, PJ_TRUE, src_addr, src_addr_len, tdata);
    ...
    if (ice->tx_ufrag.slen == 0) {
        ...
        LOG4((ice->obj_name, "Received an early check for comp %d", rcheck->comp_id));
...
static void handle_incoming_check(pj_ice_sess *ice, const pj_ice_rx_check *rcheck)
{
    ...
    if (i == ice->rcand_cnt) {
        ...
        rcand->foundation.slen = pj_ansi_snprintf(rcand->foundation.ptr, 36, "f%p", p);

        LOG4((ice->obj_name, "Added new remote candidate from the request: %s:%d",
              pj_sockaddr_print(&rcand->addr, raddr, sizeof(raddr), 2), pj_sockaddr_get_port(&rcand->addr)));
    ...
    for (i = 0; i < ice->clist.count; ++i) {
        pj_ice_sess_check *c = &ice->clist.checks[i];
        if (c->lcand->comp_id == rcheck->comp_id && c->lcand->transport_id == rcheck->transport_id) {
            lcand = c->lcand;
            break;
        }
    }
    if (lcand == NULL) {
         ...
         */
        LOG4((ice->obj_name, "Received Binding request but no local candidate is found!"));
        return;
    }
#endif
    ...
    if (i != ice->clist.count) {
        ...
        if (c->state == PJ_ICE_SESS_CHECK_STATE_FROZEN || c->state == PJ_ICE_SESS_CHECK_STATE_WAITING) {
            ...
            if (rcheck->use_candidate) {
                for (j = 0; j < ice->valid_list.count; ++j) {
                    ...
                    if (vc->lcand->transport_id == c->lcand->transport_id && vc->rcand == c->rcand) {
                        /* Set nominated flag */
    ...
    /* Note: only do this if we don't have too many checks in checklist */
        ...
        if (ice->is_nominating && !ice->opt.aggressive) {
            ...
            if (!pj_timer_entry_running(&ice->clist.timer)) {
                pj_time_val delay = {0, 0};
                pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap, &ice->clist.timer, &delay, PJ_TRUE,
                                                  ice->grp_lock);
            }
        } else {
            pj_bool_t nominate;
...
PJ_DEF(pj_status_t) pj_ice_sess_send_data(pj_ice_sess *ice, unsigned comp_id, const void *data, pj_size_t data_len)
{
    pj_status_t status = PJ_SUCCESS;
    pj_ice_sess_comp *comp;
    ...
    status = (*ice->cb.on_tx_pkt)(ice, comp_id, transport_id, data, data_len, &addr, pj_sockaddr_get_len(&addr));
...
PJ_DEF(pj_status_t)
pj_ice_sess_on_rx_pkt(pj_ice_sess *ice, unsigned comp_id, unsigned transport_id, void *pkt, pj_size_t pkt_size,
                      const pj_sockaddr_t *src_addr, int src_addr_len)
{
     ...
     */
    status = pj_stun_msg_check((const pj_uint8_t *)pkt, pkt_size, PJ_STUN_IS_DATAGRAM | PJ_STUN_NO_FINGERPRINT_CHECK);
    if (status == PJ_SUCCESS) {
        status = pj_stun_session_on_rx_pkt(comp->stun_sess, pkt, pkt_size, PJ_STUN_IS_DATAGRAM, msg_data, NULL,
                                           src_addr, src_addr_len);
        if (status != PJ_SUCCESS) {
            pj_strerror(status, ice->tmp.errmsg, sizeof(ice->tmp.errmsg));
            LOG4((ice->obj_name, "Error processing incoming message: %s", ice->tmp.errmsg));
        }
        pj_grp_lock_release(ice->grp_lock);
    } else {
        ...
        (*ice->cb.on_rx_data)(ice, comp_id, transport_id, pkt, pkt_size, src_addr, src_addr_len);
        status = PJ_SUCCESS;
    }
