...
/* Transport IDs */
enum tp_type { TP_NONE, TP_STUN, TP_TURN };
...
static pj_status_t ice_tx_pkt(pj_ice_sess *ice, unsigned comp_id, unsigned transport_id, const void *pkt,
                              pj_size_t size, const pj_sockaddr_t *dst_addr, unsigned dst_addr_len);
static void ice_rx_data(pj_ice_sess *ice, unsigned comp_id, unsigned transport_id, void *pkt, pj_size_t size,
...
static pj_bool_t stun_on_rx_data(pj_stun_sock *stun_sock, void *pkt, unsigned pkt_len, const pj_sockaddr_t *src_addr,
                                 unsigned addr_len);
/* Notifification when asynchronous send operation has completed. */
static pj_bool_t stun_on_data_sent(pj_stun_sock *stun_sock, pj_ioqueue_op_key_t *send_key, pj_ssize_t sent);
...
/* TURN callbacks */
static void turn_on_rx_data(pj_turn_sock *turn_sock, void *pkt, unsigned pkt_len, const pj_sockaddr_t *peer_addr,
                            unsigned addr_len);
 ...
 * Copy configuration.
 */
PJ_DEF(void) pj_ice_strans_cfg_copy(pj_pool_t *pool, pj_ice_strans_cfg *dst, const pj_ice_strans_cfg *src)
{
    unsigned i;
    ...
    for (i = 0; i < src->turn_tp_cnt; ++i) {
        ...
        pj_stun_auth_cred_dup(pool, &dst->turn_tp[i].auth_cred, &src->turn_tp[i].auth_cred);
    }
}
 ...
 */
static pj_status_t add_update_turn(pj_ice_strans *ice_st, pj_ice_strans_comp *comp, unsigned idx, unsigned max_cand_cnt)
{
            ...
            for (i = 0; i < comp->cand_cnt; ++i) {
                if (comp->cand_list[i].type == PJ_ICE_CAND_TYPE_SRFLX) {
                    ...
                    if (ice_st->cfg.af == pj_AF_UNSPEC() ||
                        comp->cand_list[i].base_addr.addr.sa_family == ice_st->cfg.af) {
                        break;
                    }
                }
            }
    ...
    if (ice_st->cfg.comp[comp_idx].qos_params.flags)
        pj_memcpy(&sock_cfg->qos_params, &ice_st->cfg.comp[comp_idx].qos_params, sizeof(sock_cfg->qos_params));
    ...
    status = pj_turn_sock_create(&ice_st->cfg.stun_cfg, turn_cfg->af, turn_cfg->conn_type, &turn_sock_cb, sock_cfg,
...
static pj_bool_t ice_cand_equals(pj_ice_sess_cand *lcand, pj_ice_sess_cand *rcand)
{
    ...
    if (lcand->type != rcand->type || lcand->status != rcand->status || lcand->comp_id != rcand->comp_id ||
        ...
        || lcand->prio != rcand->prio || pj_sockaddr_cmp(&lcand->addr, &rcand->addr) != 0 ||
...
static pj_status_t add_stun_and_host(pj_ice_strans *ice_st, pj_ice_strans_comp *comp, unsigned idx,
                                     unsigned max_cand_cnt)
{
    ...
    status = pj_stun_sock_create(&ice_st->cfg.stun_cfg, NULL, stun_cfg->af, &stun_sock_cb, sock_cfg, data,
                                 &comp->stun[idx].sock);
    if (status != PJ_SUCCESS)
        return status;
    ...
    while (stun_cfg->server.slen) {
        ...
        status = pj_stun_sock_start(comp->stun[idx].sock, &stun_cfg->server, stun_cfg->port, ice_st->cfg.resolver);
        if (status != PJ_SUCCESS) {
            /// sess_dec_ref(ice_st);
        ...
        pj_ice_calc_foundation(ice_st->pool, &cand->foundation, cand->type, &cand->base_addr);
        comp->cand_cnt++;
        max_cand_cnt--;
    ...
    if (stun_cfg->max_host_cands) {
        ...
        status = pj_stun_sock_get_info(comp->stun[idx].sock, &stun_sock_info);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (ice_st->obj_name, status, "Failed in querying STUN socket info"));
            return status;
        }

        for (i = 0; i < stun_sock_info.alias_cnt && cand_cnt < stun_cfg->max_host_cands; ++i) {
            unsigned j;
            ...
            if (stun_cfg->loop_addr == PJ_FALSE) {
                if (stun_cfg->af == pj_AF_INET() && (pj_ntohl(addr->ipv4.sin_addr.s_addr) >> 24) == 127) {
                    continue;
            ...
            if (cand_duplicate) {
                PJ_LOG(4, (ice_st->obj_name, "Comp %d: host candidate %s (tpid=%d) is a duplicate", comp->comp_id,
                           pj_sockaddr_print(&cand->addr, addrinfo, sizeof(addrinfo), 3), cand->transport_id));
             ...
             * address family
             */
            if (comp->ice_st->cfg.af != pj_AF_UNSPEC() && addr->addr.sa_family == comp->ice_st->cfg.af &&
                comp->cand_list[comp->default_cand].base_addr.addr.sa_family != ice_st->cfg.af) {
                comp->default_cand = (unsigned)(cand - comp->cand_list);
            }

            PJ_LOG(4, (ice_st->obj_name, "Comp %d/%d: host candidate %s (tpid=%d) added", comp->comp_id,
                       comp->cand_cnt - 1, pj_sockaddr_print(&cand->addr, addrinfo, sizeof(addrinfo), 3),
                       cand->transport_id));
        }
    }
...
static pj_status_t create_comp(pj_ice_strans *ice_st, unsigned comp_id)
{
    ...
    for (i = 0; i < ice_st->cfg.stun_tp_cnt; ++i) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(3,
                      (ice_st->obj_name, status, "Failed creating STUN transport #%d for comp %d", i, comp->comp_id));
            // return status;
        }
    }
    ...
    for (i = 0; i < ice_st->cfg.turn_tp_cnt; ++i) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(3,
                      (ice_st->obj_name, status, "Failed creating TURN transport #%d for comp %d", i, comp->comp_id));
    ...
    if (comp->cand_cnt == 0) {
        PJ_LOG(4, (ice_st->obj_name, "Error: no candidate is created due to settings"));
        return PJ_EINVAL;
    }
...
static pj_status_t alloc_send_buf(pj_ice_strans *ice_st, unsigned buf_size)
{
    if (buf_size > ice_st->buf_size) {
        ...
        ice_st->buf_pool =
            pj_pool_create(ice_st->pf, "ice_buf", (buf_size + sizeof(pending_send)) * ice_st->num_buf, 512, NULL);
        if (!ice_st->buf_pool)
            return PJ_ENOMEM;

        ice_st->buf_size = buf_size;
        ice_st->send_buf = pj_pool_calloc(ice_st->buf_pool, ice_st->num_buf, sizeof(pending_send));
        for (i = 0; i < ice_st->num_buf; i++) {
            ice_st->send_buf[i].buffer = pj_pool_alloc(ice_st->buf_pool, buf_size);
        }
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_create(const char *name, const pj_ice_strans_cfg *cfg, unsigned comp_cnt, void *user_data,
                     const pj_ice_strans_cb *cb, pj_ice_strans **p_ice_st)
{
    ...
    PJ_ASSERT_RETURN(comp_cnt && cb && p_ice_st && comp_cnt <= PJ_ICE_MAX_COMP, PJ_EINVAL);

    if (name == NULL)
        name = "ice%p";

    pool = pj_pool_create(cfg->stun_cfg.pf, name, PJNATH_POOL_LEN_ICE_STRANS, PJNATH_POOL_INC_ICE_STRANS, NULL);
    ...
    PJ_LOG(4, (ice_st->obj_name, "Creating ICE stream transport with %d component(s)", comp_cnt));
    pj_log_push_indent();
     ...
     */
    if (cfg->stun_tp_cnt == 0 && (cfg->stun.server.slen || cfg->stun.max_host_cands)) {
    ...
    ice_st->comp = (pj_ice_strans_comp **)pj_pool_calloc(pool, comp_cnt, sizeof(pj_ice_strans_comp *));
...
static void destroy_ice_st(pj_ice_strans *ice_st)
{
    ...
    PJ_LOG(5, (ice_st->obj_name, "ICE stream transport %p destroy request..", ice_st));
    pj_log_push_indent();
...
static void sess_fail(pj_ice_strans *ice_st, pj_ice_strans_op op, const char *title, pj_status_t status)
{
...
static void sess_init_update(pj_ice_strans *ice_st)
{
    ...
    for (i = 0; i < ice_st->comp_cnt; ++i) {
        ...
        if (!comp || comp->creating) {
            PJ_LOG(5, (ice_st->obj_name, "ICE init update: creating comp %d", (comp ? comp->comp_id : (i + 1))));
            return;
        }
    ...
    if (ice_st->ice && ice_st->ice->is_trickling && ice_st->rem_cand_end) {
        pj_ice_sess_update_check_list(ice_st->ice, NULL, NULL, 0, NULL, PJ_TRUE);
    }
}
 ...
 */
PJ_DEF(pj_status_t) pj_ice_strans_get_options(pj_ice_strans *ice_st, pj_ice_sess_options *opt)
{
 ...
 */
PJ_DEF(pj_status_t) pj_ice_strans_set_options(pj_ice_strans *ice_st, const pj_ice_sess_options *opt)
{
    PJ_ASSERT_RETURN(ice_st && opt, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t) pj_ice_strans_update_comp_cnt(pj_ice_strans *ice_st, unsigned comp_cnt)
{
    unsigned i;
    ...
    PJ_LOG(4, (ice_st->obj_name, "Updated ICE stream transport components number to %d", comp_cnt));
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_init_ice(pj_ice_strans *ice_st, pj_ice_sess_role role, const pj_str_t *local_ufrag,
                       const pj_str_t *local_passwd)
{
    ...
    /* Create! */
    status = pj_ice_sess_create(&ice_st->cfg.stun_cfg, ice_st->obj_name, role, ice_st->comp_cnt, &ice_cb, local_ufrag,
    ...
    for (i = 0; i < ice_st->comp_cnt; ++i) {
        ...
        for (j = 0; j < comp->cand_cnt; ++j) {
            ...
            if (cand->status != PJ_SUCCESS) {
                PJ_LOG(5, (ice_st->obj_name, "Candidate %d of comp %d is not added (pending)", j, i));
                continue;
            }
            ...
            /* Add the candidate */
            status = pj_ice_sess_add_cand(ice_st->ice, comp->comp_id, cand->transport_id, cand->type, cand->local_pref,
                                          &cand->foundation, &cand->addr, &cand->base_addr, &cand->rel_addr,
...
PJ_DEF(pj_bool_t) pj_ice_strans_sess_is_running(pj_ice_strans *ice_st)
{
    ...
    return ice_st && ice_st->ice && /* ice_st->ice->rcand_cnt && */
           ice_st->ice->clist.state == PJ_ICE_SESS_CHECKLIST_ST_RUNNING && !pj_ice_strans_sess_is_complete(ice_st);
}
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_get_ufrag_pwd(pj_ice_strans *ice_st, pj_str_t *loc_ufrag, pj_str_t *loc_pwd, pj_str_t *rem_ufrag,
                            pj_str_t *rem_pwd)
{
 ...
 */
PJ_DEF(unsigned) pj_ice_strans_get_cands_count(pj_ice_strans *ice_st, unsigned comp_id)
{
    unsigned i, cnt;

    PJ_ASSERT_RETURN(ice_st && ice_st->ice && comp_id && comp_id <= ice_st->comp_cnt, 0);
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_enum_cands(pj_ice_strans *ice_st, unsigned comp_id, unsigned *count, pj_ice_sess_cand cand[])
{
    unsigned i, cnt;

    PJ_ASSERT_RETURN(ice_st && ice_st->ice && comp_id && comp_id <= ice_st->comp_cnt && count && cand, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t) pj_ice_strans_get_def_cand(pj_ice_strans *ice_st, unsigned comp_id, pj_ice_sess_cand *cand)
{
    const pj_ice_sess_check *valid_pair;

    PJ_ASSERT_RETURN(ice_st && comp_id && comp_id <= ice_st->comp_cnt && cand, PJ_EINVAL);
    ...
    if (valid_pair) {
        ...
        pj_memcpy(cand, &comp->cand_list[comp->default_cand], sizeof(pj_ice_sess_cand));
    }
    return PJ_SUCCESS;
}
 ...
 * Change session role.
 */
PJ_DEF(pj_status_t) pj_ice_strans_change_role(pj_ice_strans *ice_st, pj_ice_sess_role new_role)
{
    PJ_ASSERT_RETURN(ice_st && ice_st->ice, PJ_EINVALIDOP);
    ...
    for (n = 0; n < ice_st->cfg.turn_tp_cnt; ++n) {
        ...
        for (i = 0; i < comp_cnt; ++i) {
            ...
            if (count && !comp->turn[n].err_cnt && comp->turn[n].sock) {
                status = pj_turn_sock_set_perm(comp->turn[n].sock, count, addrs, PJ_ICE_ST_USE_TURN_PERMANENT_PERM);
                if (status != PJ_SUCCESS) {
                    pj_ice_strans_stop_ice(ice_st);
                    return status;
                }
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_start_ice(pj_ice_strans *ice_st, const pj_str_t *rem_ufrag, const pj_str_t *rem_passwd,
                        unsigned rem_cand_cnt, const pj_ice_sess_cand rem_cand[])
{
    ...
    /* Update check list */
    status = pj_ice_strans_update_check_list(ice_st, rem_ufrag, rem_passwd, rem_cand_cnt, rem_cand,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_update_check_list(pj_ice_strans *ice_st, const pj_str_t *rem_ufrag, const pj_str_t *rem_passwd,
                                unsigned rem_cand_cnt, const pj_ice_sess_cand rem_cand[], pj_bool_t rcand_end)
{
    pj_bool_t checklist_created;
    pj_status_t status;

    PJ_ASSERT_RETURN(ice_st && ((rem_cand_cnt == 0) || (rem_ufrag && rem_passwd && rem_cand)), PJ_EINVAL);
    PJ_ASSERT_RETURN(ice_st->ice, PJ_EINVALIDOP);
    ...
    if (rem_ufrag && !checklist_created) {
        ...
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (ice_st->obj_name, status, "Failed setting up remote ufrag"));
            pj_grp_lock_release(ice_st->grp_lock);
            return status;
        }
    }
    ...
    if (ice_st->ice->is_trickling) {
        ...
        status =
            pj_ice_sess_update_check_list(ice_st->ice, rem_ufrag, rem_passwd, (checklist_created ? rem_cand_cnt : 0),
                                          rem_cand, (ice_st->rem_cand_end && ice_st->loc_cand_end));
        if (status != PJ_SUCCESS) {
    ...
    if (pj_ice_strans_sess_is_running(ice_st)) {
        status = setup_turn_perm(ice_st);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (ice_st->obj_name, status, "Failed setting up TURN permission"));
            pj_grp_lock_release(ice_st->grp_lock);
            return status;
        }
    }
...
static pj_status_t use_buffer(pj_ice_strans *ice_st, unsigned comp_id, const void *data, pj_size_t data_len,
                              const pj_sockaddr_t *dst_addr, int dst_addr_len, void **buffer)
{
    unsigned idx;
    pj_status_t status;
 ...
 */
static pj_status_t send_data(pj_ice_strans *ice_st, unsigned comp_id, const void *data, pj_size_t data_len,
                             const pj_sockaddr_t *dst_addr, int dst_addr_len, pj_bool_t use_buf, pj_bool_t call_cb)
{
    pj_ice_strans_comp *comp;
    ...
    PJ_ASSERT_RETURN(ice_st && comp_id && comp_id <= ice_st->comp_cnt && dst_addr && dst_addr_len, PJ_EINVAL);
    ...
    if (use_buf && ice_st->num_buf > 0) {
        status = use_buffer(ice_st, comp_id, data, data_len, dst_addr, dst_addr_len, &buf);
    ...
    if (def_cand->status == PJ_SUCCESS) {
        ...
        if (def_cand->type == PJ_ICE_CAND_TYPE_RELAYED) {
            ...
            enum { msg_disable_ind = 0xFFFF & ~(PJ_STUN_SESS_LOG_TX_IND | PJ_STUN_SESS_LOG_RX_IND) };
            ...
            status = pj_turn_sock_sendto(comp->turn[tp_idx].sock, (const pj_uint8_t *)buf, (unsigned)data_len, dst_addr,
                                         dst_addr_len);
            goto on_return;
        } else {
            ...
            if (comp->ipv4_mapped) {
                if (comp->synth_addr_len == 0 || pj_sockaddr_cmp(&comp->dst_addr, dst_addr) != 0) {
                    status = pj_sockaddr_synthesize(pj_AF_INET6(), &comp->synth_addr, dst_addr);
                    if (status != PJ_SUCCESS)
                        goto on_return;
            ...
            status = pj_stun_sock_sendto(comp->stun[tp_idx].sock, NULL, buf, (unsigned)data_len, 0, dest_addr,
                                         dest_addr_len);
            goto on_return;
        }
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_sendto(pj_ice_strans *ice_st, unsigned comp_id, const void *data, pj_size_t data_len,
                     const pj_sockaddr_t *dst_addr, int dst_addr_len)
{
    ...
    status = send_data(ice_st, comp_id, data, data_len, dst_addr, dst_addr_len, PJ_TRUE, PJ_FALSE);
    if (status == PJ_EPENDING)
        status = PJ_SUCCESS;
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_sendto2(pj_ice_strans *ice_st, unsigned comp_id, const void *data, pj_size_t data_len,
                      const pj_sockaddr_t *dst_addr, int dst_addr_len)
{
    ice_st->call_send_cb = PJ_TRUE;
    return send_data(ice_st, comp_id, data, data_len, dst_addr, dst_addr_len, PJ_TRUE, PJ_FALSE);
}
...
static void on_valid_pair(pj_ice_sess *ice)
{
    ...
    if (cb.on_valid_pair) {
        ...
        PJ_LOG(4, (ice_st->obj_name, "First ICE candidate nominated in %ds:%03d", msec / 1000, msec % 1000));
        ...
        for (i = 0; i < ice_st->comp_cnt; ++i) {
            ...
            if (check) {
                ...
                if (tp_typ == TP_TURN) {
                     ...
                     */
                    status = pj_turn_sock_bind_channel(comp->turn[tp_idx].sock, &check->rcand->addr,
...
static void on_ice_complete(pj_ice_sess *ice, pj_status_t status)
{
    ...
    if (cb.on_ice_complete) {
        if (status != PJ_SUCCESS) {
            PJ_PERROR(4, (ice_st->obj_name, status, "ICE negotiation failed after %ds:%03d", msec / 1000, msec % 1000));
        } else {
            unsigned i;
            enum { msg_disable_ind = 0xFFFF & ~(PJ_STUN_SESS_LOG_TX_IND | PJ_STUN_SESS_LOG_RX_IND) };

            PJ_LOG(4, (ice_st->obj_name, "ICE negotiation success after %ds:%03d", msec / 1000, msec % 1000));
                              ...
                              pj_size_t size, const pj_sockaddr_t *dst_addr, unsigned dst_addr_len)
{
    ...
    pj_grp_lock_acquire(ice_st->grp_lock);
    if (ice_st->num_buf > 0 && (!ice_st->send_buf || ice_st->send_buf[ice_st->buf_idx].buffer != pkt)) {
        use_buf = PJ_TRUE;
        status = use_buffer(ice_st, comp_id, pkt, size, dst_addr, dst_addr_len, &buf);
        if (status == PJ_EPENDING || status != PJ_SUCCESS) {
            pj_grp_lock_release(ice_st->grp_lock);
            return status;
        }
    ...
    TRACE_PKT((comp->ice_st->obj_name, "Component %d TX packet to %s:%d with transport %d", comp_id,
               pj_sockaddr_print(dst_addr, daddr, sizeof(addr), 2), pj_sockaddr_get_port(dst_addr), tp_typ));

    if (tp_typ == TP_TURN) {
        if (comp->turn[tp_idx].sock) {
            status = pj_turn_sock_sendto(comp->turn[tp_idx].sock, (const pj_uint8_t *)buf, (unsigned)size, dst_addr,
                                         dst_addr_len);
        } else {
            status = PJ_EINVALIDOP;
        }
        ...
        status = pj_stun_sock_sendto(comp->stun[tp_idx].sock, NULL, buf, (unsigned)size, 0, dest_addr, dest_addr_len);
    } else {
                        ...
                        const pj_sockaddr_t *src_addr, unsigned src_addr_len)
{
    ...
    if (ice_st->cb.on_rx_data) {
        (*ice_st->cb.on_rx_data)(ice_st, comp_id, pkt, size, src_addr, src_addr_len);
    }
}
...
static void check_pending_send(pj_ice_strans *ice_st)
{
    ...
    if (ice_st->num_buf > 0 && ice_st->buf_idx != ice_st->empty_idx) {
        ...
        pj_grp_lock_release(ice_st->grp_lock);
        send_data(ice_st, ps->comp_id, ps->buffer, ps->data_len, &ps->dst_addr, ps->dst_addr_len, PJ_FALSE, PJ_TRUE);
    } else {
        ice_st->is_pending = PJ_FALSE;
        pj_grp_lock_release(ice_st->grp_lock);
    }
                                 ...
                                 unsigned addr_len)
{
    ...
    if (ice_st->ice == NULL) {
         ...
         */
        if (ice_st->cb.on_rx_data) {
            (*ice_st->cb.on_rx_data)(ice_st, comp->comp_id, pkt, pkt_len, src_addr, addr_len);
        }

    } else {

        /* Hand over the packet to ICE session */
        status = pj_ice_sess_on_rx_pkt(comp->ice_st->ice, comp->comp_id, data->transport_id, pkt, pkt_len, src_addr,
                                       addr_len);
...
static pj_bool_t stun_on_status(pj_stun_sock *stun_sock, pj_stun_sock_op op, pj_status_t status)
{
    ...
    case PJ_STUN_SOCK_DNS_OP:
        if (status != PJ_SUCCESS) {
            ...
            if (!ice_st->cfg.stun_tp[tp_idx].ignore_stun_error) {
                sess_fail(ice_st, PJ_ICE_STRANS_OP_INIT, "DNS resolution failed", status);
            } else {
                PJ_LOG(4, (ice_st->obj_name, "STUN error is ignored for comp %d", comp->comp_id));
            }
        }
        break;
    ...
    case PJ_STUN_SOCK_MAPPED_ADDR_CHANGE:
        if (status == PJ_SUCCESS) {
            ...
            if (status == PJ_SUCCESS) {
                ...
                const char *op_name =
                    (op == PJ_STUN_SOCK_BINDING_OP) ? "Binding discovery complete" : "srflx address changed";
                ...
                if (info.mapped_addr.addr.sa_family == pj_AF_INET() &&
                    ...
                    for (i = 0; i < comp->cand_cnt; ++i) {
                        ...
                        a2 = &cand->base_addr;
                        if (pj_memcmp(pj_sockaddr_get_addr(a1), pj_sockaddr_get_addr(a2),
                ...
                if (dup) {
                    ...
                    pj_array_erase(comp->cand_list, sizeof(comp->cand_list[0]), comp->cand_cnt, idx);
                    --comp->cand_cnt;
                } else {
                    ...
                    if (pj_ice_strans_has_sess(ice_st)) {
                        status =
                            pj_ice_sess_add_cand(ice_st->ice, comp->comp_id, cand->transport_id, cand->type,
                                                 cand->local_pref, &cand->foundation, &cand->addr, &cand->base_addr,
                                                 &cand->rel_addr, pj_sockaddr_get_len(&cand->addr), NULL);
                    }
                }
                ...
                PJ_LOG(4, (comp->ice_st->obj_name,
                           "Comp %d: %s, "
                           "srflx address is %s",
                           comp->comp_id, op_name, pj_sockaddr_print(&info.mapped_addr, ipaddr, sizeof(ipaddr), 3)));

                sess_init_update(ice_st);

                /* Invoke on_new_candidate() callback */
                init_done = (ice_st->state == PJ_ICE_STRANS_STATE_READY);
                if (op == PJ_STUN_SOCK_BINDING_OP && status == PJ_SUCCESS && ice_st->cb.on_new_candidate &&
                    (!dup || init_done)) {
                    (*ice_st->cb.on_new_candidate)(ice_st, (dup ? NULL : cand), init_done);
                }

                if (op == PJ_STUN_SOCK_MAPPED_ADDR_CHANGE && ice_st->cb.on_ice_complete) {
                    (*ice_st->cb.on_ice_complete)(ice_st, PJ_ICE_STRANS_OP_ADDR_CHANGE, status);
                }
            }
        }
        ...
        if (status != PJ_SUCCESS) {
            ...
            if (cand)
                cand->status = status;
            if (!ice_st->cfg.stun_tp[tp_idx].ignore_stun_error || comp->cand_cnt == 1) {
                sess_fail(ice_st, PJ_ICE_STRANS_OP_INIT, "STUN binding request failed", status);
            } else {
                pj_bool_t init_done;
                ...
                /* Invoke on_new_candidate() callback */
                init_done = (ice_st->state == PJ_ICE_STRANS_STATE_READY);
                if (op == PJ_STUN_SOCK_BINDING_OP && ice_st->cb.on_new_candidate && init_done) {
                    (*ice_st->cb.on_new_candidate)(ice_st, NULL, PJ_TRUE);
                }
            }
        }
    ...
    case PJ_STUN_SOCK_KEEP_ALIVE_OP:
        if (status != PJ_SUCCESS) {
            ...
            cand->status = status;
            if (!ice_st->cfg.stun_tp[tp_idx].ignore_stun_error) {
                sess_fail(ice_st, PJ_ICE_STRANS_OP_INIT, "STUN keep-alive failed", status);
            } else {
                            ...
                            unsigned addr_len)
{
    ...
    if (comp->ice_st->ice == NULL) {
        ...
        if (comp->ice_st->cb.on_rx_data) {
            (*comp->ice_st->cb.on_rx_data)(comp->ice_st, comp->comp_id, pkt, pkt_len, peer_addr, addr_len);
        }

    } else {

        /* Hand over the packet to ICE */
        status = pj_ice_sess_on_rx_pkt(comp->ice_st->ice, comp->comp_id, data->transport_id, pkt, pkt_len, peer_addr,
                                       addr_len);

        if (status != PJ_SUCCESS) {
            ice_st_perror(comp->ice_st, "Error processing packet from TURN relay", status);
        }
    }
...
static void turn_on_state(pj_turn_sock *turn_sock, pj_turn_state_t old_state, pj_turn_state_t new_state)
{
    ...
    PJ_LOG(5, (comp->ice_st->obj_name, "TURN client state changed %s --> %s", pj_turn_state_name(old_state),
    ...
    if (new_state == PJ_TURN_STATE_READY) {
        ...
        pj_ice_calc_foundation(comp->ice_st->pool, &cand->foundation, PJ_ICE_CAND_TYPE_RELAYED, &rel_info.relay_addr);
        cand->status = PJ_SUCCESS;
        ...
        */
        ...
        PJ_LOG(4, (comp->ice_st->obj_name,
                   ...
                   pj_sockaddr_print(&rel_info.relay_addr, ipaddr, sizeof(ipaddr), 3)));

        /* For trickle ICE, add the candidate to ICE session and setup TURN
         * permission for remote candidates.
         */
        if (comp->ice_st->cfg.opt.trickle != PJ_ICE_SESS_TRICKLE_DISABLED && pj_ice_strans_has_sess(comp->ice_st)) {
            pj_sockaddr addrs[PJ_ICE_ST_MAX_CAND];
            ...
            /* Add the candidate */
            status = pj_ice_sess_add_cand(comp->ice_st->ice, comp->comp_id, cand->transport_id, cand->type,
                                          cand->local_pref, &cand->foundation, &cand->addr, &cand->base_addr,
                                          &cand->rel_addr, pj_sockaddr_get_len(&cand->addr), NULL);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(4, (comp->ice_st->obj_name, status, "Comp %d/%d: failed to add TURN (tpid=%d) to ICE",
                              comp->comp_id, cand_idx, cand->transport_id));
                sess_fail(comp->ice_st, PJ_ICE_STRANS_OP_INIT, "adding TURN candidate failed", status);
            }

            /* Gather remote addresses for this component */
            for (j = 0; j < sess->rcand_cnt && count < PJ_ARRAY_SIZE(addrs); ++j) {
                if (sess->rcand[j].addr.addr.sa_family == rel_info.relay_addr.addr.sa_family) {
            ...
            if (count) {
                ...
                if (status != PJ_SUCCESS) {
                    PJ_PERROR(4, (comp->ice_st->obj_name, status, "Comp %d/%d: TURN set perm (tpid=%d) failed",
                                  comp->comp_id, cand_idx, cand->transport_id));
                    sess_fail(comp->ice_st, PJ_ICE_STRANS_OP_INIT, "TURN set permission failed", status);
                }
            }
        }
    ...
    } else if ((old_state == PJ_TURN_STATE_RESOLVING || old_state == PJ_TURN_STATE_RESOLVED ||
        ...
        /* Invoke on_new_candidate() callback */
        if (comp->ice_st->cb.on_new_candidate && comp->ice_st->state == PJ_ICE_STRANS_STATE_READY) {
        ...
        if (info.last_status != PJ_SUCCESS) {
            if (comp->ice_st->state < PJ_ICE_STRANS_STATE_READY) {
                sess_fail(comp->ice_st, PJ_ICE_STRANS_OP_INIT, "TURN allocation failed", info.last_status);
            } else if (comp->turn[tp_idx].err_cnt > 1) {
                sess_fail(comp->ice_st, PJ_ICE_STRANS_OP_KEEP_ALIVE, "TURN refresh failed", info.last_status);
            } else {
                PJ_PERROR(4, (comp->ice_st->obj_name, info.last_status, "Comp %d: TURN allocation failed, retrying",
                              comp->comp_id));
                add_update_turn(comp->ice_st, comp, tp_idx, PJ_ICE_ST_MAX_CAND - comp->cand_cnt);
            }
        }
    }
