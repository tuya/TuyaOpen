 ...
 * Callback prototypes.
 */
static pj_status_t turn_on_send_pkt(pj_turn_session *sess, const pj_uint8_t *pkt, unsigned pkt_len,
                                    const pj_sockaddr_t *dst_addr, unsigned dst_addr_len);
static pj_status_t turn_on_stun_send_pkt(pj_turn_session *sess, const pj_uint8_t *pkt, unsigned pkt_len,
                                         const pj_sockaddr_t *dst_addr, unsigned dst_addr_len);
static void turn_on_channel_bound(pj_turn_session *sess, const pj_sockaddr_t *peer_addr, unsigned addr_len,
                                  unsigned ch_num);
static void turn_on_rx_data(pj_turn_session *sess, void *pkt, unsigned pkt_len, const pj_sockaddr_t *peer_addr,
                            unsigned addr_len);
static void turn_on_state(pj_turn_session *sess, pj_turn_state_t old_state, pj_turn_state_t new_state);
static void turn_on_connection_attempt(pj_turn_session *sess, pj_uint32_t conn_id, const pj_sockaddr_t *peer_addr,
                                       unsigned addr_len);
static void turn_on_connection_bind_status(pj_turn_session *sess, pj_status_t status, pj_uint32_t conn_id,
                                           const pj_sockaddr_t *peer_addr, unsigned addr_len);
static void turn_on_connect_complete(pj_turn_session *sess, pj_status_t status, pj_uint32_t conn_id,
                                     const pj_sockaddr_t *peer_addr, unsigned addr_len);

static pj_bool_t on_data_read(pj_turn_sock *turn_sock, void *data, pj_size_t size, pj_status_t status,
                              pj_size_t *remainder);
static pj_bool_t on_data_sent(pj_turn_sock *turn_sock, pj_ioqueue_op_key_t *send_key, pj_ssize_t sent);
static pj_bool_t on_connect_complete(pj_turn_sock *turn_sock, pj_status_t status);

/*
 * Activesock callback
 */
static pj_bool_t on_connect_complete_asock(pj_activesock_t *asock, pj_status_t status);
static pj_bool_t on_data_read_asock(pj_activesock_t *asock, void *data, pj_size_t size, pj_status_t status,
                                    pj_size_t *remainder);
static pj_bool_t on_data_sent_asock(pj_activesock_t *asock, pj_ioqueue_op_key_t *send_key, pj_ssize_t sent);

/*
 * SSL sock callback
 */
#if PJ_HAS_SSL_SOCK
static pj_bool_t on_connect_complete_ssl_sock(pj_ssl_sock_t *ssl_sock, pj_status_t status);
static pj_bool_t on_data_read_ssl_sock(pj_ssl_sock_t *ssl_sock, void *data, pj_size_t size, pj_status_t status,
                                       pj_size_t *remainder);
#endif

static pj_bool_t dataconn_on_data_read(pj_activesock_t *asock, void *data, pj_size_t size, pj_status_t status,
                                       pj_size_t *remainder);
static pj_bool_t dataconn_on_data_sent(pj_activesock_t *asock, pj_ioqueue_op_key_t *send_key, pj_ssize_t sent);
static pj_bool_t dataconn_on_connect_complete(pj_activesock_t *asock, pj_status_t status);
...
PJ_DEF(void) pj_turn_sock_tls_cfg_dup(pj_pool_t *pool, pj_turn_sock_tls_cfg *dst, const pj_turn_sock_tls_cfg *src)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_sock_create(pj_stun_config *cfg, int af, pj_turn_tp_type conn_type, const pj_turn_sock_cb *cb,
                    const pj_turn_sock_cfg *setting, void *user_data, pj_turn_sock **p_turn_sock)
{
    ...
    pool = pj_pool_create(cfg->pf, name_tmpl, PJNATH_POOL_LEN_TURN_SOCK, PJNATH_POOL_INC_TURN_SOCK, NULL);
...
#if PJ_HAS_SSL_SOCK
    pj_turn_sock_tls_cfg_dup(turn_sock->pool, &turn_sock->setting.tls_cfg, &setting->tls_cfg);
#endif
    ...
    pj_grp_lock_add_handler(turn_sock->grp_lock, pool, turn_sock, &turn_sock_on_destroy);
    ...
    status = pj_turn_session_create(cfg, pool->obj_name, af, conn_type, turn_sock->grp_lock, &sess_cb, 0, turn_sock,
                                    &turn_sock->sess);
    if (status != PJ_SUCCESS) {
        destroy(turn_sock);
        return status;
...
static void destroy(pj_turn_sock *turn_sock)
{
    ...
    PJ_LOG(4,
           (turn_sock->obj_name, "TURN socket destroy request, ref_cnt=%d", pj_grp_lock_get_ref(turn_sock->grp_lock)));
...
/* Display error */
static void show_err(pj_turn_sock *turn_sock, const char *title, pj_status_t status)
{
 ...
 * Set user data.
 */
PJ_DEF(pj_status_t) pj_turn_sock_set_user_data(pj_turn_sock *turn_sock, void *user_data)
{
 ...
 * Get info.
 */
PJ_DEF(pj_status_t) pj_turn_sock_get_info(pj_turn_sock *turn_sock, pj_turn_session_info *info)
{
 ...
 * Set software name
 */
PJ_DEF(pj_status_t) pj_turn_sock_set_software_name(pj_turn_sock *turn_sock, const pj_str_t *sw)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_sock_alloc(pj_turn_sock *turn_sock, const pj_str_t *domain, int default_port, pj_dns_resolver *resolver,
                   const pj_stun_auth_cred *cred, const pj_turn_alloc_param *param)
{
    pj_status_t status;
    ...
    /* Resolve server */
    status = pj_turn_session_set_server(turn_sock->sess, domain, default_port, resolver);
    if (status != PJ_SUCCESS) {
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_sock_set_perm(pj_turn_sock *turn_sock, unsigned addr_cnt, const pj_sockaddr addr[], unsigned options)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_turn_sock_sendto(pj_turn_sock *turn_sock, const pj_uint8_t *pkt, unsigned pkt_len, const pj_sockaddr_t *addr,
                    unsigned addr_len)
{
 ...
 */
PJ_DEF(pj_status_t) pj_turn_sock_bind_channel(pj_turn_sock *turn_sock, const pj_sockaddr_t *peer, unsigned addr_len)
{
 ...
 */
PJ_DEF(pj_status_t) pj_turn_sock_connect(pj_turn_sock *turn_sock, const pj_sockaddr_t *peer_addr, unsigned addr_len)
{
 ...
 */
PJ_DEF(pj_status_t) pj_turn_sock_disconnect(pj_turn_sock *turn_sock, const pj_sockaddr_t *peer_addr, unsigned addr_len)
...
static pj_bool_t on_connect_complete(pj_turn_sock *turn_sock, pj_status_t status)
{
    ...
    if (turn_sock->conn_type != PJ_TURN_TP_UDP) {
        PJ_LOG(5, (turn_sock->obj_name, "%s connected", turn_sock->conn_type == PJ_TURN_TP_TCP ? "TCP" : "TLS"));
    }

    /* Kick start pending read operation */
    if (turn_sock->conn_type != PJ_TURN_TP_TLS)
        status = pj_activesock_start_read(turn_sock->active_sock, turn_sock->pool, turn_sock->setting.max_pkt_size, 0);
#if PJ_HAS_SSL_SOCK
    else
    ...
    pj_ioqueue_op_key_init(&turn_sock->int_send_key, sizeof(turn_sock->int_send_key));
...
static unsigned has_packet(pj_turn_sock *turn_sock, const void *buf, pj_size_t bufsize)
{
     ...
     */
    is_stun = ((((pj_uint8_t *)buf)[0] & 0xC0) == 0) && ((GETVAL16H((const pj_uint8_t *)buf, 2) & 0x03) == 0);
...
static pj_bool_t on_data_sent_ssl_sock(pj_ssl_sock_t *ssl_sock, pj_ioqueue_op_key_t *op_key, pj_ssize_t bytes_sent)
{
    pj_turn_sock *turn_sock;
    ...
    if (bytes_sent <= 0) {
        ...
        status = (bytes_sent == 0) ? PJ_RETURN_OS_ERROR(OSERR_ENOTCONN) : (pj_status_t)-bytes_sent;
...
static pj_status_t send_pkt(pj_turn_session *sess, pj_bool_t internal, const pj_uint8_t *pkt, unsigned pkt_len,
                            const pj_sockaddr_t *dst_addr, unsigned dst_addr_len)
{
    ...
    if (turn_sock->conn_type == PJ_TURN_TP_UDP) {
        status = pj_activesock_sendto(turn_sock->active_sock, send_key, pkt, &len, 0, dst_addr, dst_addr_len);
        ...
        if (pj_sockaddr_cmp(&info.server, dst_addr) == 0) {
            ...
            status = pj_activesock_send(turn_sock->active_sock, send_key, pkt, &len, 0);
        } else {
            ...
            for (i = 0; i < PJ_TURN_MAX_TCP_CONN_CNT; ++i) {
                ...
                if (pj_sockaddr_cmp(&conn->peer_addr, dst_addr) == 0) {
                    status = pj_activesock_send(conn->asock, &conn->send_key, pkt, &len, 0);
                    break;
                }
            }
        }
                            ...
                            unsigned addr_len)
{
    ...
    if (turn_sock->cb.on_rx_data) {
        (*turn_sock->cb.on_rx_data)(turn_sock, pkt, pkt_len, peer_addr, addr_len);
    }
}
...
static void turn_on_state(pj_turn_session *sess, pj_turn_state_t old_state, pj_turn_state_t new_state)
{
    ...
    if (turn_sock->sess && new_state == PJ_TURN_STATE_RESOLVED) {
         ...
         */
        if ((turn_sock->conn_type != PJ_TURN_TP_TLS) && (turn_sock->active_sock)) {
            pj_activesock_close(turn_sock->active_sock);
            turn_sock->active_sock = NULL;
        }
#if PJ_HAS_SSL_SOCK
        else if ((turn_sock->conn_type == PJ_TURN_TP_TLS) && (turn_sock->ssl_sock)) {
            pj_ssl_sock_close(turn_sock->ssl_sock);
        ...
        if (turn_sock->setting.port_range && turn_sock->setting.port_range < max_bind_retry) {
            max_bind_retry = turn_sock->setting.port_range;
        }
        pj_sockaddr_init(turn_sock->af, &bound_addr, NULL, 0);
        if (cfg_bind_addr->addr.sa_family == pj_AF_INET() || cfg_bind_addr->addr.sa_family == pj_AF_INET6()) {
        ...
        if (turn_sock->conn_type != PJ_TURN_TP_TLS) {
            ...
            /* Bind socket */
            status = pj_sock_bind_random(sock, &bound_addr, turn_sock->setting.port_range, max_bind_retry);
            if (status != PJ_SUCCESS) {
                pj_sock_close(sock);
                pj_turn_sock_destroy(turn_sock);
                return;
            ...
            status = pj_sock_apply_qos2(sock, turn_sock->setting.qos_type, &turn_sock->setting.qos_params,
                                        (turn_sock->setting.qos_ignore_error ? 2 : 1), turn_sock->pool->obj_name, NULL);
            ...
            if (turn_sock->setting.so_rcvbuf_size > 0) {
                ...
                status = pj_sock_setsockopt_sobuf(sock, pj_SO_RCVBUF(), PJ_TRUE, &sobuf_size);
                if (status != PJ_SUCCESS) {
                    pj_perror(3, turn_sock->obj_name, status, "Failed setting SO_RCVBUF");
                } else {
                    if (sobuf_size < turn_sock->setting.so_rcvbuf_size) {
                        ...
                        PJ_LOG(5, (turn_sock->obj_name, "SO_RCVBUF set to %d", sobuf_size));
                    }
                }
            }
            if (turn_sock->setting.so_sndbuf_size > 0) {
                unsigned sobuf_size = turn_sock->setting.so_sndbuf_size;
                status = pj_sock_setsockopt_sobuf(sock, pj_SO_SNDBUF(), PJ_TRUE, &sobuf_size);
                if (status != PJ_SUCCESS) {
                    pj_perror(3, turn_sock->obj_name, status, "Failed setting SO_SNDBUF");
                } else {
                    if (sobuf_size < turn_sock->setting.so_sndbuf_size) {
                        ...
                        PJ_LOG(5, (turn_sock->obj_name, "SO_SNDBUF set to %d", sobuf_size));
                    }
                }
            }
            ...
            status = pj_activesock_create(turn_sock->pool, sock, sock_type, &asock_cfg, turn_sock->cfg.ioqueue,
                                          &asock_cb, turn_sock, &turn_sock->active_sock);
            if (status != PJ_SUCCESS)
                pj_sock_close(sock);
        }
#if PJ_HAS_SSL_SOCK
            ...
            pj_memcpy(&param.qos_params, &turn_sock->setting.qos_params, sizeof(param.qos_params));

            if (turn_sock->setting.tls_cfg.cert_file.slen || turn_sock->setting.tls_cfg.ca_list_file.slen ||
                turn_sock->setting.tls_cfg.ca_list_path.slen || turn_sock->setting.tls_cfg.privkey_file.slen) {
                status = pj_ssl_cert_load_from_files2(
                    turn_sock->pool, &turn_sock->setting.tls_cfg.ca_list_file, &turn_sock->setting.tls_cfg.ca_list_path,
                    &turn_sock->setting.tls_cfg.cert_file, &turn_sock->setting.tls_cfg.privkey_file,
                    &turn_sock->setting.tls_cfg.password, &turn_sock->cert);

            } else if (turn_sock->setting.tls_cfg.ca_buf.slen || turn_sock->setting.tls_cfg.cert_buf.slen ||
                       turn_sock->setting.tls_cfg.privkey_buf.slen) {
                status = pj_ssl_cert_load_from_buffer(
                    turn_sock->pool, &turn_sock->setting.tls_cfg.ca_buf, &turn_sock->setting.tls_cfg.cert_buf,
                    &turn_sock->setting.tls_cfg.privkey_buf, &turn_sock->setting.tls_cfg.password, &turn_sock->cert);
            }
            if (status != PJ_SUCCESS) {
                pj_turn_sock_destroy(turn_sock);
                return;
            ...
            status = pj_ssl_sock_create(turn_sock->pool, &param, &turn_sock->ssl_sock);
            ...
            if (turn_sock->cert) {
                status = pj_ssl_sock_set_certificate(turn_sock->ssl_sock, turn_sock->pool, turn_sock->cert);
        ...
        PJ_LOG(5, (turn_sock->pool->obj_name, "Connecting to %s",
        ...
        if (turn_sock->conn_type == PJ_TURN_TP_UDP) {
            ...
            status = pj_activesock_start_connect(turn_sock->active_sock, turn_sock->pool, &info.server,
            ...
            status = pj_ssl_sock_start_connect2(turn_sock->ssl_sock, &connect_param);
        }
#endif
        if (status == PJ_SUCCESS) {
            on_connect_complete(turn_sock, PJ_SUCCESS);
        } else if (status != PJ_EPENDING) {
            PJ_PERROR(3, (turn_sock->pool->obj_name, status, "Failed to connect to %s",
                          pj_sockaddr_print(&info.server, addrtxt, sizeof(addrtxt), 3)));
            pj_turn_sock_destroy(turn_sock);
            return;
        }
    ...
    if (new_state >= PJ_TURN_STATE_DESTROYING && turn_sock->sess) {
        ...
        pj_timer_heap_cancel_if_active(turn_sock->cfg.timer_heap, &turn_sock->timer, 0);
        pj_timer_heap_schedule_w_grp_lock(turn_sock->cfg.timer_heap, &turn_sock->timer, &delay, TIMER_DESTROY,
                                          turn_sock->grp_lock);
    }
}
...
static pj_bool_t dataconn_on_data_read(pj_activesock_t *asock, void *data, pj_size_t size, pj_status_t status,
                                       pj_size_t *remainder)
{
    tcp_data_conn_t *conn = (tcp_data_conn_t *)pj_activesock_get_user_data(asock);
    ...
    *remainder = size;
    while (*remainder > 0) {
        if (conn->state == DATACONN_STATE_READY) {
            /* Application data */
            if (turn_sock->cb.on_rx_data) {
                (*turn_sock->cb.on_rx_data)(turn_sock, data, (unsigned)*remainder, &conn->peer_addr,
                                            conn->peer_addr_len);
            }
            *remainder = 0;
...
static pj_bool_t dataconn_on_connect_complete(pj_activesock_t *asock, pj_status_t status)
{
    ...
    if (status == PJ_SUCCESS) {
        status = pj_activesock_start_read(asock, turn_sock->pool, turn_sock->setting.max_pkt_size, 0);
    }
    if (status == PJ_SUCCESS) {
        conn->state = DATACONN_STATE_CONN_BINDING;
        status = pj_turn_session_connection_bind(turn_sock->sess, conn->pool, conn->id, &conn->peer_addr,
                                                 conn->peer_addr_len);
    }
    if (status != PJ_SUCCESS) {
        dataconn_cleanup(conn);
                                       ...
                                       unsigned addr_len)
{
    ...
    PJ_ASSERT_ON_FAIL(turn_sock->conn_type == PJ_TURN_TP_TCP && turn_sock->alloc_param.peer_conn_type == PJ_TURN_TP_TCP,
                      return );
    ...
    if (turn_sock->cb.on_connection_attempt) {
        status = (*turn_sock->cb.on_connection_attempt)(turn_sock, conn_id, peer_addr, addr_len);
    }
    /* App rejects it */
    if (status != PJ_SUCCESS) {
        pj_perror(4, turn_sock->pool->obj_name, status, "Rejected connection attempt from peer %s",
                  pj_sockaddr_print(peer_addr, addrtxt, sizeof(addrtxt), 3));
        pj_grp_lock_release(turn_sock->grp_lock);
        return;
    }
    ...
    status = pj_activesock_create(pool, sock, pj_SOCK_STREAM(), &asock_cfg, turn_sock->cfg.ioqueue, &asock_cb, new_conn,
                                  &new_conn->asock);
    if (status != PJ_SUCCESS)
        goto on_return;
    ...
    pj_turn_session_get_info(turn_sock->sess, &info);
    status = pj_activesock_start_connect(new_conn->asock, pool, &info.server, pj_sockaddr_get_len(&info.server));
    if (status == PJ_SUCCESS) {
...
on_return:
    if (status == PJ_EPENDING) {
        ...
        /* not PJ_SUCCESS */
        pj_perror(4, pool->obj_name, status, "Failed in accepting connection from peer %s",
                  pj_sockaddr_print(peer_addr, addrtxt, sizeof(addrtxt), 3));
        ...
        if (turn_sock->cb.on_connection_status) {
            (*turn_sock->cb.on_connection_status)(turn_sock, status, conn_id, peer_addr, addr_len);
        }
    }
    pj_grp_lock_release(turn_sock->grp_lock);
}
                                           ...
                                           const pj_sockaddr_t *peer_addr, unsigned addr_len)
{
    ...
    if (!conn) {
        PJ_LOG(5, (turn_sock->pool->obj_name, "Warning: stray connection bind event"));
        pj_grp_lock_release(turn_sock->grp_lock);
        return;
    }
                                     ...
                                     const pj_sockaddr_t *peer_addr, unsigned addr_len)
{
    ...
    if (turn_sock->data_conn_cnt == PJ_TURN_MAX_TCP_CONN_CNT) {
        ...
        status = PJ_ETOOMANY;
        pj_perror(4, turn_sock->pool->obj_name, status, "Failed in connect to peer %s",
                  pj_sockaddr_print(peer_addr, addrtxt, sizeof(addrtxt), 3));
...
on_return:
    if (status == PJ_EPENDING) {
        PJ_LOG(5, (pool->obj_name, "Connecting to peer %s", pj_sockaddr_print(peer_addr, addrtxt, sizeof(addrtxt), 3)));
    } else {
        /* not PJ_SUCCESS */
