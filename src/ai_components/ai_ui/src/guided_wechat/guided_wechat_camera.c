/**
 * @file guided_wechat_camera.c
 * @brief WeChat-style camera page 鈥?LVGL preview, shutter, thumbnail and close controls.
...
typedef struct {
    ...
    uint16_t  preview_h;
    uint8_t  *thumb_buf;
} GUIDED_WECHAT_CAMERA_T;

/***********************************************************
***********************variable define**********************
***********************************************************/
static GUIDED_WECHAT_CAMERA_T sg_camera = {0};
...
static void __disp_set_thumbnail_jpeg(uint8_t *jpeg, uint32_t len)
{
    ...
    if (NULL == jpeg || 0 == len) {
        /* No thumbnail available 鈥?show a default image icon */
...
void guided_wechat_camera_init(lv_obj_t *parent)
{
    ...
    /* ---- close button 鈥?top-left corner, inset for rounded screens ---- */
    ...
    /* ---- shutter button 鈥?bottom-center, moved up ---- */
    ...
    /* ---- thumbnail button 鈥?right of shutter, clipped to rounded corners ---- */
    ...
    /* ---- AI vision toggle button 鈥?top-right corner, inset for rounded screens ---- */
 ...
 */
void guided_wechat_camera_register(void)
{
