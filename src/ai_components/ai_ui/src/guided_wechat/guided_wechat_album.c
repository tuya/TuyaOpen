/**
 * @file guided_wechat_album.c
 * @brief WeChat-style album page 鈥?single image view, all thumbnails grid
...
#endif
} GUIDED_WECHAT_ALBUM_T;

/***********************************************************
***********************variable define**********************
***********************************************************/
static GUIDED_WECHAT_ALBUM_T sg_album = {0};

/***********************************************************
***********************function define**********************
***********************************************************/

/* 鈹€鈹€ view page callbacks 鈹€鈹€ */
...
/* 鈹€鈹€ select page callbacks (chat attachment) 鈹€鈹€ */
...
/* 鈹€鈹€ helper: create an image icon button 鈹€鈹€ */
...
{
    (void)e;
    /* Switch overlay to "printing" state 鈥?keep it visible so the user sees feedback */
...
static void __create_view_page(lv_obj_t *parent)
{
     ...
     * lv_canvas_constructor clears CLICKABLE 鈥?touches pass through to view_page,
    ...
    /* Back button 鈥?top-left, inset from corner for rounded screens */
    ...
    /* Delete button 鈥?bottom-left, inset from corner for rounded screens */
    ...
    /* "鎵€鏈夌収鐗?>" button 鈥?bottom-right, text style with semi-transparent bg */
...
#if defined(ENABLE_PRINTER) && (ENABLE_PRINTER == 1)
    /* Print button 鈥?top-right, inset from corner for rounded screens */
    ...
    /* Print confirmation overlay 鈥?full-screen semi-transparent panel */
...
static void __create_all_page(lv_obj_t *parent)
{
    ...
    /* "閫夋嫨" icon button in top-right, inset from corner for rounded screens */
    ...
    /* Scrollable grid container 鈥?fixed position below top bar, never moves */
...
/* 鈹€鈹€ album display callbacks 鈹€鈹€ */
...
static void __disp_image(AI_UI_IMG_T *img)
{
    if (NULL == img || NULL == img->data || 0 == img->len) {
        /* No image to display (album empty after delete) 鈥?show empty state in-place */
...
static void __disp_all_img_thumb_list(AI_UI_IMG_T *item_arr, uint32_t arr_cnt)
{
    ...
    for (i = 0; i < arr_cnt; i++) {
        ...
        /* Circle indicator 鈥?always created, hidden until select mode is entered */
                                         ...
                                         uint8_t select_num_max)
{
    ...
    /* Create thumbnail items 鈥?single-click to select and close */
    uint32_t i;
    for (i = 0; i < arr_cnt; i++) {
        AI_UI_IMG_T *item = &item_arr[i];
 ...
 */
void guided_wechat_album_init(lv_obj_t *parent)
{
    memset(&sg_album, 0, sizeof(GUIDED_WECHAT_ALBUM_T));
 ...
 */
void guided_wechat_album_register(void)
{
