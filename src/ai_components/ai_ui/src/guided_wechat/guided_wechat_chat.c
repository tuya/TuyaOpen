/**
 * @file guided_wechat_chat.c
 * @brief WeChat-style chat page 鈥?message list, streaming, link display, image display.
...
#endif
} GUIDED_WECHAT_CHAT_T;


/* 鈹€鈹€ link click event callback 鈹€鈹€ */
...
***********************variable define**********************
***********************************************************/
static GUIDED_WECHAT_CHAT_T sg_chat = {0};
...
/* 鈹€鈹€ picture view event callbacks 鈹€鈹€ */
...
static void __picture_print_btn_cb(lv_event_t *e)
{
    ...
    if (sg_chat.cur_img_name[0] == '\0') {
        PR_NOTICE("picture: print btn 鈥?no image name, abort");
        return;
    }
...
static void __picture_attach_btn_cb(lv_event_t *e)
{
    ...
    lv_event_stop_bubbling(e);
    if (sg_chat.cur_img_name[0] == '\0') {
        PR_NOTICE("picture: attach btn 鈥?no image name, abort");
        return;
    }
...
/**
 * @brief Click picture canvas 鈥?hide picture view, show chat content.
 */
...
/**
 * @brief Generic link click handler 鈥?invokes the stored callback with argument.
 */
...
/* 鈹€鈹€ "+" button and popup menu callbacks 鈹€鈹€ */
...
/* 鈹€鈹€ chat page callbacks (registered into AI_UI_INTFS_T) 鈹€鈹€ */
...
/**
 * @brief Start streaming AI message 鈥?create empty AI bubble.
 */
...
static void __ui_add_chat_attch_img(AI_UI_IMG_T *img)
{
    ...
    /* Close button 鈥?top-right corner, solid red badge */
...
void guided_wechat_chat_init(lv_obj_t *parent)
{
    ...
    /* Style init 鈥?already inside disp lock, safe to call LVGL style APIs */
    ...
    /* Picture view area 鈥?hidden by default */
    ...
    /* Attach bar 鈥?horizontal row of thumbnails at bottom, hidden by default */
...
#if defined(ENABLE_LVGL_TP) && (ENABLE_LVGL_TP == 1)
    /* "+" button 鈥?bottom-right, offset inward so it's not flush with the corner */
    /* Use the same LVGL button type as the bottom-left menu button so the
     * default pressed-state feedback is consistent across both controls. */
    sg_chat.plus_btn = lv_btn_create(parent);
    ...
    lv_obj_set_style_shadow_color(sg_chat.plus_btn, lv_color_hex(0xB8BCC2), 0);
    lv_obj_set_style_border_width(sg_chat.plus_btn, 1, 0);
    lv_obj_set_style_border_color(sg_chat.plus_btn, lv_color_hex(0xE2E5E9), 0);
    ...
    /* Popup menu 鈥?above "+" button, hidden by default */
