...
void user_event_handler_on(tuya_iot_client_t *client, tuya_event_msg_t *event)
{
    ...
    case TUYA_EVENT_BIND_START:
        ...
        // Start WiFi connecting animation
        weather_display_show_wifi_status();
        break;
    ...
    case TUYA_EVENT_MQTT_CONNECTED:
        ...
        // MQTT connected - notify display to show weather pages
        weather_display_mqtt_connected();
        break;
...
bool user_network_check(void)
{
    ...
    bool is_connected = (status != NETMGR_LINK_DOWN);
...
void user_main(void)
{
    ...
    // Register board hardware (required for pixel display)
    ret = board_register_hardware();
    if (OPRT_OK != ret) {
        PR_ERR("board_register_hardware failed: %d", ret);
    } else {
        PR_NOTICE("Board hardware registered");
    }
    ...
    // Initialize and start weather display
    weather_display_init();
    tal_system_sleep(1000); // Wait a bit for system to stabilize
