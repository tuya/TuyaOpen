...
static int skip_exact(TUYA_FILE f, uint32_t n)
{
    ...
    while (n) {
        ...
        if (read_exact(f, tmp, c) != 0) return -1;
        n -= c;
    }
    return 0;
}
...
{
    if (!ctx || !row) return;
    if (ctx->draw_w <= 0 || ctx->draw_h <= 0) return;

    uint32_t dy_min = (uint32_t)(((uint64_t)sy * (uint64_t)ctx->draw_h + (uint64_t)ctx->src_h - 1u) / (uint64_t)ctx->src_h);
    uint32_t dy_max = (uint32_t)((((uint64_t)(sy + 1u) * (uint64_t)ctx->draw_h + (uint64_t)ctx->src_h - 1u) / (uint64_t)ctx->src_h) - 1u);
    if (dy_min > dy_max) return;
    if (dy_min >= (uint32_t)ctx->draw_h) return;
    if (dy_max >= (uint32_t)ctx->draw_h) dy_max = (uint32_t)ctx->draw_h - 1u;
...
static int row_out_cb(void *user, const uint8_t *data, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        if (ctx->row_index >= ctx->src_h) return -1;
...
{
    uint8_t hdr[8];
    if (read_exact(f, hdr, sizeof(hdr)) != 0) return -1;
    *out_len = be32(&hdr[0]);
...
static int png_stream_decode_draw(const char *path, png_row_ctx_t *ctx)
{
    ...
    while (1) {
        uint32_t clen = 0;
        uint8_t ctype[4];
        if (png_read_chunk_header(f, &clen, ctype) != 0) break;
        ...
        if (memcmp(ctype, "IHDR", 4) == 0) {
            ...
            if (ctx->color_type == 0) ctx->bpp = 1;
            else if (ctx->color_type == 2) ctx->bpp = 3;
            else if (ctx->color_type == 4) ctx->bpp = 2;
            else if (ctx->color_type == 6) ctx->bpp = 4;
            else {
            ...
            fit_aspect((int)ctx->src_w, (int)ctx->src_h, ctx->view_w, ctx->view_h, &ctx->draw_w, &ctx->draw_h, &ctx->off_x, &ctx->off_y);
            ...
            PR_NOTICE("png: IHDR %lux%lu ct=%u bpp=%u", (unsigned long)ctx->src_w, (unsigned long)ctx->src_h, (unsigned)ctx->color_type, (unsigned)ctx->bpp);
...
int png_stream_draw_1bit(const char *path, int x, int y, int w, int h)
{
    ...
    if (ctx.packet) big_free(ctx.packet);
    if (ctx.prev) big_free(ctx.prev);
    if (ctx.cur) big_free(ctx.cur);
