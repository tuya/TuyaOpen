...
int pn53x_transceive(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx,
                     const size_t szRxLen, int timeout)
{
    ...
    if ((CHIP_DATA(pnd)->type == PN532) &&
        (TgInitAsTarget ==
         pbtTx[0])) { // PN532 automatically goes into PowerDown mode when TgInitAsTarget command will be sent
    ...
    if ((CHIP_DATA(pnd)->type == PN532) &&
        (TgInitAsTarget == pbtTx[0])) { // PN532 automatically wakeup on external RF field
        CHIP_DATA(pnd)->power_mode =
            NORMAL; // When TgInitAsTarget reply that means an external RF have waken up the chip
    }
    ...
    if (res < 0) {
        ...
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG,
                "Chip error: \"%s\" (%02x), returned error: \"%s\" (%d))", pn53x_strerror(pnd),
                CHIP_DATA(pnd)->last_status_byte, nfc_strerror(pnd), res);
    } else {
        pnd->last_error = 0;
    }
    return res;
...
{
    uint8_t ui8Value =
        (bEnable) ? (CHIP_DATA(pnd)->ui8Parameters | ui8Parameter) : (CHIP_DATA(pnd)->ui8Parameters & ~(ui8Parameter));
...
static int pn53x_ReadRegister(struct nfc_device *pnd, uint16_t ui16RegisterAddress, uint8_t *ui8Value)
{
...
int pn53x_write_register(struct nfc_device *pnd, const uint16_t ui16RegisterAddress, const uint8_t ui8SymbolMask,
                         const uint8_t ui8Value)
{
    if ((ui16RegisterAddress < PN53X_CACHE_REGISTER_MIN_ADDRESS) ||
        ...
        CHIP_DATA(pnd)->wb_data[internal_address] =
            (CHIP_DATA(pnd)->wb_data[internal_address] & CHIP_DATA(pnd)->wb_mask[internal_address] & (~ui8SymbolMask)) |
            (ui8Value & ui8SymbolMask);
...
int pn53x_writeback_register(struct nfc_device *pnd)
{
    ...
    if (BUFFER_SIZE(abtReadRegisterCmd) > 1) {
        ...
        for (size_t n = 0; n < PN53X_CACHE_REGISTER_SIZE; n++) {
            if ((CHIP_DATA(pnd)->wb_mask[n]) && (CHIP_DATA(pnd)->wb_mask[n] != 0xff)) {
                CHIP_DATA(pnd)->wb_data[n] = ((CHIP_DATA(pnd)->wb_data[n] & CHIP_DATA(pnd)->wb_mask[n]) |
...
int pn53x_decode_firmware_version(struct nfc_device *pnd)
{
    ...
    szFwLen = (size_t)res;
    // Determine which version of chip it is: PN531 will return only 2 bytes, while others return 4 bytes and have the
    // first to tell the version IC
    if (szFwLen == 2) {
    ...
    case PN531:
        snprintf(CHIP_DATA(pnd)->firmware_text, sizeof(CHIP_DATA(pnd)->firmware_text), "PN531 v%d.%d", abtFw[0],
                 abtFw[1]);
        pnd->btSupportByte = SUPPORT_ISO14443A | SUPPORT_ISO18092;
        break;
    case PN532:
        snprintf(CHIP_DATA(pnd)->firmware_text, sizeof(CHIP_DATA(pnd)->firmware_text), "PN532 v%d.%d", abtFw[1],
                 abtFw[2]);
        pnd->btSupportByte = abtFw[3];
        break;
    case PN533:
    case RCS360:
        snprintf(CHIP_DATA(pnd)->firmware_text, sizeof(CHIP_DATA(pnd)->firmware_text), "PN533 v%d.%d", abtFw[1],
                 abtFw[2]);
        pnd->btSupportByte = abtFw[3];
        break;
    case PN53X:
...
static uint8_t pn53x_int_to_timeout(const int ms)
{
    uint8_t res = 0;
    if (ms) {
        res = 0x10;
...
int pn53x_set_property_int(struct nfc_device *pnd, const nfc_property property, const int value)
{
    ...
    case NP_TIMEOUT_ATR:
        ...
        return pn53x_RFConfiguration__Various_timings(pnd, pn53x_int_to_timeout(CHIP_DATA(pnd)->timeout_atr),
...
void pn53x_initiator_init_iclass_modulation(struct nfc_device *pnd)
{
    ...
    //// set bit 4 for target mode? - RxWaitRF Set to logic 1, the counter for RxWait starts only if an external RF
    /// field is detected in Target mode for NFCIP-1 or in Card Communication mode
    ...
    // GsNOffReg - Selects the conductance for the N-driver of the antenna driver pins TX1 and TX2 when the driver is
    // switched off.
    // pn512_write_register(0x23, "\x88", 1, false);
    pn53x_WriteRegister(pnd, PN53X_REG_CIU_GsNOFF, 0x88);

    // GsNOnReg - Selects the conductance for the N-driver of the antenna driver pins TX1 and TX2 when the driver is
    // switched on.
...
static int pn53x_initiator_select_passive_target_ext(struct nfc_device *pnd, const nfc_modulation nm,
    ...
    if (nm.nmt == NMT_ISO14443BI || nm.nmt == NMT_ISO14443B2SR || nm.nmt == NMT_ISO14443B2CT ||
        ...
        do {
            if (nm.nmt == NMT_ISO14443B2SR) {
                ...
                if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_TxAuto, 0xef, 0x07)) <
                    0) // Initial RFOn, Tx2 RFAutoEn, Tx1 RFAutoEn
                    return res;
                if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_CWGsP, 0x3f, 0x3f)) <
                    0) // Conductance of the P-Driver
                    return res;
                if ((res = pn53x_write_register(pnd, PN53X_REG_CIU_ModGsP, 0x3f, 0x12)) <
                    0) // Driver P-output conductance for the time of modulation
                    return res;

                // Getting random Chip_ID
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtInitiate, szInitiateLen, abtRx, sizeof(abtRx),
                                                            timeout)) < 0) {
                ...
                abtSelect[1] = abtRx[0];
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtSelect, sizeof(abtSelect), abtRx, sizeof(abtRx),
                                                            timeout)) < 0) {
                    return res;
                }
                szTargetsData = (size_t)res;
                if ((res = pn53x_initiator_transceive_bytes(pnd, pbtInitData, szInitData, abtTargetsData,
                                                            sizeof(abtTargetsData), timeout)) < 0) {
                    if ((res == NFC_ERFTRANS) && (CHIP_DATA(pnd)->last_status_byte == 0x01)) { // Chip timeout
                        continue;
                    } else
                        return res;
                ...
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtReqt, sizeof(abtReqt), abtTargetsData + 2,
                                                            sizeof(abtTargetsData) - 2, timeout)) < 0) {
                    if ((res == NFC_ERFTRANS) && (CHIP_DATA(pnd)->last_status_byte == 0x01)) { // Chip timeout
                        continue;
                    } else
                        return res;
                ...
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtRead, sizeof(abtRead), abtTargetsData + 4,
                                                            sizeof(abtTargetsData) - 4, timeout)) < 0) {
                    return res;
                }
                ...
                if (pn53x_initiator_transceive_bytes(pnd, abtReqt, sizeof(abtReqt), NULL, 0, timeout) < 0) {
                    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG,
                            "got expected timeout on iClass activate all");
                    // if ((res == NFC_ERFTRANS) && (CHIP_DATA(pnd)->last_status_byte == 0x01)) { // Chip timeout
                    //   continue;
                    // } else
                    //   return res;
                ...
                if ((res = pn53x_initiator_transceive_bytes(pnd, abtReqt, sizeof(abtReqt), &abtAnticol[1],
                                                            sizeof(abtAnticol) - 1, timeout)) < 0) {
                ...
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG,
                        "iClass raw UID: %02x %02x %02x %02x %02x %02x %02x %02x", abtTargetsData[0], abtTargetsData[1],
                        abtTargetsData[2], abtTargetsData[3], abtTargetsData[4], abtTargetsData[5], abtTargetsData[6],
                        abtTargetsData[7]);
                szTargetsData = 8;
                nttmp.nm      = nm;
                if ((res = pn53x_decode_target_data(abtTargetsData, szTargetsData, CHIP_DATA(pnd)->type, nm.nmt,
                                                    &(nttmp.nti))) < 0) {
                    return res;
                }
            } else {
        ...
        if ((!CHIP_DATA(pnd)->progressive_field) &&
            (res = nfc_device_set_property_bool(pnd, NP_ACTIVATE_FIELD, false)) < 0) {
            return res;
        }
        ...
        if ((res = pn53x_InListPassiveTarget(pnd, pm, 1, pbtInitData, szInitData, abtTargetsData, &szTargetsData,
                                             timeout)) <= 0)
            return res;
        ...
        nttmp.nm = nm;
        if ((res = pn53x_decode_target_data(abtTargetsData + 1, szTargetsData - 1, CHIP_DATA(pnd)->type, nm.nmt,
                                            &(nttmp.nti))) < 0) {
            return res;
        }
...
int pn53x_initiator_select_passive_target(struct nfc_device *pnd, const nfc_modulation nm, const uint8_t *pbtInitData,
...
int pn53x_initiator_poll_target(struct nfc_device *pnd, const nfc_modulation *pnmModulations,
                                const size_t szModulations, const uint8_t uiPollNr, const uint8_t uiPeriod,
                                nfc_target *pnt)
{
    int res = 0;
        ...
        do {
            for (size_t p = 0; p < uiPollNr; p++) {
                for (size_t n = 0; n < szModulations; n++) {
                    ...
                    if ((res = pn53x_initiator_select_passive_target_ext(pnd, pnmModulations[n], pbtInitiatorData,
                                                                         szInitiatorData, pnt, timeout_ms)) < 0) {
...
int pn53x_initiator_select_dep_target(struct nfc_device *pnd, const nfc_dep_mode ndm, const nfc_baud_rate nbr,
                                      const nfc_dep_info *pndiInitiator, nfc_target *pnt, const int timeout)
{
    const uint8_t abtPassiveInitiatorData[] = {0x00, 0xff, 0xff, 0x00,
                                               0x0f}; // Only for 212/424 kpbs: First 4 bytes shall be set like this
                                                      // according to NFCIP-1, last byte is TSN (Time Slot Number)
    ...
    int res;
    if (pndiInitiator) {
        res = pn53x_InJumpForDEP(pnd, ndm, nbr, pbtPassiveInitiatorData, pndiInitiator->abtNFCID3, pndiInitiator->abtGB,
                                 pndiInitiator->szGB, pnt, timeout);
    } else {
    ...
    if (pbtRx != NULL) {
        if (szRxLen > szRx) {
            log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
                    "Buffer size is too short: %" PRIuPTR " available(s), %" PRIuPTR " needed", szRx, szRxLen);
            return NFC_EOVFLOW;
        }
        // Copy the received bytes
...
static void __pn53x_init_timer(struct nfc_device *pnd, const uint32_t max_cycles)
{
    ...
    // Initialize timer
    pn53x_write_register(pnd, PN53X_REG_CIU_TMode, 0xFF,
                         SYMBOL_TAUTO | ((CHIP_DATA(pnd)->timer_prescaler >> 8) & SYMBOL_TPRESCALERHI));
...
static uint32_t __pn53x_get_timer(struct nfc_device *pnd, const uint8_t last_cmd_byte)
{
    ...
    if (counter == 0) {
        ...
        parity = (last_cmd_byte >> 7) ^ ((last_cmd_byte >> 6) & 1) ^ ((last_cmd_byte >> 5) & 1) ^
                 ((last_cmd_byte >> 4) & 1) ^ ((last_cmd_byte >> 3) & 1) ^ ((last_cmd_byte >> 2) & 1) ^
...
static int pn53x_ISO14443A_Barcode_is_present(struct nfc_device *pnd)
{
    ...
    // We turn RF field off first for a better detection rate but this doesn't work well with ASK LoGO
    if ((!CHIP_DATA(pnd)->progressive_field) &&
        (ret = nfc_device_set_property_bool(pnd, NP_ACTIVATE_FIELD, false)) < 0) {
        return ret;
    }
...
static int pn53x_ISO14443A_MFC_is_present(struct nfc_device *pnd)
{
    ...
    if ((CHIP_DATA(pnd)->type == PN533) && (CHIP_DATA(pnd)->current_target->nti.nai.btSak != 0x09)) {
        ...
        iso14443_cascade_uid(CHIP_DATA(pnd)->current_target->nti.nai.abtUid,
                             CHIP_DATA(pnd)->current_target->nti.nai.szUidLen, pbtInitiatorData, &szInitiatorData);
        if ((ret = pn53x_set_property_bool(pnd, NP_INFINITE_SELECT, false)) < 0)
            return ret;
        if ((ret = pn53x_initiator_select_passive_target_ext(pnd, CHIP_DATA(pnd)->current_target->nm, pbtInitiatorData,
                                                             szInitiatorData, NULL, 300)) == 1) {
            ret = NFC_SUCCESS;
...
static int pn53x_ISO14443B_ICLASS_is_present(struct nfc_device *pnd)
{
    ...
    abtReqt[0]    = 0x0c; // iClass SELECT
    abtAnticol[0] = 0x81; // iClass ANTICOL
    if (pn53x_initiator_transceive_bytes(pnd, abtReqt, sizeof(abtReqt), &abtAnticol[1], sizeof(abtAnticol) - 1,
                                         timeout) < 0) {
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "timeout on iClass anticol");
        return NFC_ETGRELEASED;
        ;
...
int pn53x_target_init(struct nfc_device *pnd, nfc_target *pnt, uint8_t *pbtRx, const size_t szRxLen, int timeout)
{
    ...
    case NMT_ISO14443A: {
        ...
        // Set UID
        // Note: in this mode we can only emulate a single size (4 bytes) UID where the first is hard-wired by PN53x as
        // 0x08
    ...
    while (!targetActivated) {
        ...
        if ((res = pn53x_TgInitAsTarget(pnd, ptm, pbtMifareParams, pbtTkt, szTkt, pbtFeliCaParams, pbtNFCID3t, pbtGBt,
                                        szGBt, pbtRx, szRxLen, &btActivatedMode, timeout)) < 0) {
            if (res == NFC_ETIMEOUT) {
                pn53x_idle(pnd);
            }
            return res;
        ...
        szRx               = (size_t)res;
        nfc_modulation nm  = {.nmt = NMT_DEP, // Silent compilation warnings
                              .nbr = NBR_UNDEFINED};
        ...
        if (pnt->nm.nmt == nm.nmt) { // Actual activation have the right modulation type
            if ((pnt->nm.nbr == NBR_UNDEFINED) || (pnt->nm.nbr == nm.nbr)) { // Have the right baud rate (or undefined)
                if ((pnt->nm.nmt != NMT_DEP) || (pnt->nti.ndi.ndm == NDM_UNDEFINED) ||
                    (pnt->nti.ndi.ndm == ndm)) { // Have the right DEP mode (or is not a DEP)
...
static struct sErrorMessage {
    ...
    {EBITCOUNT, "Erroneous Bit Count"}, // During an anti-collision/select operation (ISO/IEC14443-3 Type A and
                                        // ISO/IEC18092 106 kbps passive mode), an erroneous Bit Count has been detected
    {EFRAMING, "Framing Error"},        // Framing error during MIFARE operation
    {EBITCOLL,
     "Bit-collision"}, // An abnormal bit-collision has been detected during bit wise anti-collision at 106 kbps
    {ESMALLBUF, "Communication Buffer Too Small"}, // Communication buffer size insufficient
    {EBUFOVF,
     "Buffer Overflow"}, // RF Buffer overflow has been detected by the CIU (bit BufferOvfl of the register CIU_Error)
    {ERFPROTO, "RF Protocol Error"}, // RF Protocol error (see PN53x manual)
    {EOVHEAT, "Chip Overheating"},   // Temperature error: the internal temperature sensor has detected overheating, and
    ...
    /* DEP errors */
    {ERFTIMEOUT, "RF Timeout"}, // In active communication mode, the RF field has not been switched on in time by the
                                // counterpart (as defined in NFCIP-1 standard)
    {EDEPUNKCMD, "Unknown DEP Command"},
    {EDEPINVSTATE, "Invalid DEP State"}, // DEP Protocol: Invalid device state, the system is in a state which does not
                                         // allow the operation
    {ENAD, "NAD Missing in DEP Frame"},
    /* MIFARE */
    {EMFAUTH, "Mifare Authentication Error"},
    /* Misc */
    {EINVRXFRAM, "Invalid Received Frame"}, // DEP Protocol, Mifare or ISO/IEC14443-4: The data format does not match to
                                            // the specification.
    ...
    {ECID, "Card ID Mismatch"}, // ISO14443 type B: Card ID mismatch, meaning that the expected card has been exchanged
                                // with another one.
...
int pn53x_RFConfiguration__Various_timings(struct nfc_device *pnd, const uint8_t fATR_RES_Timeout,
                                           const uint8_t fRetryTimeout)
{
    uint8_t abtCmd[] = {
        RFConfiguration, RFCI_TIMING,
        0x00,             // RFU
...
{
    uint8_t abtCmd[] = {
        RFConfiguration, RFCI_RETRY_DATA,
        MaxRtyCOM // MaxRtyCOM, default: 0x00 (no retry, only one try), inifite: 0xff
    };
    return pn53x_transceive(pnd, abtCmd, sizeof(abtCmd), NULL, 0, -1);
}

int pn53x_RFConfiguration__MaxRetries(struct nfc_device *pnd, const uint8_t MxRtyATR, const uint8_t MxRtyPSL,
                                      const uint8_t MxRtyPassiveActivation)
{
    // Retry format: 0x00 means only 1 try, 0xff means infinite
    uint8_t abtCmd[] = {
        RFConfiguration, RFCI_RETRY_SELECT,
 ...
 * @param pbtInitiatorData Optional initiator data used for Felica, ISO14443B, Topaz Polling or for ISO14443A selecting
 * a specific UID
 * @param szInitiatorData Length of initiator data \a pbtInitiatorData
 * @param pbtTargetsData pointer on a pre-allocated byte array to receive TargetData[n] as described in pn53x user
 * manual
 * @param pszTargetsData size_t pointer where size of \a pbtTargetsData will be written
 *
 * @note Selected targets count can be found in \a pbtTargetsData[0] if available (i.e. \a pszTargetsData content is
 * more than 0)
...
int pn53x_InListPassiveTarget(struct nfc_device *pnd, const pn53x_modulation pmInitModulation,
                              const uint8_t szMaxTargets, const uint8_t *pbtInitiatorData, const size_t szInitiatorData,
                              uint8_t *pbtTargetsData, size_t *pszTargetsData, int timeout)
{
    ...
    // Set the optional initiator data (used for Felica, ISO14443B, Topaz Polling or for ISO14443A selecting a specific
    // UID).
    if (pbtInitiatorData)
                     ...
                     const uint8_t btPollNr, const uint8_t btPeriod, nfc_target *pntTargets, const int timeout)
{
    ...
    // InAutoPoll frame looks like this { 0xd4, 0x60, 0x0f, 0x01, 0x00 } => { direction, command, pollnr, period,
    // types... }
    ...
    if (res < 0) {
        ...
        if (szTargetFound > 0) {
            ...
            ln = *(pbt++);
            if ((res = pn53x_decode_target_data(pbt, ln, CHIP_DATA(pnd)->type, pntTargets[0].nm.nmt,
                                                &(pntTargets[0].nti))) < 0) {
                return res;
            }
            pbt += ln;
 ...
 * @param pbtPassiveInitiatorData NFCID1 (4 bytes) at 106kbps (optionnal, see NFCIP-1: 11.2.1.26) or Polling Request
 * Frame's payload (5 bytes) at 212/424kbps (mandatory, see NFCIP-1: 11.2.2.5)
 ...
 */
int pn53x_InJumpForDEP(struct nfc_device *pnd, const nfc_dep_mode ndm, const nfc_baud_rate nbr,
                       const uint8_t *pbtPassiveInitiatorData, const uint8_t *pbtNFCID3i, const uint8_t *pbtGBi,
                       const size_t szGBi, nfc_target *pnt, const int timeout)
{
    // Max frame size = 1 (Command) + 1 (ActPass) + 1 (Baud rate) + 1 (Next) + 5 (PassiveInitiatorData) + 10 (NFCID3) +
    // 48 (General bytes) = 67 bytes
    uint8_t abtCmd[67] = {InJumpForDEP, (ndm == NDM_ACTIVE) ? 0x01 : 0x00};

    size_t offset = 4; // 1 byte for command, 1 byte for DEP mode (Active/Passive), 1 byte for baud rate, 1 byte for
                       // following parameters flag

    switch (nbr) {
    case NBR_106:
        abtCmd[2] = 0x00; // baud rate is 106 kbps
        if (pbtPassiveInitiatorData &&
            (ndm == NDM_PASSIVE)) { /* can't have passive initiator data when using active mode */
            abtCmd[3] |= 0x01;
...
int pn53x_TgInitAsTarget(struct nfc_device *pnd, pn53x_target_mode ptm, const uint8_t *pbtMifareParams,
                         const uint8_t *pbtTkt, size_t szTkt, const uint8_t *pbtFeliCaParams, const uint8_t *pbtNFCID3t,
                         const uint8_t *pbtGBt, const size_t szGBt, uint8_t *pbtRx, const size_t szRxLen,
                         uint8_t *pbtModeByte, int timeout)
{
    uint8_t abtCmd[39 + 47 + 48] = {TgInitAsTarget}; // Worst case: 39-byte base, 47 bytes max. for General Bytes, 48
                                                     // bytes max. for Historical Bytes
...
int pn53x_build_frame(uint8_t *pbtFrame, size_t *pszFrame, const uint8_t *pbtData, const size_t szData)
{
    if (szData <= PN53x_NORMAL_FRAME__DATA_MAX_LEN) {
        ...
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
                "We can't send more than %d bytes in a raw (requested: %" PRIdPTR ")",
...
int pn53x_get_supported_baud_rate(nfc_device *pnd, const nfc_mode mode, const nfc_modulation_type nmt,
