...
struct card_atqa const_ca[] = {{0x0044, 0xffff, "MIFARE Ultralight", {0, -1}},
    ...
    if (pnai->szAtsLen && verbose) {
        ...
        if (pnai->szAtsLen > offset) {
            ...
            if (CIB != 0x00 && CIB != 0x10 && (CIB & 0xf0) != 0x80) {
                ...
                if (CIB == 0xc1) {
                    off +=
                        snprintf(dst + off, size - off, "    * Tag byte: Mifare or virtual cards of various types\n");
                    uint8_t L = pnai->abtAts[offset];
                    offset++;
                    if (L != (pnai->szAtsLen - offset)) {
                        off +=
                            snprintf(dst + off, size - off, "    * Warning: Type Identification Coding length (%i)", L);
    ...
    if (verbose) {
        ...
        for (i = 0; i < sizeof(const_ca) / sizeof(const_ca[0]); i++) {
            if ((atqa & const_ca[i].mask) == const_ca[i].atqa) {
                for (j = 0; (j < sizeof(const_ca[i].saklist) / sizeof(const_ca[i].saklist[0])) &&
                            (const_ca[i].saklist[j] >= 0);
                     j++) {
...
void snprint_nfc_target(char *dst, size_t size, const nfc_target *pnt, bool verbose)
{
    if (NULL != pnt) {
        ...
        off += snprintf(dst + off, size - off, "%s (%s%s) target:\n", str_nfc_modulation_type(pnt->nm.nmt),
                        str_nfc_baud_rate(pnt->nm.nbr),
                        (pnt->nm.nmt != NMT_DEP)           ? ""
                        : (pnt->nti.ndi.ndm == NDM_ACTIVE) ? "active mode"
                                                           : "passive mode");
        switch (pnt->nm.nmt) {
        case NMT_ISO14443A:
