    ...
    do {                                                                                                               \
        for (size_t i = 0; i < (sizeof(pn53x_registers) / sizeof(pn53x_register)); i++) {                              \
            if (X == pn53x_registers[i].ui16Address) {                                                                 \
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "%s (%s)",                                    \
...
static const pn53x_register pn53x_registers[] = {
    ...
    PNREG(PN53X_REG_CIU_GsNOFF, "Selects the conductance of the antenna driver pins TX1 and TX2 for load modulation "
                                "when own RF field is switched OFF"),
    ...
    PNREG(PN53X_REG_CIU_GsNOn, "Selects the conductance of the antenna driver pins TX1 and TX2 for modulation, when "
                               "own RF field is switched ON"),
    PNREG(PN53X_REG_CIU_CWGsP,
          "Selects the conductance of the antenna driver pins TX1 and TX2 when not in modulation phase"),
    PNREG(PN53X_REG_CIU_ModGsP,
          "Selects the conductance of the antenna driver pins TX1 and TX2 when in modulation phase"),
