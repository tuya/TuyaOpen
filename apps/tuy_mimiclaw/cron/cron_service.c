...
OPERATE_RET cron_service_start(void)
{
    ...
    MIMI_LOGI(TAG, "cron started (%d jobs, check every %u sec)",
              s_job_count, (unsigned)(MIMI_CRON_CHECK_INTERVAL_MS / 1000));
    return OPRT_OK;
}
...
OPERATE_RET cron_add_job(cron_job_t *job)
{
    ...
    MIMI_LOGI(TAG, "added cron job: %s (%s) kind=%s next=%lld",
              job->name,
              job->id,
