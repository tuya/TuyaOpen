 ...
 * --------------------------------------------------------------------------- */
static void *s_speex = NULL;
static void *s_rnn_vad = NULL;
...
OPERATE_RET demo_audio_aec_init(uint32_t min_speech_len_ms, uint32_t max_speech_interval_ms, uint32_t frame_bytes)
{
    ...
    if (frame_bytes == 0U || (frame_bytes % 2U) != 0U) {
        PR_ERR("invalid frame_bytes=%u", (uint32_t)frame_bytes);
        return OPRT_INVALID_PARM;
    }

    /* OS: speex_aes_create(frame_size / 2) — frame_size is bytes */
    s_speex = speex_aes_create((int)(frame_bytes / 2U));
    if (s_speex == NULL) {
        PR_ERR("speex_aes_create failed");
        return OPRT_COM_ERROR;
    }
    (void) speex_aes_set_param(s_speex, DEMO_AEC_SPEEX_LEVEL);
    (void) speex_ns_set_param(s_speex, DEMO_AEC_NS_LEVEL1, DEMO_AEC_NS_LEVEL2);
    ...
    (void) rnn_vad_init(&vad_param, s_rnn_vad);
    ...
    if (s_linearaec == NULL) {
        PR_ERR("linearaec alloc failed size=%u", (uint32_t)(frame_bytes * 2U));
        demo_audio_aec_deinit();
    ...
    PR_NOTICE("demo Speex+RNN ready frame_bytes=%u speech=%u/%u thr=%d",
              (uint32_t)frame_bytes, (uint32_t)min_speech_len_ms, (uint32_t)max_speech_interval_ms,
...
int demo_audio_aec_process(int16_t *mic_data, int16_t *ref_data, int16_t *out_data)
{
    ...
    (void) speex_aes_process(s_speex, (short *)mic_data, (short *)ref_data, (short *)out_data);
    ...
    if (s_linearaec != NULL) {
        (void) speex_get_param(s_speex, NULL, (short *)s_linearaec);
    }

    if ((s_cnt++ % 500U) == 0U) {
        PR_DEBUG("speex+rnn n=%u vad=%d", (uint32_t)s_cnt, (int)s_vad_flag);
    }
    return 0;
}
