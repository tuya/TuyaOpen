 ...
 * @brief IPC demo media callbacks: live camera over P2P
 * @version 2.0
 * @date 2026-08-28
...
void tuya_ipc_demo_start(void)
{
    ...
    PR_NOTICE("tuya_ipc_demo: ready (source=%s, sensor %ux%u@%u, av fps %u, live_rec=%d)",
              have_camera ? "camera" : "none", (uint32_t)DEMO_CAM_WIDTH, (uint32_t)DEMO_CAM_HEIGHT,
...
static void __demo_mic_frame_cb(const int16_t *pcm, uint32_t samples)
{
...
#if DEMO_LIVE_RECORD
    if (local_store_rec_is_open()) {
        uint8_t  rec[DEMO_AUDIO_FRAME_BYTES];
        uint32_t roff;

        for (roff = 0; roff + DEMO_AUDIO_FRAME_BYTES <= in_n; roff += DEMO_AUDIO_FRAME_BYTES) {
            uint32_t i;

            for (i = 0; i < DEMO_AUDIO_FRAME_BYTES; i++) {
                rec[i] = linear2ulaw(pcm8k[roff + i]);
            }
            (void)local_store_rec_write_audio(rec, DEMO_AUDIO_FRAME_BYTES,
                                              ts0 + (uint64_t)(roff * 1000u / DEMO_AUDIO_PORT_RATE));
        }
    }
#endif
    ...
    for (off = 0; off + DEMO_AUDIO_FRAME_BYTES <= in_n; off += DEMO_AUDIO_FRAME_BYTES) {
        ...
        for (i = 0; i < DEMO_AUDIO_FRAME_BYTES; i++) {
            g711[i] = linear2ulaw(pcm8k[off + i]);
        }
...
int demo_on_recv_audio_frame_callback(MEDIA_FRAME *media_frame)
{
    ...
    for (i = 0; i < n; i++) {
        pcm8k[i] = (int16_t)ulaw2linear(((const uint8_t *)media_frame->data)[i]);
    }
