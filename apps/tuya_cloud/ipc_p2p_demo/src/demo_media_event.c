...
static void __pb_send_thread(void *arg)
{
    ...
    while (s_pb_running) {
        ...
        if (hdr.type == LOCAL_STORE_FRAME_AUDIO) {
            MEDIA_AUDIO_FRAME_T af;

            memset(&af, 0, sizeof(af));
            af.audio_codec = TUYA_CODEC_AUDIO_G711U;
            af.audio_sample = TUYA_AUDIO_SAMPLE_8K;
            af.audio_databits = TUYA_AUDIO_DATABITS_16;
            af.audio_channel = TUYA_AUDIO_CHANNEL_MONO;
            af.p_audio_buf = (uint8_t *)payload;
            af.buf_len = hdr.size;
            af.timestamp = hdr.timestamp;
            af.pts = hdr.timestamp * 1000ULL;

            rt = tuya_ipc_media_playback_send_audio_frame(0, &af);
            if (rt == OPRT_NOT_EXIST || rt == OPRT_SOCK_CONN_ERR) {
                PR_NOTICE("pb audio stop, playback not active rt=%d cnt=%u", rt, frame_cnt);
                s_pb_running = FALSE;
                break;
            }
            if (rt == OPRT_OK) {
