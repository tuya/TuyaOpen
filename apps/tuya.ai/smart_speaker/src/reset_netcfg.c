...
static void reset_netcfg_timer(TIMER_ID timer_id, void *arg)
{
    PR_DEBUG("%s", __func__);
    reset_count_write(0);
...
static OPERATE_RET _reset_netcfg_clear(void *data)
{
    PR_DEBUG("%s", __func__);
    reset_count_write(0);
...
int reset_netcfg_check(void)
{
    ...
    tal_event_subscribe(EVENT_RESET, "reset_netconfig", _reset_netcfg_clear, SUBSCRIBE_TYPE_NORMAL);
...
int reset_netcfg_start(void)
{
    ...
    tal_sw_timer_create(reset_netcfg_timer, NULL, &rst_config_timer);
