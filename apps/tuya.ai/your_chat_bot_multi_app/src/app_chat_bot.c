...
static void __app_button_function_cb(char *name, TDL_BUTTON_TOUCH_EVENT_E event, void *argc)
{
    ...
    case TDL_BUTTON_LONG_PRESS_HOLD: {
        PR_DEBUG("button long press hold");
...
static OPERATE_RET __app_open_button(void)
{
    ...
    tdl_button_event_register(sg_button_hdl, TDL_BUTTON_LONG_PRESS_HOLD, __app_button_function_cb);
    return rt;
}
#endif
...
OPERATE_RET ai_audio_player_play_alert(AI_AUDIO_ALERT_TYPE_E type)
{
    ...
    //                                     1);
    // } break;
    case AI_AUDIO_ALERT_MUYU: {
        rt = ai_audio_player_data_write(alert_id, (uint8_t *)media_src_muyu_zh, sizeof(media_src_muyu_zh),1);
    } break;
