...
static void __ui_light_theme_init(APP_THEME_COLORS_T *theme)
{
    ...
    PR_DEBUG("------------->__ui_light_theme_init");
}
...
void ui_init(void)
{
    ...
    // if (sg_ui.ui.screen != NULL) {
    //     return ;
    // }
    sg_ui.ui.screen = lv_screen_active();
    lv_obj_set_style_text_font(sg_ui.ui.screen, sg_ui.font.text, 0);
    lv_obj_set_style_text_color(sg_ui.ui.screen, sg_ui.theme.text, 0);
    lv_obj_set_style_bg_color(sg_ui.ui.screen, sg_ui.theme.background, 0);

    // Container
    sg_ui.ui.container = lv_obj_create(sg_ui.ui.screen);
    ...
    lv_obj_set_style_bg_color(sg_ui.ui.content, lv_color_hex(0xF0F0F0), 0);
    ...
    lv_obj_add_event_cb(sg_ui.ui.screen, _ui_enent_Gesture, LV_EVENT_GESTURE, NULL);
    return ;
}

void ui_deinit(void)
{
    lv_obj_del(sg_ui.ui.container);
}
