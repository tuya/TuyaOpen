...
OPERATE_RET audio_dp_obj_proc(dp_obj_recv_t *dpobj)
{
    ...
    for (index = 0; index < dpobj->dpscnt; index++) {
        ...
        case SWITCH_DP_ID: {
            bool led_state = dp->value.dp_bool;
            PR_DEBUG("led_state:%d", led_state);
            app_set_led_onoff(led_state);
            break;
        }
        case BRIGHT_VALUE_DP_ID: {
            int brightness = dp->value.dp_value;
            app_set_led_brightness(brightness);
...
void user_main(void)
{
    ...
    ret = tal_uart_init(USR_UART_NUM, &cfg);
    if (ret != OPRT_OK) {
        PR_ERR("tal_uart_init failed");
    }
