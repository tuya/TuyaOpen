...
title BLE Peripheral 工作流程

start
#pink:入口函数：tuya_app_main;
#palegreen:蓝牙初始化：tal_ble_bk_init;
#palegreen:设置蓝牙广播包：tal_ble_advertising_data_set;
#palegreen:开启蓝牙广播：tal_ble_advertising_start;
#palegreen:等待主机连接 \n     ......;
->连接成功;
#palegreen:蓝牙进入连接状态;
#palegreen:读取特征值：tal_ble_server_common_read_update;
#palegreen:进入参数更新事件，\n 更新主机从机的 \n 连接参数;
#palegreen:进入 MTU 交换请求事件，\n 选择主机和从机中两者最小的MTU;
#palegreen:主机向从机发送数据;
#palegreen:进入蓝牙数据接收事件;
#palegreen:主机断开连接;
#palegreen:进入蓝牙断开连接事件;
#palegreen:重新开启蓝牙广播：tal_ble_advertising_start;
#pink:结束;
@enduml
```
...
```
打开手机 `nRF connect` [APP](https://play.google.com/store/apps/details?id=no.nordicsemi.android.mcp&hl=en&gl=US)，扫描名为 `TY` 的蓝牙设备，点击连接。
