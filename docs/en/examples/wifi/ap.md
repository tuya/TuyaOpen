...
+ 1. STA模式：STA模式是WIFI模块的工作模式之一，它是一种无线客户端模式，用于连接到现有的无线网络。在STA模式下，WIFI模块作为一个客户端，连接到一个现有的无线网络，与其他设备进行数据传输和通信。
+ 2. AP模式：AP模式是WIFI模块的工作模式之一，它是一种无线接入点模式，用于创建一个无线网络。在AP模式下，WIFI模块作为一个服务器，创建一个无线网络，其他设备可以连接到该无线网络，与WIFI模块进行数据传输和通信。
+ 3. STA+AP模式：STA+AP模式是WIFI模块的工作模式之一，它是一种混合模式，用于同时支持STA模式和AP模式。在STA+AP模式下，WIFI模块既可以作为一个客户端连接到现有的无线网络，也可以作为一个服务器创建一个无线网络，同时支持两种工作模式。
...
#palegreen:WiFi 初始化：tal_wifi_init;
#palegreen:WiFi 模式设置为 AP 模式：tal_wifi_set_work_mode;
#palegreen:启动 AP：tal_wifi_ap_start;
#palegreen:repeat:UDP 广播：tal_wifi_ap_send_broadcast is;
repeat while () is (每5s扫描);
#pink:结束;
end
@enduml
```
