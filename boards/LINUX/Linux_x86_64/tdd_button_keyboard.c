...
static MUTEX_HANDLE sg_stdin_spill_mtx;
static char         sg_stdin_spill_buf[STDIN_SPILL_CAP];
static UINT_T       sg_stdin_spill_head;
static UINT_T       sg_stdin_spill_tail;
static UINT_T       sg_stdin_spill_count;
static BOOL_T       sg_stdin_spill_inited;

static void __stdin_spill_init_once(void)
{
    if (sg_stdin_spill_inited) {
        return;
    }
    if (tal_mutex_create_init(&sg_stdin_spill_mtx) == OPRT_OK) {
        sg_stdin_spill_inited = TRUE;
    }
}

static void __stdin_spill_push(char ch)
{
    __stdin_spill_init_once();
    if (!sg_stdin_spill_inited) {
        PR_WARN("keyboard stdin spill: drop ch=0x%02x (init failed)", (unsigned char)ch);
        return;
    }

    tal_mutex_lock(sg_stdin_spill_mtx);
    if (sg_stdin_spill_count >= STDIN_SPILL_CAP) {
        sg_stdin_spill_head = (sg_stdin_spill_head + 1) % STDIN_SPILL_CAP;
        sg_stdin_spill_count--;
        PR_WARN("keyboard stdin spill: overflow, dropping oldest byte");
    }
    sg_stdin_spill_buf[sg_stdin_spill_tail] = ch;
    sg_stdin_spill_tail                     = (sg_stdin_spill_tail + 1) % STDIN_SPILL_CAP;
    sg_stdin_spill_count++;
    tal_mutex_unlock(sg_stdin_spill_mtx);

    PR_DEBUG("keyboard stdin spill: push ch=0x%02x '%c' (queued=%u)", (unsigned char)ch,
             (ch >= 32 && ch < 127) ? ch : '.', sg_stdin_spill_count);
}

BOOL_T tdd_keyboard_stdin_spill_pop(char *ch)
{
    if (ch == NULL) {
        return FALSE;
    }
    ...
    tal_mutex_lock(sg_stdin_spill_mtx);
    if (sg_stdin_spill_count == 0) {
        tal_mutex_unlock(sg_stdin_spill_mtx);
        return FALSE;
    }

    *ch                 = sg_stdin_spill_buf[sg_stdin_spill_head];
    sg_stdin_spill_head = (sg_stdin_spill_head + 1) % STDIN_SPILL_CAP;
    sg_stdin_spill_count--;
    tal_mutex_unlock(sg_stdin_spill_mtx);
    return TRUE;
...
UINT_T tdd_keyboard_stdin_spill_pending(void)
{
    UINT_T pending = 0;
...
static OPERATE_RET __tdd_button_read_value(TDL_BUTTON_OPRT_INFO *dev, uint8_t *value)
{
    ...
    *value = p_ctx->key_pressed;
    ...
    // Non-button keys are forwarded to voice_app_cli via the spill queue.
    while ((ret = read(STDIN_FILENO, &ch, 1)) > 0) {
        ...
        if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9')) {
            ...
            if (ch_upper == name_upper) {
                ...
                *value                 = 1;
                PR_DEBUG("keyboard button '%s': matched key '%c'", p_ctx->name, ch);
            }
        }
