...
static void __sdl_present_locked(TDD_DISP_SDL_T *dev)
{
    ...
    if (dev->use_surface) {
        ...
        int conv_rt = SDL_ConvertPixels((int)dev->width, (int)dev->height, SDL_PIXELFORMAT_ARGB8888, dev->conv_argb,
                                        (int)dev->width * 4, surf->format->format, surf->pixels, surf->pitch);
...
static void __sdl_thread_cb(void *args)
{
    ...
    PR_NOTICE("SDL video driver: %s", SDL_GetCurrentVideoDriver());
    {
        ...
        PR_NOTICE("env DISPLAY=%s WAYLAND_DISPLAY=%s XDG_SESSION_TYPE=%s", disp ? disp : "(null)",
                  wayland ? wayland : "(null)", session ? session : "(null)");
    }
    ...
    dev->window = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, dev->width, dev->height,
                                   SDL_WINDOW_SHOWN);
    if (!dev->window) {
    ...
    if (dev->use_surface) {
        SDL_Surface *surf = SDL_GetWindowSurface(dev->window);
        if (surf && surf->format) {
            PR_NOTICE("SDL window surface fmt=0x%x bpp=%u pitch=%d", surf->format->format, surf->format->BitsPerPixel,
                      surf->pitch);
        } else {
        ...
        dev->texture = SDL_CreateTexture(dev->renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING,
...
static OPERATE_RET __sdl_flush(TDD_DISP_DEV_HANDLE_T device, TDL_DISP_FRAME_BUFF_T *frame_buff)
{
    ...
    if (sg_sdl_flush_cnt == 0) {
        PR_NOTICE("SDL first flush: %ux%u fmt=%d len=%u", frame_buff->width, frame_buff->height, frame_buff->fmt,
                  frame_buff->len);
    }
    sg_sdl_flush_cnt++;
...
static OPERATE_RET __tp_read(TDD_TP_DEV_HANDLE_T device, uint8_t max_num, TDL_TP_POS_T *point, uint8_t *point_num)
{
    ...
    if (dev->swipe_active) {
        ...
        const int32_t step =
            __sdl_clamp_i32(dev->swipe_remain_px, -TUYA_SDL_WHEEL_SWIPE_STEP, TUYA_SDL_WHEEL_SWIPE_STEP);
