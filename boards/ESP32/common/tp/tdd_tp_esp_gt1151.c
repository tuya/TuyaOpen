/**
 * @file tdd_tp_esp_gt1151.c
 * @brief One-shot init + register helper for GT1151 capacitive touch panels via ESP-IDF esp_lcd_touch.
 *
...
/**
 * @brief Initialise a GT1151 touch controller and register it as a TuyaOpen TDD tp device.
 * @param[in] name Device name used for later lookup.
 * @param[in] cfg  I2C / GPIO / TP configuration.
 * @return OPRT_OK on success, error code otherwise.
 */
OPERATE_RET tdd_tp_esp_i2c_gt1151_register(char *name, TDD_TP_ESP_GT1151_CFG_T *cfg)
{
    i2c_master_bus_handle_t       i2c_bus      = NULL;
    ...
    /* Leave transaction_timeout_ms at its default (0 -> esp_lcd treats it as -1,
       an infinite wait), exactly like the official BSP. GT1151 shares I2C0 with
       the RGB panel's PSRAM DMA; when a DMA burst holds the bus, the read must
       BLOCK until the bus frees and then complete. A finite timeout (we tried
       100 ms) instead aborts that read with a NACK every time a refresh burst
       runs longer than the cap — the touch NACK we were chasing. */
    ...
    ESP_LOGI(TAG, "Initialize gt1151 touch controller");
    esp_rt = esp_lcd_touch_new_i2c_gt1151(tp_io_handle, &tp_cfg, &esp_tp_hdl);
    if (esp_rt != ESP_OK || NULL == esp_tp_hdl) {
        ESP_LOGE(TAG, "Failed to init gt1151: %s", esp_err_to_name(esp_rt));
        return OPRT_COM_ERROR;
    }
